...
TL;DR: 提出上下文增强对比损失函数，通过两个收敛目标解决对比学习中的信息失真问题，在8个基准数据集上优于16种现有方法，特别在系统性失真任务上表现突出。


<details>
  <summary>Details</summary>
Motivation: 传统对比学习中的对比损失函数虽然能有效辨别样本相似性，但可能引入信息失真。模型可能过度依赖相同标签的样本信息，而忽略来自同一原始图像的正样本对，特别是在大规模数据集中。这种信息失真会影响学习效果和模型性能。

Method: 提出上下文增强对比损失函数，包含两个收敛目标：1）标签敏感组件，区分相同和不同类别的特征，提高对比训练效率；2）上下文增强组件，使来自同一源图像的增强样本更接近，同时使所有其他样本远离，解决信息失真问题。

Result: 在8个大规模基准数据集（CIFAR10、CIFAR100、Caltech-101、Caltech-256、ImageNet、BiasedMNIST、UTKFace、CelebA）上评估，该方法在泛化性能和学习收敛速度方面均优于16种最先进的对比学习方法。特别在BiasedMNIST下游数据集上，相比原始对比损失函数有22.9%的改进。

Conclusion: 上下文增强对比损失函数能有效解决对比学习中的信息失真问题，提高学习效率和模型性能。该方法在系统性失真任务上表现突出，为更高效和公平的下游训练提供了有前景的解决方案。
...
中文标题: 上下文增强对比损失：在对比学习框架中增强样本内在联系的表示

中文摘要: 对比学习已获得广泛关注并在众多大规模基准测试中推动了最先进的性能。在对比学习中，对比损失函数通过旋转或裁剪等技术在辨别样本相似性方面发挥着关键作用。然而，这种学习机制也可能引入来自增强样本的信息失真。这是因为训练模型可能会过度依赖具有相同标签的样本信息，同时忽略源自同一原始图像的正样本对，尤其是在大规模数据集中。本文提出了一种上下文增强对比损失函数，通过包含两个收敛目标同时提高学习效果并解决信息失真问题。第一个组件对标签对比特别敏感，区分相同和不同类别的特征，从而提高对比训练效率。同时，第二个组件使来自同一源图像的增强样本更接近，并使所有其他样本远离。我们在图像分类任务上评估了所提出的方法，这些任务涵盖了8个广泛认可的大规模基准数据集：CIFAR10、CIFAR100、Caltech-101、Caltech-256、ImageNet、BiasedMNIST、UTKFace和CelebA数据集。实验结果表明，所提出的方法在泛化性能和学习收敛速度方面均优于16种最先进的对比学习方法。有趣的是，我们的技术在解决系统性失真任务方面表现突出。在BiasedMNIST下游数据集上，与原始对比损失函数相比，它展示了22.9%的改进，突显了其在更高效和公平的下游训练方面的潜力。
...
TL;DR: FineGRAIN提出了一种联合评估文本到图像模型和视觉语言模型的方法，通过测试VLM是否能识别T2I模型生成的图像中的27种特定失败模式，并创建了包含5个T2I模型生成图像和3个VLM标注的数据集。


<details>
  <summary>Details</summary>
Motivation: 文本到图像模型虽然能生成视觉上令人印象深刻的图像，但经常无法准确捕捉用户提示中的特定属性（如正确数量的对象和指定颜色）。同时，视觉语言模型的基准测试未能跟上VLM用于标注的复杂场景的需求。需要一种分层评估框架来比较不同图像生成模型的提示遵循能力。

Method: 提出结构化方法，通过测试VLM是否能识别T2I模型在挑战性提示下生成的图像中的27种特定失败模式。创建了包含5个T2I模型（Flux、SD3-Medium、SD3-Large、SD3.5-Medium、SD3.5-Large）生成图像和3个VLM（Molmo、InternVL3、Pixtral）标注的数据集，使用LLM（Llama3）测试VLM是否正确识别生成图像中的失败模式。

Result: 通过分析精选提示集上的失败模式，揭示了属性保真度和对象表示方面的系统性错误。发现当前指标不足以捕捉这些细微错误，强调了针对性基准测试对于提高生成模型可靠性和可解释性的重要性。

Conclusion: FineGRAIN框架为联合评估T2I模型和VLM提供了结构化方法，揭示了现有模型的系统性错误，并强调了需要更精细的评估基准来推动生成模型的发展。
...
中文摘要: 文本到图像模型能够生成视觉上令人印象深刻的图像，但它们经常无法准确捕捉用户提示中的特定属性，例如具有指定颜色的正确数量的对象。此类错误的多样性凸显了需要一个分层评估框架来比较不同图像生成模型的提示遵循能力。同时，视觉语言模型的基准测试未能跟上VLM用于标注的复杂场景的需求。在这项工作中，我们提出了一种结构化方法，通过测试VLM是否能识别文本到图像模型在挑战性提示下生成的图像中的27种特定失败模式，来联合评估T2I模型和VLM。我们的第二个贡献是一个数据集，包含5个T2I模型（Flux、SD3-Medium、SD3-Large、SD3.5-Medium、SD3.5-Large）生成的提示和图像，以及来自VLM（Molmo、InternVL3、Pixtral）的相应标注，这些标注由LLM（Llama3）标注，用于测试VLM是否正确识别生成图像中的失败模式。通过分析精选提示集上的失败模式，我们揭示了属性保真度和对象表示方面的系统性错误。我们的研究结果表明，当前指标不足以捕捉这些细微错误，强调了针对性基准测试对于提高生成模型可靠性和可解释性的重要性。
...
TL;DR: SelVA是一个文本条件选择性视频到音频生成模型，能够从多对象视频中仅生成用户指定的声音，解决了现有方法一次性生成混合声音的问题。


<details>
  <summary>Details</summary>
Motivation: 在多媒体制作中，音频轨道需要单独处理每个声源以进行精确编辑、混音和创意控制。然而，当前方法一次性生成混合声音，主要因为视觉特征纠缠，区域提示往往无法指定特定声源。

Method: 提出SelVA模型，将文本提示作为目标声源的显式选择器，调制视频编码器以提取提示相关视频特征。使用补充令牌通过抑制文本无关激活来促进交叉注意力，采用自增强方案克服单声道音频监督数据的缺乏。

Result: 在VGG-MONOAUDIO基准测试上，SelVA在音频质量、语义对齐和时间同步方面表现出色，实验和消融研究一致验证了其有效性。

Conclusion: SelVA成功解决了文本条件选择性视频到音频生成任务，为多媒体制作提供了精确的音频生成能力，通过显式文本选择和特征调制实现了目标声源的准确提取。
...
中文标题: 听重要的！文本条件选择性视频到音频生成

中文摘要: 这项工作引入了一个新任务：文本条件选择性视频到音频（V2A）生成，该任务仅从多对象视频中生成用户意图的声音。这种能力在多媒体制作中尤其重要，因为音频轨道需要为每个声源单独处理以进行精确编辑、混音和创意控制。然而，当前方法一次性生成混合声音，主要是因为视觉特征纠缠，区域提示往往无法指定声源。我们提出了SelVA，一种新颖的文本条件V2A模型，将文本提示作为目标声源的显式选择器，并调制视频编码器以清晰提取提示相关视频特征。提出的补充令牌通过抑制文本无关激活来促进交叉注意力，实现高效的参数调整，产生稳健的语义和时间定位。SelVA进一步采用自增强方案来克服单声道音频监督数据的缺乏。我们在VGG-MONOAUDIO上评估SelVA，这是一个为此任务策划的干净单源视频基准。广泛的实验和消融研究一致验证了其在音频质量、语义对齐和时间同步方面的有效性。代码和演示可在https://jnwnlee.github.io/selva-demo/获取。
...
<details>
  <summary>Details</summary>
Motivation: 癌症治疗的有效性依赖于早期诊断和治疗，医学影像是临床医生确定癌症诊断的关键初始工具，而遗传信息在后续阶段用于选择治疗方案。本研究旨在建立从医学影像到体细胞突变的映射，实现早期预测。

Method: 提出LLOST模型：1）将病灶图像转换为点云表示，实现成像模态不变性；2）使用双变分自编码器，分别处理病灶点云和体细胞突变计数；3）通过共享潜在空间耦合两个域的特征；4）每个潜在空间使用条件归一化流先验来适应各域的多样分布。

Result: 在癌症影像档案和癌症基因组档案的泛癌数据集上进行实验，模型在预测特定突变计数和突变发生方面表现出良好性能，并揭示了影像与体细胞突变域之间反映癌症类型的共享模式。

Conclusion: 该研究成功建立了从医学影像到体细胞突变的映射模型，为早期癌症诊断和治疗选择提供了新方法。未来可改进模型并扩展到其他遗传领域。
...
中文摘要: 医学影像是临床医生确定患者癌症诊断的关键初始工具，可实现快速干预和更可靠的患者预后。在患者诊断的后续阶段，提取遗传信息以帮助选择特定的治疗方案。由于癌症治疗的疗效通常依赖于早期诊断和治疗，我们构建了一个深度潜在变量模型，根据相应的医学影像确定患者的体细胞突变谱。我们首先引入病灶图像的点云表示，以实现对成像模态的不变性。然后提出LLOST模型，该模型具有双变分自编码器，通过一个单独的共享潜在空间耦合在一起，统一了病灶点云和不同体细胞突变计数的特征。因此，我们的模型包含三个潜在空间，每个空间都使用条件归一化流先验进行学习，以适应每个域的多样分布。我们在癌症影像档案的去标识化医学图像和癌症基因组档案的泛癌数据集中的相应体细胞突变上进行了定性和定量实验。我们展示了模型在预测特定突变计数方面的性能，以及准确预测突变发生的能力。特别是，揭示了影像和体细胞突变域之间反映癌症类型的共享模式。最后，我们讨论了如何改进模型以及未来可能的研究方向，包括扩展到其他遗传领域。
...
TL;DR: AV-SpeakerBench是一个专注于说话人中心视听推理的基准测试，包含3212个多项选择题，评估多模态大语言模型在真实世界视频中对谁说话、说什么、何时说话等细粒度推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有视频基准测试很少评估对人类语音的细粒度推理，许多任务要么可以通过视觉解决，要么只粗略评估语音，无法深入了解模型是否能对齐说话人、说话内容和时间点。

Method: 引入AV-SpeakerBench基准，具有三个特点：(1) 以说话人为核心推理单元而非场景；(2) 融合基础的问题设计，将视听依赖嵌入问题语义；(3) 专家标注确保时间精度和跨模态有效性。

Result: Gemini系列模型表现最佳，Gemini 2.5 Pro获得最好结果。开源模型中，Qwen3-Omni-30B接近Gemini 2.0 Flash，但仍远落后于Gemini 2.5 Pro，主要原因是视听融合能力较弱而非视觉感知。

Conclusion: AV-SpeakerBench为未来多模态系统的细粒度视听推理建立了严谨基础，揭示了当前模型在视听融合方面的局限性。
...
中文标题: 看、听、理解：多模态大语言模型中视听人类语音理解的基准测试

中文摘要: 多模态大语言模型（MLLMs）被期望能够联合解释视觉、音频和语言，然而现有的视频基准测试很少评估对人类语音的细粒度推理。许多任务仍然可以通过视觉解决或仅粗略评估语音，对模型是否能对齐谁说话、说什么以及何时发生提供了有限的洞察。我们引入了AV-SpeakerBench，这是一个包含3212个多项选择题的精选基准测试，专注于真实世界视频中的说话人中心视听推理。它具有以下特点：(1) 以说话人为核心推理单元而非场景的说话人中心化表述；(2) 融合基础的问题设计，将视听依赖嵌入问题语义；(3) 专家标注确保时间精度和跨模态有效性。综合评估显示，Gemini系列模型始终优于开源系统，其中Gemini 2.5 Pro取得了最佳结果。在开源模型中，Qwen3-Omni-30B接近Gemini 2.0 Flash，但仍远落后于Gemini 2.5 Pro，主要原因是较弱的视听融合能力而非视觉感知。我们相信AV-SpeakerBench为未来多模态系统中细粒度视听推理的推进建立了严谨基础。
...
TL;DR: SPF是一种新的流匹配方法，通过时空金字塔结构实现高效的气候模拟，在ClimateBench上超越基线模型，并利用ClimateSuite数据集进行扩展，展示了良好的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的生成模型依赖天气尺度自回归进行气候模拟，这种方法对于长期气候模拟速度慢，且在非平稳强迫下难以实现稳定展开。需要一种能够高效、并行模拟多时间尺度气候的方法。

Method: 提出时空金字塔流匹配（SPF），将生成轨迹划分为时空金字塔结构，逐步增加空间分辨率以减少计算量，每个阶段与特定时间尺度耦合，支持在金字塔任意时间层级直接采样。每个阶段都基于规定的物理强迫（如温室气体或气溶胶）进行条件化。

Result: 在ClimateBench上，SPF在年和月时间尺度上优于强大的流匹配基线和预训练模型，同时提供快速采样，特别是在较粗的时间层级。扩展的SPF模型在跨气候模型的保留场景中表现出良好的泛化能力。

Conclusion: SPF和ClimateSuite为跨时间尺度和现实未来场景的准确、高效、概率性气候模拟奠定了基础。该方法能够处理非平稳强迫，支持多时间尺度并行模拟。
...
中文摘要: 生成模型有潜力改变我们模拟地球变化气候的方式。以前的生成方法依赖天气尺度的自回归进行气候模拟，但这对于长期气候模拟本质上很慢，并且尚未证明在非平稳强迫下的稳定展开。本文介绍了时空金字塔流（SPF），这是一种新的流匹配方法类别，在空间和时间尺度上分层建模数据。受级联视频模型的启发，SPF将生成轨迹划分为时空金字塔，逐步增加空间分辨率以减少计算量，并将每个阶段与相关时间尺度耦合，以实现在金字塔中任意时间层级的直接采样。这种设计，加上每个阶段基于规定的物理强迫（如温室气体或气溶胶）进行条件化，使得在多个时间尺度上实现高效、并行的气候模拟成为可能。在ClimateBench上，SPF在年和月时间尺度上优于强大的流匹配基线和预训练模型，同时提供快速采样，特别是在较粗的时间层级。为了扩展SPF，我们策划了ClimateSuite，这是迄今为止最大的地球系统模拟集合，包含超过33,000个模拟年，涵盖十个气候模型，也是第一个包含气候干预模拟的数据集。我们发现扩展的SPF模型在跨气候模型的保留场景中表现出良好的泛化能力。SPF和ClimateSuite共同为跨时间尺度和现实未来场景的准确、高效、概率性气候模拟奠定了基础。数据和代码公开在https://github.com/stanfordmlgroup/spf。

</details>


### [7] [Enhancing Cross Domain SAR Oil Spill Segmentation via Morphological Region Perturbation and Synthetic Label-to-SAR Generation](https://arxiv.org/abs/2512.02290)
*Andre Juarez,Luis Salsavilca,Frida Coaquira,Celso Gonzales*

Main category: cs.CV

TL;DR: 该论文提出了MORP-Synth框架，通过形态学区域扰动和合成标签到SAR生成的两阶段方法，提升SAR油污分割模型从地中海到秘鲁海岸的跨域泛化能力。


<details>
  <summary>Details</summary>
Motivation: SAR油污分割深度学习模型在不同区域间泛化能力差，主要由于海况、后向散射统计特性和油污形态的差异。这一问题在秘鲁海岸尤为严重，因为该地区标记的Sentinel-1数据稀缺。

Method: 提出两阶段合成增强框架MORP-Synth：阶段A应用形态学区域扰动，这是一种基于曲率引导的标签空间方法，生成油污和类似区域的真实几何变化；阶段B使用条件生成INADE模型从编辑后的掩码渲染SAR样纹理。

Result: 构建了包含2112个标记的512×512补丁的秘鲁数据集，评估了七种分割架构。在地中海数据上预训练的模型在秘鲁域上mIoU从67.8%下降到51.8%；MORP-Synth将性能提升高达+6 mIoU，并显著提高少数类IoU（油污+10.8，类似区域+14.6）。

Conclusion: MORP-Synth框架有效解决了SAR油污分割的跨域泛化问题，通过合成数据增强显著提升了模型在数据稀缺区域的性能，为遥感图像分析中的域适应问题提供了有效解决方案。
...
中文摘要: SAR油污分割的深度学习模型由于海况、后向散射统计特性和油污形态的差异，在不同区域间往往难以泛化，这一限制在秘鲁海岸尤为严重，因为该地区标记的Sentinel-1数据仍然稀缺。为了解决这个问题，我们提出了MORP-Synth，一个两阶段的合成增强框架，旨在改善从地中海到秘鲁条件的迁移。阶段A应用形态学区域扰动，这是一种曲率引导的标签空间方法，生成油污和类似区域的真实几何变化。阶段B使用条件生成INADE模型从编辑后的掩码渲染SAR样纹理。我们编译了一个包含2112个标记的512×512补丁的秘鲁数据集，来自40个Sentinel-1场景（2014-2024），并与地中海CleanSeaNet基准协调，评估了七种分割架构。在地中海数据上预训练的模型在秘鲁域上的mIoU从67.8%下降到51.8%；MORP-Synth将性能提升高达+6 mIoU，并显著提高少数类IoU（油污+10.8，类似区域+14.6）。

</details>


### [8] [Video Diffusion Models Excel at Tracking Similar-Looking Objects Without Supervision](https://arxiv.org/abs/2512.02339)
*Chenshuang Zhang,Kang Zhang,Joon Son Chung,In So Kweon,Junmo Kim,Chengzhi Mao*

Main category: cs.CV

TL;DR: 视频扩散模型无需监督即可通过运动特征区分视觉相似物体，其去噪过程的早期阶段分离出运动信息，后期阶段细化外观，从而实现鲁棒跟踪。


<details>
  <summary>Details</summary>
Motivation: 当前计算机视觉中区分视觉相似物体仍是一个关键挑战。虽然有监督跟踪器表现不错，但自监督跟踪器在视觉线索模糊时表现不佳，限制了其在不依赖大量标注数据时的可扩展性和泛化能力。

Method: 利用预训练视频扩散模型内在学习的运动表示进行跟踪，无需任务特定训练。该方法基于发现：扩散模型的去噪过程在早期高噪声阶段分离出运动信息，而在后期阶段细化外观特征。

Result: 该方法在区分视觉相似物体方面显著提升性能，在现有基准测试和新引入的视觉相似物体跟踪测试中，比近期自监督方法提升高达6个百分点。可视化验证表明，这些扩散模型衍生的运动表示能够在挑战性视角变化和形变下鲁棒跟踪甚至完全相同的物体。

Conclusion: 预训练视频扩散模型固有地学习到适合跟踪的运动表示，无需任务特定训练。该方法在视觉相似物体跟踪这一现有方法未充分探索的失败点上取得显著改进，为自监督跟踪提供了新方向。
...
中文摘要: 通过运动区分视觉相似物体仍然是计算机视觉中的一个关键挑战。虽然有监督跟踪器显示出潜力，但当代自监督跟踪器在视觉线索变得模糊时表现不佳，限制了它们在不依赖大量标注数据时的可扩展性和泛化能力。我们发现，预训练的视频扩散模型固有地学习到适合跟踪的运动表示，无需任务特定训练。这种能力源于它们的去噪过程在早期高噪声阶段分离出运动信息，与后期的外观细化阶段不同。基于这一发现，我们的自监督跟踪器在区分视觉相似物体方面显著提升性能，这是现有方法未充分探索的失败点。我们的方法在现有基准测试和新引入的专注于跟踪视觉相似物体的测试中，比近期自监督方法提升高达6个百分点。可视化验证表明，这些扩散模型衍生的运动表示能够在挑战性视角变化和形变下鲁棒跟踪甚至完全相同的物体。

</details>


### [9] [TALO: Pushing 3D Vision Foundation Models Towards Globally Consistent Online Reconstruction](https://arxiv.org/abs/2512.02341)
*Fengyi Zhang,Tianjun Zhang,Kasra Khosoussi,Zheng Zhang,Zi Huang,Yadan Luo*

Main category: cs.CV

TL;DR: TALO提出了一种基于薄板样条的高自由度长期对齐框架，通过全局传播的控制点校正空间不一致性，采用点无关的子图配准设计，提升3D视觉基础模型在在线场景中的全局一致性重建。


<details>
  <summary>Details</summary>
Motivation: 现有的3D视觉基础模型在在线场景（如驾驶场景）中，由于预测是在时间窗口上进行的，难以保持跨时间的一致性。现有方法通过求解全局变换来对齐连续预测，但在假设有效性、局部对齐范围和噪声几何鲁棒性方面存在根本限制。

Method: 提出基于薄板样条的高自由度长期对齐框架，利用全局传播的控制点校正空间不一致性；采用点无关的子图配准设计，对噪声几何预测具有固有鲁棒性；框架完全即插即用，兼容多种3D基础模型和相机配置。

Result: 在多个数据集、骨干模型和相机设置上的广泛实验表明，该方法始终产生更一致的几何结构和更低的轨迹误差，突出了其鲁棒性和通用性。

Conclusion: TALO框架有效解决了3D视觉基础模型在在线场景中的全局一致性问题，通过高自由度对齐和鲁棒配准设计，显著提升了重建质量和轨迹精度。
...
中文摘要: 3D视觉基础模型通过单次前向传播从未校准图像中重建关键3D属性方面表现出强大的泛化能力。然而，在驾驶场景等在线设置中部署时，预测是在时间窗口上进行的，这使得保持跨时间一致性变得非平凡。最近的策略通过求解全局变换来对齐连续预测，但我们的分析揭示了它们在假设有效性、局部对齐范围和噪声几何鲁棒性方面的根本限制。在这项工作中，我们提出了一个基于薄板样条的高自由度和长期对齐框架，利用全局传播的控制点来校正空间变化的不一致性。此外，我们采用了点无关的子图配准设计，对噪声几何预测具有固有鲁棒性。所提出的框架完全即插即用，兼容多种3D基础模型和相机配置（例如，单目或环视）。大量实验表明，我们的方法在多个数据集、骨干模型和相机设置上始终产生更一致的几何结构和更低的轨迹误差，突出了其鲁棒性和通用性。代码已在https://github.com/Xian-Bei/TALO公开提供。

</details>


### [10] [Understanding and Harnessing Sparsity in Unified Multimodal Models](https://arxiv.org/abs/2512.02351)
*Shwai He,Chaorui Deng,Ang Li,Shen Yan*

Main category: cs.CV

TL;DR: 该论文系统分析了统一多模态模型中的稀疏性，发现理解组件具有显著可压缩性，而生成组件对压缩高度敏感。作者提出了基于混合专家的适配方法，通过稀疏激活恢复生成质量，使BAGEL模型仅激活约一半参数即可达到完整模型性能。
...
Method: 首先使用训练无关的剪枝作为探测方法，对统一多模态模型组件进行系统性分析，包括深度剪枝和宽度缩减。然后提出混合专家（MoE）适配方法，将生成模块划分为多个专家并启用稀疏激活来恢复生成质量。通过专家冻结调优验证稀疏激活的有效性，并展示完全可训练的适配能带来额外增益。

Result: 研究发现理解组件在理解和生成任务中都表现出显著可压缩性，在生成任务中更为明显。生成组件对压缩高度敏感，即使在中等压缩比下性能也会急剧下降。提出的MoE适配方法使BAGEL模型在仅激活约一半参数的情况下，性能与完整模型相当。

Conclusion: 统一多模态模型中的理解和生成组件对压缩表现出不同敏感性。通过混合专家适配实现稀疏激活可以有效解决生成组件的压缩敏感性问题，显著提高推理效率而不牺牲性能。
...
中文摘要: 大型多模态模型在理解和生成方面都取得了显著进展。最近的研究致力于开发统一的多模态模型，将异构组件集成到单一框架中以支持这两种能力。然而，这种统一引入了推理效率问题，例如特定任务或样本可能不需要统一模型的全部知识或容量。目前对这些低效性在不同组件中如何表现的系统性理解仍然有限。在本工作中，我们首先使用训练无关的剪枝作为探测方法，对统一多模态模型组件进行系统性分析，考虑深度剪枝和宽度缩减。我们的研究表明，理解组件在理解和生成任务中都表现出显著的可压缩性，在生成任务中更为明显。相比之下，生成组件对压缩高度敏感，即使在中等压缩比下性能也会急剧下降。为了解决这一限制，我们提出了混合专家（MoE）适配方法，灵感来源于不同样本中观察到的动态激活模式。该方法将生成模块划分为多个专家，并启用稀疏激活来恢复生成质量。我们通过专家冻结调优验证了稀疏激活的有效性，并进一步证明完全可训练的适配能带来额外增益。因此，适配后的BAGEL模型在仅激活约一半参数的情况下，性能与完整模型相当。代码发布于\href{https://github.com/Shwai-He/SparseUnifiedModel}{此链接}。
...
TL;DR: 本文对nuScenes自动驾驶数据集进行了全面回顾，揭示了其创建细节、技术特点、对行业的影响，以及基于该数据集的研究进展和挑战。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶发展依赖大规模标注数据集，nuScenes作为行业标杆数据集，其创建细节、技术特点和对后续研究的影响尚未被系统总结。本文旨在填补这一空白，全面审视nuScenes在自动驾驶发展中的关键作用。

Method: 通过深入分析nuScenes数据集的创建过程、技术规格、扩展版本（nuImages和Panoptic nuScenes），追溯其对后续数据集的影响，并系统梳理基于该数据集的各类任务和研究方法。

Result: 揭示了nuScenes作为首个包含雷达数据、跨大陆城市驾驶场景、完全自动驾驶车辆收集的数据集的创新性；展示了其对自动驾驶社区标准制定的深远影响；总结了基于nuScenes的感知、定位、预测、规划等任务的研究进展。

Conclusion: nuScenes数据集在推动自动驾驶研究标准化和多模态融合方面发挥了关键作用，为行业设立了重要基准。尽管取得了显著进展，自动驾驶领域仍面临诸多挑战，nuScenes为这些挑战的研究提供了重要平台。
...
中文摘要: 深度学习彻底改变了自动驾驶车辆（AV）和高级驾驶辅助系统（ADAS）。作为一种数据驱动的方法，深度学习依赖于大量详细标注的驾驶数据。因此，数据集与硬件和算法一起，成为自动驾驶发展的基础构建模块。在这项工作中，我们重新审视了最广泛使用的自动驾驶数据集之一：nuScenes数据集。nuScenes体现了自动驾驶发展的关键趋势，它是第一个包含雷达数据的数据集，展示了来自两大洲的多样化城市驾驶场景，并且是通过在公共道路上运行的完全自动驾驶车辆收集的，同时促进了多模态传感器融合、标准化基准测试以及包括感知、定位与建图、预测和规划在内的广泛任务。我们前所未有地深入探讨了nuScenes及其扩展nuImages和Panoptic nuScenes的创建过程，总结了许多迄今为止在学术出版物中未曾披露的技术细节。此外，我们追溯了nuScenes对后来发布的众多其他数据集的影响，以及它如何定义了社区至今仍在使用的众多标准。最后，我们概述了使用nuScenes数据集的官方和非官方任务，并回顾了主要的方法论发展，从而提供了对自动驾驶文献的全面调查，特别关注nuScenes。
...
TL;DR: 提出了一种弱监督无标定多视角人群计数方法，直接使用人群计数作为监督信号，无需密度图标注，通过自监督排序损失和多尺度先验增强模型能力，利用语义信息改进视角匹配，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多视角人群计数能有效缓解遮挡问题，但现有方法需要昂贵的相机标定和人群标注。虽然已有无标定方法，但仍需图像级人群标注。本文旨在开发一种更实用、标注成本更低的弱监督无标定方法。

Method: 1. 直接使用人群计数作为单视角计数模块的监督信号，而非密度图；2. 引入自监督排序损失，利用多尺度先验增强模型感知能力；3. 利用语义信息改进视角匹配精度；4. 无需相机标定和场景级标注。

Result: 在三个广泛使用的多视角人群计数数据集上，该方法在弱监督设置下优于现有最先进方法，证明了其在实际部署中的优越性。

Conclusion: WSCF-MVCC方法通过弱监督和无标定设计，显著降低了标注和标定成本，同时保持了高精度，更适合实际应用场景。
...
中文摘要: 多视角人群计数可以有效缓解单图像人群计数中常见的遮挡问题。现有的基于深度学习的多视角人群计数方法将不同摄像机视角的图像投影到公共空间以获得地面密度图，这需要大量且昂贵的人群标注和相机标定。因此，提出了无需相机标定和场景级人群标注的无标定方法。然而，现有的无标定方法仍然需要昂贵的图像级人群标注来训练单视角计数模块。因此，在本文中，我们提出了一种弱监督无标定多视角人群计数方法（WSCF-MVCC），直接使用人群计数作为单视角计数模块的监督信号，而不是从人群标注构建的密度图。相反，我们利用了一个利用多尺度先验的自监督排序损失来增强模型的感知能力，而无需额外的标注成本。此外，所提出的模型利用语义信息实现更准确的视角匹配，从而获得更精确的场景级人群计数估计。所提出的方法在三个广泛使用的多视角计数数据集上，在弱监督设置下优于最先进的方法，表明与标定方法相比，它更适合实际部署。代码发布于 https://github.com/zqyq/Weakly-MVCC。
...
TL;DR: VACoT是一个在视觉语言模型推理过程中动态调用图像增强的框架，通过后处理变换提升模型在挑战性和分布外输入上的鲁棒性，特别针对OCR相关对抗场景。


<details>
  <summary>Details</summary>
Motivation: 视觉数据增强在训练鲁棒视觉模型中很重要，但在视觉语言模型中关注有限。VLMs主要依赖大规模真实数据或合成多样性，导致在传统模型能可靠处理的基本感知任务上表现不佳。由于VLMs预训练和微调成本高昂，继续在增强数据上训练效果有限且收益递减。

Method: VACoT框架在模型推理过程中动态调用图像增强，结合去噪等后处理变换。它整合了结构化的通用视觉增强集合，扩展查询图像视图，同时通过高效的智能体强化学习降低训练复杂度和计算开销。提出条件奖励方案，鼓励必要的增强同时惩罚冗长响应，确保感知任务中简洁有效的推理。

Result: 在13个感知基准测试中展示了VACoT的优越性，并引入AdvOCR来突显后处理视觉增强在对抗场景中的泛化优势。显著提升了在挑战性和分布外输入上的鲁棒性，特别是在OCR相关对抗场景中。

Conclusion: VACoT通过推理时动态图像增强，有效提升了视觉语言模型在感知任务中的鲁棒性，特别是在对抗场景下，同时避免了昂贵的重新训练成本。
...
中文摘要: 虽然视觉数据增强仍然是训练鲁棒视觉模型的基石，但在视觉语言模型中却受到有限关注，后者主要依赖大规模真实数据获取或合成多样性。因此，它们可能在传统模型能可靠处理的基本感知任务上遇到困难。鉴于VLMs预训练和微调的巨大成本，继续在增强数据上训练产生的收益有限且递减。本文提出了视觉增强思维链（VACoT），一个在模型推理过程中动态调用图像增强的框架。通过结合去噪等后处理变换，VACoT显著提升了在挑战性和分布外输入上的鲁棒性，特别是在OCR相关对抗场景中。与先前局限于局部裁剪的方法不同，VACoT整合了结构化的通用视觉增强集合，扩展了查询图像视图，同时通过高效的智能体强化学习降低了训练复杂度和计算开销。我们提出了条件奖励方案，鼓励必要的增强同时惩罚冗长响应，确保感知任务中简洁有效的推理。我们在13个感知基准测试中展示了VACoT的优越性，并进一步引入AdvOCR来突显后处理视觉增强在对抗场景中的泛化优势。
...
TL;DR: 本研究比较了ResNet-50和SqueezeNet两种机器学习模型在胸部X光片结核病检测中的性能，发现SqueezeNet在准确率、精确度、召回率和F1分数等各项指标上均优于ResNet-50，展示了机器学习在结核病诊断中的潜力。
...
Method: 研究使用Kaggle数据集中的4,200张胸部X光片，比较了预训练的ResNet-50模型和通用SqueezeNet模型。预处理包括数据分割、增强和调整大小以提高训练效率。使用准确率、精确度、召回率和混淆矩阵等指标评估模型性能。

Result: SqueezeNet模型表现优异：损失32%，准确率89%，精确度98%，召回率80%，F1分数87%。相比之下，ResNet-50模型损失54%，准确率73%，精确度88%，召回率52%，F1分数65%。

Conclusion: 机器学习在结核病检测中具有巨大潜力，特别是SqueezeNet模型表现出色。将此类模型集成到移动设备中可扩展其在缺乏结核病检测资源地区的应用。但仍需开发更快、更小、更准确的模型来支持全球结核病防治工作。
...
中文摘要: 本研究探讨了机器学习模型（特别是预训练的ResNet-50模型和通用SqueezeNet模型）在利用胸部X光图像诊断结核病（TB）中的应用。结核病是一种持续影响人类数千年的传染病，在诊断方面面临挑战，特别是在资源有限的环境中。传统方法（如痰涂片镜检和培养）效率低下，这促使研究者探索深度学习和计算机视觉等先进技术。该研究使用来自Kaggle的数据集，包含4,200张胸部X光片，开发和比较了两种机器学习模型的性能。预处理包括数据分割、增强和调整大小以提高训练效率。使用准确率、精确度、召回率和混淆矩阵等评估指标来评估模型性能。结果显示，SqueezeNet实现了32%的损失、89%的准确率、98%的精确度、80%的召回率和87%的F1分数。相比之下，ResNet-50模型表现出54%的损失、73%的准确率、88%的精确度、52%的召回率和65%的F1分数。本研究强调了机器学习在结核病检测中的潜力以及对早期识别和治疗启动的可能影响。将此类模型集成到移动设备中的可能性扩展了它们在缺乏结核病检测资源地区的实用性。然而，尽管结果令人鼓舞，但继续开发更快、更小、更准确的结核病检测模型对于支持全球抗击结核病的努力仍然至关重要。
...
TL;DR: BEVDilation是一种以激光雷达为中心的3D目标检测多模态融合方法，通过将图像特征作为隐式指导而非直接拼接，缓解了深度估计误差带来的空间错位问题，同时解决了点云稀疏性和语义限制。


<details>
  <summary>Details</summary>
Motivation: 现有LiDAR和相机融合方法存在几何精度差异问题，无差别融合会导致性能下降。需要一种能优先利用LiDAR高精度几何信息，同时有效利用相机语义信息的融合策略。

Method: 1. 提出LiDAR-centric框架，将图像BEV特征作为隐式指导而非直接拼接；2. 设计Sparse Voxel Dilation Block，利用图像先验稠密化前景体素；3. 引入Semantic-Guided BEV Dilation Block，通过图像语义指导增强LiDAR特征扩散和长距离上下文捕获。

Result: 在nuScenes基准测试中达到SOTA性能，计算效率有竞争力。与简单融合方法相比，对深度噪声表现出更强的鲁棒性。

Conclusion: BEVDilation通过以激光雷达为中心的融合策略，有效解决了多模态融合中的几何精度差异问题，在保持计算效率的同时提升了3D目标检测性能，对深度噪声具有更好的鲁棒性。
...
中文摘要: 在鸟瞰图（BEV）表示中融合激光雷达和摄像头信息已被证明在3D目标检测中非常有效。然而，由于这些传感器之间几何精度的根本差异，先前方法中的无差别融合通常会导致性能下降。在本文中，我们提出了BEVDilation，一种新颖的以激光雷达为中心的框架，在融合中优先考虑激光雷达信息。通过将图像BEV特征表述为隐式指导而非简单拼接，我们的策略有效缓解了由图像深度估计误差引起的空间错位。此外，图像指导可以有效帮助以激光雷达为中心的范式解决点云的稀疏性和语义限制。具体来说，我们提出了稀疏体素扩张块，通过图像先验来稠密化前景体素，从而缓解固有的点稀疏性。此外，我们引入了语义引导的BEV扩张块，通过图像语义指导和长距离上下文捕获来增强激光雷达特征扩散处理。在具有挑战性的nuScenes基准测试中，BEVDilation实现了比最先进方法更好的性能，同时保持了有竞争力的计算效率。重要的是，与简单融合相比，我们以激光雷达为中心的策略对深度噪声表现出更强的鲁棒性。源代码可在https://github.com/gwenzhang/BEVDilation获取。
...
TL;DR: 本文提出了一种新颖的无地图轨迹预测算法，通过时间、空间和频率域的多域增强方法，结合专家混合机制和选择性注意力模块，有效处理复杂交互场景中的轨迹预测问题。


<details>
  <summary>Details</summary>
Motivation: 轨迹预测对自动驾驶系统的可靠性和安全性至关重要，但在复杂交互场景中仍具有挑战性。现有方法往往难以从冗余数据中高效提取有价值的场景信息，导致计算效率降低和预测准确性下降，特别是在处理复杂的智能体交互时。

Method: 1. 提出无地图轨迹预测算法，实现跨时间、空间和频率域的轨迹预测；2. 在时间信息处理中使用专家混合(MoE)机制自适应选择关键频率分量；3. 提取这些分量并整合多尺度时间特征；4. 提出选择性注意力模块过滤时间和空间交互中的冗余信息；5. 设计多模态解码器，在补丁级和点级损失监督下获得合理的轨迹结果。

Result: 在Nuscences数据集上的实验证明了该算法的优越性，验证了其在处理复杂交互场景中的有效性。

Conclusion: 本文提出的多域增强无地图轨迹预测算法通过选择性注意力机制和专家混合机制，能够有效处理复杂交互场景中的轨迹预测问题，在减少冗余信息处理的同时提高了预测准确性。
...
中文摘要: 轨迹预测对于自动驾驶系统的可靠性和安全性至关重要，但在复杂的交互场景中仍然是一个具有挑战性的任务。现有方法往往难以从冗余数据中高效提取有价值的场景信息，从而降低了计算效率和预测准确性，特别是在处理复杂的智能体交互时。为了解决这些挑战，我们提出了一种新颖的无地图轨迹预测算法，实现了跨时间、空间和频率域的轨迹预测。具体来说，在时间信息处理中，我们利用专家混合(MoE)机制自适应地选择关键频率分量。同时，我们提取这些分量并整合多尺度时间特征。随后，提出了一个选择性注意力模块来过滤时间和空间交互中的冗余信息。最后，我们设计了一个多模态解码器。在补丁级和点级损失的监督下，我们获得了合理的轨迹结果。在Nuscences数据集上的实验证明了我们算法的优越性，验证了其在处理复杂交互场景中的有效性。
...
TL;DR: U4D是一个不确定性感知的4D LiDAR世界建模框架，通过估计空间不确定性图来定位语义挑战区域，采用"从难到易"的两阶段生成方法，并结合时空混合块确保时间一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的生成框架通常对所有空间区域进行统一处理，忽略了真实场景中不同区域的不确定性差异。这种统一生成方式在复杂或模糊区域会产生伪影，限制了真实性和时间稳定性。

Method: 1. 使用预训练分割模型估计空间不确定性图来定位语义挑战区域；2. 采用"从难到易"的两阶段生成：不确定性区域建模（重建高熵区域）和不确定性条件补全（合成剩余区域）；3. 引入时空混合块自适应融合空间和时间表示。

Result: 大量实验表明，U4D能够生成几何保真且时间一致的LiDAR序列，提升了4D世界建模在自主感知和仿真中的可靠性。

Conclusion: U4D通过不确定性感知的方法解决了现有4D LiDAR世界建模框架在复杂区域生成伪影和时间不一致的问题，为自动驾驶和具身AI提供了更可靠的4D世界建模方案。
...
中文摘要: 从LiDAR序列建模动态3D环境对于构建可靠的4D世界以支持自动驾驶和具身AI至关重要。然而，现有的生成框架通常对所有空间区域进行统一处理，忽略了真实场景中不同区域的不确定性差异。这种统一生成方式在复杂或模糊区域会产生伪影，限制了真实性和时间稳定性。在本工作中，我们提出了U4D，一个用于4D LiDAR世界建模的不确定性感知框架。我们的方法首先从预训练的分割模型估计空间不确定性图，以定位语义挑战区域。然后通过两个连续阶段以"从难到易"的方式进行生成：(1) 不确定性区域建模，以精细的几何保真度重建高熵区域；(2) 不确定性条件补全，在学习的结构先验下合成剩余区域。为了进一步确保时间一致性，U4D引入了时空混合块，在扩散过程中自适应地融合空间和时间表示。大量实验表明，U4D能够生成几何保真且时间一致的LiDAR序列，提升了4D世界建模在自主感知和仿真中的可靠性。

</details>


### [18] [SurfFill: Completion of LiDAR Point Clouds via Gaussian Surfel Splatting](https://arxiv.org/abs/2512.03010)
*Svenja Strobel,Matthias Innmann,Bernhard Egger,Marc Stamminger,Linus Franke*

Main category: cs.CV

TL;DR: SurfFill是一种结合LiDAR和相机优势的点云补全方法，通过高斯面元重建在LiDAR遗漏区域生成高质量点云，特别适用于薄结构和边缘区域。


<details>
  <summary>Details</summary>
Motivation: LiDAR在平坦区域精度高但容易遗漏小几何结构和暗材料细节，而摄影测量能捕捉细节但精度不足。需要结合两者优势实现完整高质量点云重建。

Method: 1. 分析LiDAR光束发散导致的伪影；2. 引入基于密度变化的模糊性启发式识别遗漏区域；3. 约束高斯面元重建在模糊区域进行优化和密集化；4. 提取高斯基元采样补全点云；5. 扩展分治方案处理大规模场景。

Result: 在合成和真实场景的LiDAR点云补全任务中，SurfFill方法优于先前重建方法，能有效补全薄结构和边缘区域，生成更完整高质量的点云。

Conclusion: SurfFill成功结合了LiDAR和相机捕获的优势，通过高斯面元重建在LiDAR遗漏区域生成高质量点云，为大规模点云补全提供了有效解决方案。
...
中文标题: SurfFill：基于高斯面元散射的LiDAR点云补全方法

中文摘要: 激光雷达（LiDAR）捕获的点云通常被认为是主动三维重建的黄金标准。虽然其在平坦区域的精度非常出色，但捕获过程容易遗漏小的几何结构，并且在黑暗、吸光材料上可能失效。另一种方法是拍摄场景的多张照片并应用三维摄影测量来推断这些细节，因为这些区域通常代表特征丰富的区域。然而，摄影测量在特征缺失区域很少能达到LiDAR的精度。因此，我们建议通过引入SurfFill：一种基于高斯面元的LiDAR补全方案，结合LiDAR和相机捕获的优势。我们分析LiDAR捕获过程，并将LiDAR光束发散归因于伪影的主要因素，这些伪影主要出现在薄结构和边缘处。我们利用这一见解，通过评估点云中密度的变化，为补全扫描引入模糊性启发式方法。这使我们能够识别靠近遗漏区域的点，然后我们可以使用这些点来生长额外的点以完成扫描。对于这种点生长，我们约束高斯面元重建[Huang等人，2024]，将优化和密集化集中在这些模糊区域。最后，提取重建中模糊区域的高斯基元，并采样点以完成点云。为了应对大规模重建的挑战，我们通过分治方案扩展了该流程，用于建筑物大小的点云补全。我们在合成和真实场景的LiDAR点云补全任务上进行了评估，发现我们的方法优于先前的重建方法。
...
TL;DR: 本文提出了一种实时无人机摄影测量框架，能够在飞行过程中进行在线3D重建和质量评估，并提供反馈指导无人机路径规划，显著减少覆盖缺口和重复飞行成本。


<details>
  <summary>Details</summary>
Motivation: 传统的离线无人机摄影测量无法满足灾害响应等时间关键应用的需求。现有实时方法主要关注图像处理，缺乏对3D重建质量的在线评估，也无法提供指导性反馈来优化图像采集。

Method: 基于SfM-on-the-fly框架，集成了三个模块：1）在线增量粗网格生成，用于动态扩展稀疏3D点云；2）带有可操作指标的在线网格质量评估；3）用于实时轨迹优化的预测性路径规划。

Result: 综合实验表明，该方法能够实现近实时的现场重建和评估，提供的可操作反馈显著减少了覆盖缺口和重复飞行成本。

Conclusion: 通过集成数据采集、处理、3D重建评估和在线反馈，该框架为从传统被动工作模式向更智能、自适应探索工作流程的转变提供了可行方案。
...
中文摘要: 与传统离线无人机摄影测量相比，实时无人机摄影测量对于灾害响应和主动数字孪生维护等时间关键的地理空间应用至关重要。然而，大多数现有方法专注于实时处理捕获的图像或序列帧，而没有明确评估进行中的3D重建质量或提供指导性反馈来增强目标区域的图像采集。本文提出了实时反馈SfM，这是一种用于实时无人机摄影测量的探索-利用框架，能够在近实时时间内迭代探索未见区域并利用已观测和重建的区域。基于SfM-on-the-fly，所提出的方法集成了三个模块：（1）用于动态扩展稀疏3D点云的在线增量粗网格生成；（2）带有可操作指标的在线网格质量评估；（3）用于实时轨迹优化的预测性路径规划。综合实验表明，我们的方法实现了近实时的现场重建和评估，同时提供了显著减少覆盖缺口和重复飞行成本的可操作反馈。通过集成数据采集、处理、3D重建评估和在线反馈，我们的实时反馈SfM可以成为从传统被动工作模式向更智能、自适应探索工作流程转变的替代方案。代码现已发布于https://github.com/IRIS-LAB-whu/OntheflySfMFeedback。
...
TL;DR: FDTA提出了一种显式特征精炼框架，通过空间适配器、时间适配器和身份适配器三个互补视角增强目标嵌入的判别性，解决了端到端多目标跟踪方法中关联准确率低的问题。
...
Method: FDTA引入了显式特征精炼框架，包含三个互补组件：1）空间适配器（SA）集成深度感知线索以实现空间连续性；2）时间适配器（TA）聚合历史信息以捕捉时间依赖；3）身份适配器（IA）利用质量感知对比学习实现实例级可分离性。

Result: 在多个具有挑战性的MOT基准测试（包括DanceTrack、SportsMOT和BFT）上取得了最先进的性能，证明了所提出的判别性嵌入增强策略的有效性。

Conclusion: FDTA通过从三个互补视角显式精炼目标特征，显著提升了目标嵌入的判别性，从而有效解决了端到端多目标跟踪方法中的关联准确率问题，在多个基准测试上实现了最先进的性能。
...
中文标题: 从检测到关联：学习用于多目标跟踪的判别性目标嵌入

中文摘要: 端到端多目标跟踪方法最近通过将检测和关联统一在单一框架中取得了显著进展。尽管这些方法具有强大的检测性能，但它们的关联准确率相对较低。通过详细分析，我们观察到共享DETR架构生成的目标嵌入显示出过高的目标间相似性，因为它只强调单帧内的类别级区分。相比之下，跟踪需要跨帧的实例级区分以及空间和时间连续性，而当前的端到端方法对目标嵌入的优化不足。为了解决这个问题，我们引入了FDTA（从检测到关联），这是一个显式特征精炼框架，从三个互补视角增强目标判别性。具体来说，我们引入了空间适配器（SA）来集成深度感知线索以实现空间连续性，时间适配器（TA）来聚合历史信息以捕捉时间依赖，以及身份适配器（IA）来利用质量感知对比学习实现实例级可分离性。大量实验表明，FDTA在多个具有挑战性的MOT基准测试（包括DanceTrack、SportsMOT和BFT）上取得了最先进的性能，突显了我们提出的判别性嵌入增强策略的有效性。代码可在https://github.com/Spongebobbbbbbbb/FDTA获取。
...
TL;DR: 该研究复现了RaDelft 4D雷达数据集的结果，并开发了一个无需人工标注的相机引导雷达标注流程，通过将雷达点云投影到相机语义分割图上进行空间聚类，显著提高了雷达标注的准确性，同时研究了不同雾度对雷达标注性能的影响。


<details>
  <summary>Details</summary>
Motivation: 4D雷达在恶劣天气条件下具有强大的环境感知潜力，但雷达语义分割的研究受到开源数据集和标注稀缺的限制。RaDelft数据集虽然具有开创性，但只提供LiDAR标注且没有公开的雷达标注生成代码，限制了研究的可复现性和下游应用。

Method: 开发了一个相机引导的雷达标注流程：将雷达点云投影到基于相机的语义分割图上，然后应用空间聚类技术生成准确的雷达标注，整个过程无需依赖人工标注。

Result: 成功复现了RaDelft组的数值结果，并证明相机引导的标注流程能够生成准确的雷达点云标注，显著提高了雷达标注的准确性。同时量化研究了不同雾度水平对雷达标注性能的影响。

Conclusion: 建立了一个可复现的框架，使研究社区能够训练和评估标注的4D雷达数据，为雷达语义分割研究提供了重要的工具和基准。
...
中文标题: 复现和扩展RaDelft 4D雷达与相机辅助标注

中文摘要: 4D雷达的最新进展凸显了其在恶劣条件下进行稳健环境感知的潜力，但雷达语义分割的进展仍受限于开源数据集和标注的稀缺性。RaDelft数据集虽然具有开创性，但只提供LiDAR标注且没有公开的雷达标注生成代码，限制了可复现性和下游研究。在这项工作中，我们复现了RaDelft组的数值结果，并证明相机引导的雷达标注流程可以在不依赖人工标注的情况下为雷达点云生成准确的标注。通过将雷达点云投影到基于相机的语义分割图上并应用空间聚类，我们创建的标注显著提高了雷达标注的准确性。这些结果建立了一个可复现的框架，使研究社区能够训练和评估标注的4D雷达数据。此外，我们研究并量化了不同雾度水平如何影响雷达标注性能。
...
TL;DR: Nav-R²是一个用于开放词汇目标导航的框架，通过结构化思维链推理和相似性感知记忆，显式建模目标-环境关系和环境-动作规划关系，在未见物体定位上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 开放词汇目标导航中，现有方法存在决策过程不透明、对未见物体定位成功率低的问题，需要更有效的推理机制来提升泛化能力。

Method: 提出Nav-R²框架，包含结构化思维链推理和相似性感知记忆。思维链推理显式建模目标-环境关系和环境-动作规划关系；SA-Mem从时间和语义角度压缩视频帧并融合历史观察，保留最相关的特征。

Result: 在未见物体定位上达到最先进的性能，避免了对已见物体类别的过拟合，同时保持2Hz的实时推理速度，且不引入额外参数。

Conclusion: Nav-R²通过显式关系推理和高效记忆机制，显著提升了开放词汇目标导航的泛化能力和决策透明度，为智能体在未知环境中定位新颖物体提供了有效解决方案。
...
中文摘要: 开放词汇设置下的目标导航要求智能体在未见环境中定位新颖物体，然而现有方法存在决策过程不透明和对未见物体定位成功率低的问题。为解决这些挑战，我们提出了Nav-R²框架，该框架通过结构化思维链推理结合相似性感知记忆，显式建模两种关键关系类型：目标-环境建模和环境-动作规划。我们构建了NavR²-CoT数据集，教导模型感知环境、关注周围上下文中与目标相关的物体，并最终制定未来动作计划。我们的SA-Mem通过压缩视频帧和融合历史观察，从时间和语义角度保留最目标相关和当前观察相关的特征，同时不引入额外参数。与先前方法相比，Nav-R²通过简化高效的流程在定位未见物体方面达到了最先进的性能，避免了对已见物体类别的过拟合，同时保持2Hz的实时推理速度。资源将在github链接中公开提供。
...
TL;DR: 提出GuiDG框架，通过训练多个领域专家模型而非单一通用模型来提升视觉语言模型的领域泛化能力，使用提示调优和跨模态注意力实现高效自适应集成。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型微调面临领域特异性和泛化能力之间的权衡问题，单一通用模型微调会损害对未见领域的泛化性能，需要新的方法来平衡这一矛盾。

Method: 提出两阶段GuiDG框架：1）使用提示调优在划分的源领域上训练多个参数高效专家模型；2）引入跨模态注意力模块，通过自适应集成专家知识来指导视觉编码器的微调。

Result: 在标准领域泛化基准和新构建的ImageNet-DG数据集上的实验表明，GuiDG在保持效率的同时显著优于现有最先进的微调方法。

Conclusion: 通过理论分析和实验验证，证明了训练多个领域专家模型比微调单一通用模型具有更好的泛化能力，GuiDG框架有效解决了视觉语言模型微调中的领域泛化问题。
...
中文摘要: 微调大型预训练视觉语言模型已成为下游适应的主流范式，但面临领域特异性和领域泛化能力之间的关键权衡。当前方法通常在整个数据集上微调通用模型，这可能损害对未见领域的泛化能力。为填补这一空白，我们提供了VLM微调泛化能力的理论理解，揭示了在划分的源领域上训练多个参数高效专家模型比微调通用模型具有更好的泛化能力。受此启发，我们提出了两步骤的领域专家指导DG框架。GuiDG首先使用提示调优获取源领域专家，然后引入跨模态注意力模块通过自适应专家集成来指导视觉编码器的微调。为更好评估少样本DG，我们从ImageNet及其变体构建了ImageNet-DG。在标准DG基准和ImageNet-DG上的大量实验表明，GuiDG在保持效率的同时改进了最先进的微调方法。
...
TL;DR: GUI探索实验室是一个模拟环境引擎，通过监督微调+单轮强化学习+多轮强化学习的渐进式训练方法，显著提升GUI智能体的屏幕导航能力。


<details>
  <summary>Details</summary>
Motivation: 现实GUI环境复杂且专有，难以获取全面环境信息，这限制了GUI智能体导航能力的系统研究和基准测试。

Method: 1. 开发GUI探索实验室模拟环境引擎；2. 采用渐进式训练策略：监督微调→单轮强化学习→多轮强化学习。

Result: 监督微调奠定基础知识基础，单轮强化学习增强泛化能力，多轮强化学习通过探索策略进一步提升导航性能，在静态和交互式基准测试中表现优异。

Conclusion: 强化学习方法在GUI导航中具有优势，渐进式训练策略为构建更强大、更可泛化的GUI智能体提供了实用指导。
...
中文摘要: 随着大型视觉语言模型的快速发展，图形用户界面（GUI）智能体任务的重点从单屏幕任务转向复杂的屏幕导航挑战。然而，现实世界的GUI环境（如PC软件和移动应用程序）通常复杂且专有，难以获取智能体训练和评估所需的全面环境信息。这一限制阻碍了对智能体导航能力的系统研究和基准测试。为了解决这一限制，我们引入了GUI探索实验室，这是一个用于GUI智能体导航研究的模拟环境引擎，能够灵活定义和组合屏幕、图标和导航图，同时提供对环境信息的完全访问，以进行全面的智能体训练和评估。通过大量实验，我们发现监督微调能够有效记忆基础知识，为后续训练奠定关键基础。在此基础上，单轮强化学习进一步增强了在未见场景中的泛化能力。最后，多轮强化学习通过交互式试错鼓励探索策略的发展，从而进一步提高屏幕导航性能。我们在静态和交互式基准测试中验证了我们的方法，证明我们的发现能够有效泛化到现实世界场景。这些发现展示了强化学习方法在GUI导航中的优势，并为构建更强大、更可泛化的GUI智能体提供了实用指导。
...
TL;DR: LightHCG是一种轻量级因果性青光眼检测模型，使用HSIC解缠和因果表示学习，在保持高性能的同时大幅减少模型参数。


<details>
  <summary>Details</summary>
Motivation: 现有AI青光眼检测模型存在可靠性不足、参数过多、可能存在虚假相关性以及难以应用于干预分析和临床模拟等问题，需要开发更轻量、更可靠的因果性检测方法。

Method: 采用卷积VAE架构，结合HSIC（希尔伯特-施密特独立性准则）进行潜在空间解缠，并使用图自编码器进行无监督因果表示学习，构建轻量级因果性青光眼检测模型。

Result: LightHCG在青光眼分类任务中表现出更高性能，同时模型权重比现有先进视觉模型（如InceptionV3、MobileNetV2、VGG16）减少93~99%，并增强了AI驱动干预分析的可能性。

Conclusion: LightHCG成功实现了轻量级、高性能的因果性青光眼检测，为AI在医疗诊断中的可靠应用和干预分析提供了新思路。
...
中文摘要: 作为一种典型的视神经退行性疾病，青光眼因其不可逆性和对人类视野的严重影响而威胁着数百万人。青光眼主要表现为视力模糊、视野变暗或周边视野丧失，通常是由于眼压升高或视网膜内新生血管形成导致的视神经损伤所致。传统上，大多数青光眼相关研究和临床诊断侧重于通过视野测试、视盘检查和眼压计测量等患者数据来检测这些视神经损伤。近年来，随着计算机视觉AI模型的进步，如VGG16或视觉变换器（ViT），基于视网膜眼底图像或OCT的AI自动化青光眼检测和视杯分割在辅助传统诊断方面表现出显著性能。然而，当前AI驱动的青光眼检测方法在可靠性、参数使用过多、检测中可能存在虚假相关性以及在干预分析或临床模拟应用方面的局限性仍有很大改进空间。因此，本研究引入了一种新颖的因果表示驱动的青光眼检测模型：LightHCG，这是一种极其轻量级的基于卷积VAE的潜在青光眼表示模型，能够考虑视神经区域内青光眼相关物理因素之间的真实因果关系。通过使用基于HSIC的潜在空间解缠和图自编码器基于无监督因果表示学习，LightHCG不仅在分类青光眼方面表现出更高性能且权重减少93~99%，而且与现有先进视觉模型（如InceptionV3、MobileNetV2或VGG16）相比，还增强了AI驱动干预分析的可能性。
...
TL;DR: 该论文提出了一种动量自蒸馏方法，在有限计算资源下提升医学视觉语言预训练的效率与性能，通过结合动量机制和梯度累积来扩大有效批量，同时利用自蒸馏增强知识提取。


<details>
  <summary>Details</summary>
Motivation: 医学领域标注数据获取困难，需要鲁棒的视觉语言模型。现有对比学习方法需要大批量训练，计算资源要求高，限制了在资源有限环境中的应用。同时，医疗数据稀缺，需要更有效地从有限数据和模型中提取知识。

Method: 提出动量自蒸馏方法：1）利用动量自蒸馏增强多模态学习，通过教师-学生架构进行知识蒸馏；2）将动量机制与梯度累积相结合，在不增加实际内存消耗的情况下扩大有效批量大小，提高训练效率。

Result: 在零样本分类中达到与SOTA方法相当的性能；在少样本适应方面实现超过90%的AUC-ROC；检索任务性能提升2-3%；仅使用单GPU即可实现高训练效率，保持合理训练时间。

Conclusion: 该方法成功解决了医学视觉语言预训练中的计算资源限制问题，通过动量自蒸馏和梯度累积技术，在有限资源下实现了高性能的多模态学习，为资源受限环境中的医学AI应用提供了可行方案。
...
中文摘要: 在医疗保健领域，获取详细标注具有挑战性，这凸显了鲁棒视觉语言模型（VLMs）的重要性。预训练的VLMs可以在小数据集上进行微调或进行零样本推理，实现与任务特定模型相当的性能。对比学习（CL）是训练VLMs的关键范式，但本质上需要大批量进行有效学习，这使得计算需求大，通常仅限于资源充足的机构。此外，在医疗数据有限的情况下，在训练过程中优先从数据和模型中提取知识以提高性能至关重要。因此，我们专注于利用动量方法结合蒸馏来同时解决计算效率和知识利用问题。我们的贡献可总结如下：（1）利用动量自蒸馏增强多模态学习，（2）将动量机制与梯度累积相结合，在不增加资源消耗的情况下扩大有效批量大小。我们的方法在零样本分类中达到了与最先进（SOTA）方法相当的性能，同时在少样本适应方面提供了显著提升，实现了超过90%的AUC-ROC，并将检索任务提高了2-3%。重要的是，我们的方法在单GPU上实现了高训练效率，同时保持了合理的训练时间。我们的方法旨在通过减少资源需求同时提高SOTA方法的性能来推进高效多模态学习。我们的方法实现可在https://github.com/phphuc612/MSD获取。
...
<details>
  <summary>Details</summary>
Motivation: 当前大模型适应新任务面临数据有限和计算资源紧张的问题。元学习方法需要额外的元训练阶段且成本高、不稳定。同时，已有大量任务特定的预训练模型，但如何以最小额外训练将它们迁移到新任务的研究相对不足。

Method: BOLT框架包含离线阶段和在线阶段。离线阶段从多个任务向量中收集主导奇异方向并按层正交化，形成可重用的基。在线阶段冻结这些基，仅训练每层的小型对角系数集，产生参数可控的低秩更新。

Result: 实验表明，BOLT提供了强大的免训练初始化（通过池化源任务系数获得），以及参数高效的微调路径，在性能上优于常见的PEFT基线和代表性元学习初始化方法。

Conclusion: 将适应约束在任务相关的正交子空间为未见任务的迁移提供了有效替代方案，实现了高效的小样本和测试时适应。
...
中文标题: 面向基的低秩迁移：用于小样本和测试时适应

中文摘要: 在数据和计算预算紧张的情况下，将大型预训练模型适应到未见任务仍然具有挑战性。元学习方法显式学习良好的初始化，但它们需要额外的元训练阶段，训练成本高且可能不稳定。同时，任务特定的预训练模型数量不断增长，但如何以最小的额外训练将它们迁移到新任务的研究相对不足。我们提出了BOLT（面向基的低秩迁移），这是一个通过提取正交的、任务相关的谱基并在该子空间内进行适应的框架，而不是通过合并权重来重用现有的微调模型。在离线阶段，BOLT从多个任务向量中收集主导奇异方向并按层正交化以形成可重用的基。在线阶段，我们冻结这些基，仅训练每层的小型对角系数集，产生参数可控的低秩更新。这种设计提供了（i）通过池化源任务系数获得的强大免训练初始化，以及利用共享正交基的轻量级重新缩放步骤；（ii）参数高效的微调路径，在我们的实验中，与常见的PEFT基线以及代表性元学习初始化相比，实现了稳健的性能。我们的结果表明，将适应约束在任务相关的正交子空间为未见任务的迁移提供了有效的替代方案。
...
TL;DR: HouseLayout3D是一个新的真实世界3D布局估计基准，专注于多楼层建筑布局，而MultiFloor3D是一个无需训练的基线方法，利用现有场景理解技术，在多个数据集上超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 当前3D布局估计模型主要在合成数据集上训练，只能处理简单的单房间或单楼层环境，无法处理大型多楼层建筑，且需要将场景分割成单个楼层处理，这移除了对连接多个楼层的结构（如楼梯）进行推理所必需的全局空间上下文。

Method: 提出了MultiFloor3D，一个无需训练的基线方法，利用最近的场景理解方法（如深度估计、语义分割等）来处理多楼层建筑的3D布局估计，无需在特定数据集上进行训练。

Result: MultiFloor3D在HouseLayout3D基准和先前数据集上都优于现有的3D布局估计模型，证明了其在处理多楼层建筑布局方面的有效性。

Conclusion: HouseLayout3D基准和MultiFloor3D基线方法为全建筑尺度布局估计研究提供了重要基础，突出了在这一方向进一步研究的必要性。
...
中文标题: HouseLayout3D：野外3D布局估计的基准和无训练基线方法

中文摘要: 当前的3D布局估计模型主要在包含简单单房间或单楼层环境的合成数据集上训练。因此，它们无法原生处理大型多楼层建筑，需要在处理前将场景分割成单个楼层，这移除了对连接多个楼层的结构（如楼梯）进行推理所必需的全局空间上下文。在这项工作中，我们引入了HouseLayout3D，一个真实世界基准，旨在支持向全建筑尺度布局估计的进展，包括多个楼层和建筑复杂的空间。我们还提出了MultiFloor3D，一个简单的无需训练基线，利用最近的场景理解方法，在我们的基准和先前数据集上都优于现有的3D布局估计模型，突出了在这一方向进一步研究的必要性。数据和代码可在https://houselayout3d.github.io获取。
...
TL;DR: STL是一个自训练框架，通过"先看后想"的结构化推理模板，让模型先提取视觉属性文本，再用于推理，通过自生成结构化理由进行学习，并加入负面理由增强判别能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在准确感知和鲁棒推理方面存在局限，提升推理能力通常依赖高质量人工标注的思维链数据，成本高昂且忽视感知能力。

Method: 提出See-Think-Learn自训练框架，采用"先看后想"结构化推理模板，让模型先提取视觉属性文本再用于推理；通过自生成结构化理由进行训练，并加入负面理由（解释为何某些答案是错误的）增强判别能力。

Result: 在多个领域实验中，STL始终优于仅基于答案或自生成推理的基线方法，定性分析确认其生成的理由质量较高。

Conclusion: STL为增强视觉语言模型的多模态推理能力提供了一个成本效益高的解决方案，能同时提升感知和推理能力。
...
中文标题: 看、想、学：一种自教多模态推理器

中文摘要: 视觉语言模型在整合视觉感知与语言理解方面取得了显著进展。然而，有效的多模态推理既需要准确感知又需要鲁棒推理，任一方面的弱点都会限制VLM的性能。先前增强推理的努力通常依赖于高质量的思维链数据，这些数据通过劳动密集型的人工标注、昂贵的专有模型或忽视感知的自训练方法获得。为解决这些限制，我们提出了一个简单而有效的自训练框架，称为"看-想-学"。其核心是引入结构化推理模板，鼓励模型先看后想，首先提取视觉属性的文本形式，然后使用它们来指导推理。该框架通过让模型在自训练循环中生成并学习自己的结构化理由，共同改进感知和推理。此外，我们用负面理由（即解释为何某些答案选择是错误的）来增强训练数据，以增强模型区分正确和误导性响应的能力。这促进了更具区分性和鲁棒性的学习。跨多个领域的实验表明，STL始终优于仅基于答案或自生成推理的基线方法，而定性分析确认了其理由的高质量。因此，STL为增强VLM的多模态推理能力提供了一个成本效益高的解决方案。
...
TL;DR: 音频-视频联合去噪训练能提升纯视频生成质量，即使只关注视频模态。通过参数高效的AVFullDiT架构，研究发现音频作为特权信号能帮助模型学习视觉事件与声学后果的因果关系，从而正则化视频动态。


<details>
  <summary>Details</summary>
Motivation: 探究音频-视频联合去噪训练是否能在仅关注视频质量的情况下改善视频生成，验证跨模态协同训练是否能开发更强、更具物理基础的世界模型。

Method: 提出参数高效的Audio-Video Full DiT (AVFullDiT)架构，利用预训练的文本到视频(T2V)和文本到音频(T2A)模块进行联合去噪。在相同设置下训练T2AV模型和仅T2V的对照模型进行比较。

Result: 首次系统性地证明音频-视频联合去噪能带来超越同步性的好处。在具有大运动和物体接触运动的挑战性子集上观察到一致的改进，表明音频作为特权信号能帮助模型内化视觉事件与声学后果的因果关系。

Conclusion: 跨模态协同训练是开发更强、更具物理基础世界模型的有前景方法，音频-视频联合去噪能通过正则化视频动态来提升视频生成质量。
...
中文标题: 听觉是否有助于视觉？探究视频生成中的音频-视频联合去噪

中文摘要: 最近的音频-视频生成系统表明，耦合模态不仅有利于音频-视频同步，也有利于视频模态本身。我们提出一个基本问题：即使我们只关心视频质量，音频-视频联合去噪训练是否能改善视频生成？为了研究这个问题，我们引入了一个参数高效的音频-视频完整DiT（AVFullDiT）架构，该架构利用预训练的文本到视频（T2V）和文本到音频（T2A）模块进行联合去噪。我们在相同设置下训练（i）带有AVFullDiT的T2AV模型和（ii）仅T2V的对应模型。我们的结果首次提供了系统性证据，表明音频-视频联合去噪能带来超越同步性的好处。我们在具有大运动和物体接触运动的挑战性子集上观察到一致的改进。我们假设预测音频作为一种特权信号，鼓励模型内化视觉事件与其声学后果之间的因果关系（例如，碰撞×撞击声），从而正则化视频动态。我们的发现表明，跨模态协同训练是开发更强、更具物理基础世界模型的有前景方法。代码和数据集将公开提供。
...
TL;DR: 该论文提出了3DSPMR方法，通过利用已探索区域的关系、视觉和几何线索来增强多模态大语言模型，以解决顺序性具身任务中的空间记忆和推理问题。


<details>
  <summary>Details</summary>
Motivation: 现有室内具身任务研究通常要求智能体主动探索未知环境并进行场景推理以实现特定目标。然而在实际部署中，智能体经常面临顺序性任务，每个新子任务都基于前一个任务的完成，且某些子任务可能不可行（如搜索不存在的物体）。与单任务设置相比，核心挑战在于如何重用先前探索积累的空间知识来支持后续推理和探索。

Method: 提出了3DSPMR（3D空间记忆推理）方法，该方法利用已探索区域的关系、视觉和几何线索来增强多模态大语言模型（MLLMs），用于顺序性具身任务中的推理和探索。这是首个将几何信息明确纳入基于MLLM的空间理解和推理的工作。

Result: 在SEER-Bench基准测试上的广泛实验验证表明，3DSPMR在顺序性具身问答（EQA）和具身多模态导航（EMN）任务上都取得了显著的性能提升。

Conclusion: 3DSPMR通过整合关系、视觉和几何线索，有效增强了MLLM在顺序性具身任务中的空间理解和推理能力，为解决实际部署中的顺序性任务挑战提供了有效解决方案。
...
中文摘要: 现有的室内具身任务研究通常要求智能体主动探索未知环境并进行场景推理以实现特定目标。然而在实际部署中，智能体经常面临顺序性任务，每个新子任务都基于前一个任务的完成，且某些子任务可能不可行，例如搜索不存在的物体。与单任务设置相比，核心挑战在于如何重用先前探索积累的空间知识来支持后续推理和探索。在这项工作中，我们研究了这一尚未充分探索但具有实际意义的具身AI挑战。为了评估这一挑战，我们引入了SEER-Bench，一个新的顺序性具身探索与推理基准，涵盖两个经典具身任务：具身问答（EQA）和具身多模态导航（EMN）。基于SEER-Bench，我们提出了3DSPMR，一种3D空间记忆推理方法，利用已探索区域的关系、视觉和几何线索来增强多模态大语言模型（MLLMs），用于顺序性具身任务中的推理和探索。据我们所知，这是首个将几何信息明确纳入基于MLLM的空间理解和推理的工作。广泛的实验验证表明，3DSPMR在顺序性EQA和EMN任务上都取得了显著的性能提升。
...
TL;DR: TGDD提出了一种轨迹引导的数据集蒸馏方法，通过动态对齐模型训练轨迹中的特征分布来提升合成数据的表达能力，同时引入分布约束正则化减少类别重叠，在保持效率的同时显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有的分布匹配方法在数据集蒸馏中效率高但存在局限性：它们忽略了训练过程中特征表示的演化，导致合成数据的表达能力受限，从而影响下游任务性能。需要一种能够捕捉训练动态的方法来提升合成数据的质量和代表性。

Method: TGDD将分布匹配重新定义为沿模型训练轨迹的动态对齐过程。在每个训练阶段，通过对齐合成数据集和原始数据集的特征分布来捕捉演化语义。同时引入分布约束正则化来减少类别重叠，确保合成数据既保持语义多样性又具有代表性。

Result: 在十个数据集上的实验表明，TGDD实现了最先进的性能，特别是在高分辨率基准测试上获得了5.0%的准确率提升。该方法在性能和效率之间取得了良好平衡，且无需额外的优化开销。

Conclusion: TGDD通过轨迹引导的动态分布对齐方法，有效解决了现有分布匹配方法忽略训练动态的问题，显著提升了数据集蒸馏的性能，为高效数据集压缩提供了新的解决方案。
...
中文摘要: 数据集蒸馏将大型数据集压缩为紧凑的合成数据集，以降低存储和计算成本。在各种方法中，基于分布匹配的方法因其高效率而受到关注。然而，它们往往忽略了训练过程中特征表示的演化，这限制了合成数据的表达能力并削弱了下游性能。为了解决这个问题，我们提出了轨迹引导的数据集蒸馏方法，将分布匹配重新定义为沿模型训练轨迹的动态对齐过程。在每个训练阶段，TGDD通过对齐合成数据集和原始数据集的特征分布来捕捉演化语义。同时，它引入了分布约束正则化来减少类别重叠。这种设计帮助合成数据既保持语义多样性又具有代表性，从而提升了下游任务的性能。无需额外的优化开销，TGDD在性能和效率之间取得了良好的平衡。在十个数据集上的实验表明，TGDD实现了最先进的性能，特别是在高分辨率基准测试上获得了5.0%的准确率提升。
...
TL;DR: WorldPack通过压缩内存技术（轨迹打包+内存检索）解决了视频世界模型中长期空间一致性问题，在短上下文下实现了高质量的长时生成。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在长时生成中存在空间不一致问题，主要原因是长上下文输入计算成本过高，现有方法难以实现时空一致的长期世界建模。

Method: 提出WorldPack模型，采用压缩内存技术：1）轨迹打包提高上下文效率；2）内存检索保持展开一致性并支持空间推理。

Result: 在Minecraft的LoopNav基准测试中，WorldPack在长期一致性方面显著优于现有最先进模型，实现了更好的空间一致性、保真度和生成质量。

Conclusion: 压缩内存技术有效解决了视频世界模型中的长期空间一致性问题，WorldPack在短上下文条件下实现了高质量的长期生成，为世界建模提供了新思路。
...
中文摘要: 视频世界模型因其能够基于过去观察和导航动作生成高保真度的未来视觉观察而受到广泛关注。时间和空间一致的长时期世界建模一直是一个长期存在的问题，即使是最先进的模型也未能解决，这主要是由于长上下文输入所需的计算成本过高。在本文中，我们提出了WorldPack，一种具有高效压缩内存的视频世界模型，它显著提高了长期生成中的空间一致性、保真度和质量，尽管上下文长度要短得多。我们的压缩内存包括轨迹打包和内存检索；轨迹打包实现了高上下文效率，而内存检索则保持了展开过程中的一致性，并帮助需要空间推理的长期生成。我们的性能在LoopNav（一个专门用于评估长期一致性的Minecraft基准测试）上进行了评估，我们验证了WorldPack明显优于强大的最先进模型。
...
TL;DR: G-SHARP是一个基于开源高斯光栅化器的实时手术场景重建框架，在商业兼容性、变形建模和遮挡处理方面优于现有方法，并能在边缘硬件上实时部署。


<details>
  <summary>Details</summary>
Motivation: 现有实时内窥镜重建方法通常依赖于非商业衍生产品，限制了在实际手术环境中的部署。需要一种商业兼容、能处理组织变形和遮挡的实时重建方案。

Method: 基于Apache-2.0许可的GSplat可微分高斯光栅化器原生构建，实现原则性变形建模和稳健遮挡处理，提供Holoscan SDK应用在NVIDIA边缘硬件上部署。

Result: 在EndoNeRF pulling基准测试中实现了最先进的重建质量，具有强速度-精度权衡，适合术中实时使用。

Conclusion: G-SHARP是首个商业兼容的实时手术重建管道，克服了现有方法的部署限制，实现了高质量、实时的手术场景重建，可在实际手术室环境中应用。
...
中文摘要: 我们提出了G-SHARP，这是一个商业兼容的实时手术场景重建框架，专为需要快速准确3D建模可变形组织的最小侵入性手术而设计。虽然最近的高斯泼溅方法已经推进了实时内窥镜重建，但现有实现通常依赖于非商业衍生产品，限制了可部署性。G-SHARP通过成为首个基于GSplat（Apache-2.0）可微分高斯光栅化器原生构建的手术管道来克服这些限制，实现了在EndoNeRF pulling基准测试中的原则性变形建模、稳健遮挡处理和高保真重建。我们的结果表明，在适合术中使用的强速度-精度权衡下，实现了最先进的重建质量。最后，我们提供了一个Holoscan SDK应用程序，可在NVIDIA IGX Orin和Thor边缘硬件上部署G-SHARP，实现实际手术室环境中的实时手术可视化。
...
TL;DR: UCAgents是一个分层多智能体框架，通过结构化证据审计和单向收敛机制，解决医疗诊断中视觉语言模型的推理脱离问题，在提高准确性的同时大幅降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在医疗诊断中存在推理脱离问题，即语言解释与图像证据脱节，损害临床信任。现有多智能体框架虽然能减轻单一模型偏见，但开放式讨论会放大文本噪声和计算成本，且未能有效将推理锚定在视觉证据上。

Method: 提出UCAgents分层多智能体框架，受临床工作流程启发，强制执行单向收敛：禁止立场变更，限制智能体交互为有针对性的证据验证。引入一轮询问讨论揭示视觉-文本错位风险，通过结构化证据审计抑制修辞漂移，增强视觉信号提取。使用信息论形式化视觉模糊性和文本噪声的双重噪声瓶颈。

Result: 在四个医疗VQA基准测试中，UCAgents在PathVQA上达到71.3%的准确率，比最先进技术提高6.0%，同时降低87.7%的令牌成本。评估表明框架在揭示更多视觉证据和避免文本干扰之间取得了良好平衡。

Conclusion: UCAgents通过单向收敛机制和结构化证据审计，有效解决了医疗诊断中视觉语言模型的推理脱离问题，在保持诊断可靠性的同时显著提升计算效率，具备现实世界临床部署的潜力。
...
中文摘要: 视觉语言模型在医疗诊断中展现出潜力，但存在推理脱离问题，即语言流畅的解释与可验证的图像证据脱节，损害了临床信任。最近的多智能体框架通过模拟多学科团队辩论来减轻单一模型偏见，但开放式讨论放大了文本噪声和计算成本，同时未能将推理锚定在视觉证据这一医疗决策的基石上。我们提出了UCAgents，一个通过结构化证据审计强制执行单向收敛的分层多智能体框架。受临床工作流程启发，UCAgents禁止立场变更，并将智能体交互限制在有针对性的证据验证上，抑制修辞漂移同时增强视觉信号提取。在UCAgents中，引入了一轮询问讨论来揭示视觉-文本错位的潜在风险。这种设计共同约束了视觉模糊性和文本噪声，我们通过信息论形式化了这种双重噪声瓶颈。在四个医疗VQA基准上的广泛实验表明，UCAgents实现了卓越的准确性（PathVQA上71.3%，比最先进技术提高6.0%），同时降低了87.7%的令牌成本。评估结果进一步证实UCAgents在揭示更多视觉证据和避免混淆性文本干扰之间取得了平衡。这些结果表明UCAgents展现出对现实世界临床部署至关重要的诊断可靠性和计算效率。代码可在https://github.com/fqhank/UCAgents获取。
...
TL;DR: 该论文提出3D-SLIM，一种针对3D场景语言理解的注意力掩码策略，通过几何自适应掩码和指令感知掩码解决因果注意力在3D场景中的局限性，显著提升性能而不增加参数或修改架构。


<details>
  <summary>Details</summary>
Motivation: 现有方法将语言模型的因果注意力机制直接应用于3D场景理解存在两个根本冲突：1）顺序无关的3D对象之间的顺序偏见；2）受限的对象-指令注意力，阻碍了任务特定推理。需要一种更适合3D空间结构的注意力机制。

Method: 提出3D-SLIM（3D空间语言指令掩码），包含两个关键组件：1）几何自适应掩码：基于空间密度而非令牌顺序约束注意力；2）指令感知掩码：使对象令牌能直接访问指令上下文。该方法简单，无需架构修改或额外参数。

Result: 在多个基准测试和LLM基线上进行广泛实验，验证了3D-SLIM的有效性。该方法在各种3D场景语言任务上带来显著的性能提升，强调了解码器设计在3D多模态推理中的关键作用。

Conclusion: 3D-SLIM通过替换因果注意力掩码为适应3D空间结构的自适应掩码，有效解决了现有方法在3D场景理解中的局限性。该方法简单高效，无需额外成本，为3D多模态推理提供了重要的解码器设计见解。
...
中文标题: 掩码至关重要：解锁LLMs在3D场景语言理解中的空间推理能力

中文摘要: 近年来，3D场景语言理解领域通过将大型语言模型（LLMs）的通用推理能力迁移到3D多模态上下文中取得了进展。然而，现有方法通常采用语言建模中的标准解码器，这些解码器依赖于因果注意力掩码。这种设计在3D场景理解中引入了两个根本冲突：顺序无关的3D对象之间的顺序偏见，以及受限的对象-指令注意力，阻碍了任务特定推理。为了克服这些限制，我们提出了3D空间语言指令掩码（3D-SLIM），一种有效的掩码策略，用适应3D场景空间结构的自适应注意力掩码替换因果掩码。我们的3D-SLIM包含两个关键组件：基于空间密度而非令牌顺序约束注意力的几何自适应掩码，以及使对象令牌能直接访问指令上下文的指令感知掩码。这种设计使模型能够基于空间关系处理对象，同时受用户任务指导。3D-SLIM简单，无需架构修改，不增加额外参数，但在各种3D场景语言任务上带来了显著的性能提升。在多个基准测试和LLM基线上进行的广泛实验验证了其有效性，并强调了解码器设计在3D多模态推理中的关键作用。
...
TL;DR: 该研究揭示了基于深度学习和高斯混合模型的部分点云配准方法中特征向量对平移和旋转的不变性问题，提出了注意力引导的参考点偏移层来解决这一问题，显著提升了DeepGMR和UGMMReg的性能。


<details>
  <summary>Details</summary>
Motivation: 研究旨在解决基于深度学习和高斯混合模型的部分点云配准方法中存在的理论问题，特别是DeepGMR等方法在部分点云配准时特征向量对平移和旋转缺乏不变性的问题。

Method: 提出了注意力引导的参考点偏移层，该层使用经过充分研究的注意力模块来寻找两个部分点云的共同参考点，而不是重叠区域，从而获得变换不变的特征。

Result: ARPS层显著提升了DeepGMR及其变体UGMMReg的性能，这些扩展模型甚至超越了先前使用注意力块和Transformer提取重叠区域或共同参考点的深度学习方法。

Conclusion: 该研究为基于深度学习和高斯混合模型的配准方法提供了更深入的见解，提出的ARPS层通过寻找共同参考点而非重叠区域，有效解决了特征向量对变换缺乏不变性的问题。
...
中文标题: 基于注意力引导的参考点偏移的高斯混合部分点云配准

中文摘要: 本研究探讨了在部分点云配准中，特征向量对输入点云平移和旋转的不变性影响，特别是在基于深度学习和高斯混合模型的技术领域。我们揭示了使用GMM的深度学习配准方法存在的理论和实践问题，特别关注了该领域开创性研究DeepGMR在部分点云配准中的局限性。我们的主要目标是揭示这些方法背后的原因，并提出一个易于理解的解决方案。为此，我们引入了基于注意力的参考点偏移层，该层能够鲁棒地识别两个部分点云的共同参考点，从而获得变换不变的特征。ARPS层使用经过充分研究的注意力模块来寻找共同参考点，而不是重叠区域。因此，它显著提升了DeepGMR及其近期变体UGMMReg的性能。此外，这些扩展模型甚至超越了先前使用注意力块和Transformer提取重叠区域或共同参考点的深度学习方法。我们相信这些发现为使用深度学习和GMM的配准方法提供了更深入的见解。
...
TL;DR: dots.ocr是一个单视觉语言模型，首次在统一端到端框架中联合学习文档布局解析的三个核心任务，通过可扩展数据引擎合成多语言语料库，在OmniDocBench上达到SOTA，并在新的126语言基准XDocParse上超越最佳竞争对手7.4个百分点。


<details>
  <summary>Details</summary>
Motivation: 文档布局解析是AI访问和解释结构化知识的关键门户，但现有方法依赖碎片化的多阶段流程，存在错误传播问题，且无法利用联合训练的优势。需要统一的端到端解决方案来提升性能。

Method: 提出dots.ocr单视觉语言模型，首次在统一框架中联合学习布局检测、文本识别和关系理解三个核心任务。采用高度可扩展的数据引擎合成大规模多语言语料库，支持多样化的语言、布局和领域。

Result: 在综合基准OmniDocBench上达到最先进性能。在新提出的126语言基准XDocParse上，dots.ocr比最佳竞争对手高出7.4个百分点，展现了卓越的多语言能力。

Conclusion: dots.ocr证明了统一端到端框架在文档布局解析中的优势，通过联合学习和可扩展数据引擎实现了强大的多语言性能，为全球文档智能研究提供了新基准。
...
中文摘要: 文档布局解析是人工智能访问和解释世界海量结构化知识的关键门户。这一过程包括布局检测、文本识别和关系理解，对于赋能下一代视觉语言模型尤为重要。然而，当前方法依赖碎片化的多阶段流程，存在错误传播问题，且无法利用联合训练的协同效应。本文介绍dots.ocr，这是一个单一视觉语言模型，首次在统一的端到端框架中展示了联合学习三个核心任务的优势。这得益于一个高度可扩展的数据引擎，它合成了庞大的多语言语料库，使模型能够在涵盖多样化语言、布局和领域的广泛任务中提供稳健性能。我们统一范式的有效性通过综合基准OmniDocBench上的最先进性能得到验证。此外，为促进全球文档智能研究，我们引入了XDocParse，这是一个涵盖126种语言的具有挑战性的新基准。在这个测试平台上，dots.ocr建立了强大的新基线，以显著的+7.4分优势超越次优竞争对手，证明了其无与伦比的多语言能力。
...
TL;DR: GeoDiT是一个基于扩散模型的视觉语言模型，专门用于地理空间理解。它通过并行细化过程重新构建地理空间生成，解决了自回归模型在地理空间任务中的结构不匹配问题，在图像描述、视觉定位和多目标检测等任务上取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 自回归模型的结构与地理空间理解固有的并行性质存在结构性不匹配，它们强制对场景施加刚性的顺序叙述，从根本上阻碍了结构化、连贯输出的生成。作者挑战这一范式，将地理空间生成重新构建为并行细化过程。

Method: 引入GeoDiT，这是第一个专门为地理空间领域设计的基于扩散的视觉语言模型。该方法采用并行细化过程，实现从粗到细的整体合成，同时解析所有语义元素。

Result: GeoDiT在需要结构化、以对象为中心输出的基准测试中建立了新的最先进水平。在图像描述、视觉定位和多目标检测等任务上取得了显著增益，这些正是自回归模型表现不佳的任务。

Conclusion: 研究表明，将生成过程与数据的内在结构对齐是解锁复杂地理空间分析中卓越性能的关键。GeoDiT验证了基于扩散的并行方法在地理空间理解中的优势。
...
中文标题: GeoDiT：基于扩散的地理空间理解视觉语言模型

中文摘要: 自回归模型在结构上与地理空间理解固有的并行性质存在不匹配，它们强制对场景施加刚性的顺序叙述，从根本上阻碍了结构化、连贯输出的生成。我们通过将地理空间生成重新构建为并行细化过程来挑战这一范式，实现从粗到细的整体合成，同时解析所有语义元素。为此，我们引入了GeoDiT，这是第一个专门为地理空间领域设计的基于扩散的视觉语言模型。大量实验表明，GeoDiT在需要结构化、以对象为中心输出的基准测试中建立了新的最先进水平。它在图像描述、视觉定位和多目标检测等任务上取得了显著增益，这些正是自回归模型表现不佳的任务。我们的工作验证了将生成过程与数据的内在结构对齐是解锁复杂地理空间分析中卓越性能的关键。
...
TL;DR: ViT-SR采用两阶段训练策略：首先通过着色任务进行自监督预训练，学习通用视觉表示；然后针对4倍超分辨率进行微调，通过预测高频残差图像来简化残差学习。


<details>
  <summary>Details</summary>
Motivation: 单图像超分辨率（SISR）在计算机视觉中仍然是一个难题。传统方法可能无法充分学习丰富的视觉表示。本文旨在通过两阶段训练策略提升Vision Transformer在超分辨率任务上的性能。

Method: 1. 自监督预训练阶段：在着色任务上训练Vision Transformer，学习通用视觉表示
2. 微调阶段：将预训练模型调整为4倍超分辨率任务
3. 残差学习设计：预测添加到初始双三次插值的高频残差图像，简化学习过程

Result: 在DIV2K基准数据集上训练和评估，ViT-SR取得了令人印象深刻的SSIM 0.712和PSNR 22.90 dB。这些结果证明了该方法在超分辨率任务上的有效性。

Conclusion: 两阶段训练策略有效提升了Vision Transformer在图像超分辨率任务上的性能。自监督预训练为复杂图像修复任务提供了有价值的表示学习方法。未来可以通过更大的ViT架构或替代预训练任务进一步改进。
...
中文标题: 用于图像修复的两阶段视觉变换器：着色预训练 + 残差上采样

中文摘要: 在计算机视觉中，单图像超分辨率（SISR）仍然是一个难题。我们提出了ViT-SR，这是一种采用两阶段训练策略来提升视觉变换器（ViT）性能的新技术。在我们的方法中，模型通过在着色任务上进行自监督预训练阶段，从数据本身学习丰富、可泛化的视觉表示。然后，预训练模型被调整为4倍超分辨率。通过预测添加到初始双三次插值的高频残差图像，这种设计简化了残差学习。在DIV2K基准数据集上训练和评估的ViT-SR取得了令人印象深刻的SSIM 0.712和PSNR 22.90 dB。这些结果证明了我们两阶段方法的有效性，并突出了自监督预训练在复杂图像修复任务中的潜力。通过更大的ViT架构或替代预训练任务可能实现进一步改进。
...
TL;DR: SkyMoE是一个基于专家混合架构的视觉语言模型，专门用于遥感多任务解释。它通过自适应路由器将不同任务和粒度级别的遥感图像分配给专门的语言模型专家处理，实现了局部细节和全局上下文的有效平衡。


<details>
  <summary>Details</summary>
Motivation: 当前通用视觉语言模型在遥感任务中表现不佳，现有地理空间VLMs采用统一建模策略，无法有效区分不同任务类型和解释粒度，导致在局部细节感知和全局上下文理解之间难以取得平衡。

Method: 1. 采用专家混合（MoE）架构，包含自适应路由器生成任务和粒度感知的路由指令；2. 引入上下文解耦增强策略，创建局部和全局特征的对比对；3. 构建MGRS-Bench综合基准进行评估。

Result: 在21个公共数据集上的实验表明，SkyMoE在多个遥感解释任务中实现了最先进的性能，验证了其适应性、可扩展性和卓越的多粒度理解能力。

Conclusion: SkyMoE通过专家混合架构成功解决了遥感多任务解释中的粒度平衡问题，为地理空间视觉语言模型的发展提供了新方向，在复杂遥感场景中展现出强大的泛化能力。
...
中文摘要: 大型视觉语言模型（VLMs）的出现显著提高了地理空间解释的效率和灵活性。然而，通用VLMs在遥感（RS）任务中仍然不够理想。现有的地理空间VLMs通常采用统一建模策略，难以区分任务类型和解释粒度，限制了它们在局部细节感知和全局上下文理解之间的平衡能力。本文提出了SkyMoE，一种专为多模态、多任务遥感解释设计的专家混合（MoE）视觉语言模型。SkyMoE采用自适应路由器，生成任务和粒度感知的路由指令，使专门的大型语言模型专家能够处理不同的子任务。为了进一步促进专家解耦和粒度敏感性，我们引入了上下文解耦增强策略，在局部和全局特征之间创建对比对，引导专家进行层级特定的表示学习。我们还构建了MGRS-Bench，一个涵盖多个遥感解释任务和粒度级别的综合基准，用于评估复杂场景下的泛化能力。在21个公共数据集上的广泛实验表明，SkyMoE在各项任务中实现了最先进的性能，验证了其在遥感领域的适应性、可扩展性和卓越的多粒度理解能力。
...
TL;DR: 本论文研究零样本异常检测中的一致异常问题，提出CoDeGraph框架解决该问题，并扩展到3D医学成像和视觉语言模型应用。


<details>
  <summary>Details</summary>
Motivation: 零样本异常检测在工业检测和医学成像中日益重要，但现有基于距离的方法容易受到"一致异常"（重复出现的相似异常）的系统性偏置影响，导致检测失败。需要从理论和算法层面解决这一核心挑战。

Method: 1. 形式化一致异常问题，分析预训练ViT补丁表示的统计几何行为，识别相似性缩放和邻居耗尽现象
2. 提出CoDeGraph框架：多阶段图构建、社区检测、结构化细化来过滤一致异常
3. 扩展到3D医学成像：提出无需训练的体标记化策略实现零样本3D异常检测
4. 连接批处理和文本方法：用CoDeGraph伪掩码监督视觉语言模型

Result: 1. 理论层面：识别了导致一致异常问题的两个关键现象
2. 算法层面：CoDeGraph能有效抑制一致异常影响
3. 应用层面：实现了真正的零样本3D异常检测，无需3D训练样本
4. 方法融合：成功连接了批处理和文本驱动的零样本方法

Conclusion: 本论文为零样本异常检测提供了一套完整的理论理解和实践解决方案，从问题形式化到算法设计，再到3D扩展和方法融合，系统性地解决了一致异常这一核心挑战，推动了零样本异常检测领域的发展。
...
中文摘要: 零样本异常分类与分割（AC/AS）旨在无需任何训练数据的情况下检测异常样本和区域，这种能力在工业检测和医学成像中日益重要。本论文旨在研究零样本AC/AS的核心挑战，并提出基于理论和算法设计的原理性解决方案。

我们首先形式化了一致异常问题，这是一种故障模式，其中重复出现的相似异常会系统性地偏置基于距离的方法。通过分析预训练视觉变换器中补丁表示的统计和几何行为，我们识别出两个关键现象——相似性缩放和邻居耗尽——描述了在高度相似对象的设置中，正常补丁之间的关系如何随一致异常的存在而变化。

然后我们引入CoDeGraph，这是一个基于图的框架，用于过滤一致异常，建立在相似性缩放和邻居耗尽现象之上。通过多阶段图构建、社区检测和结构化细化，CoDeGraph有效地抑制了一致异常的影响。

接下来，我们将此框架扩展到3D医学成像，提出了一种无需训练、计算高效的MRI数据体积标记化策略。这使得真正的零样本3D异常检测流程成为可能，并表明无需任何3D训练样本即可实现体积异常分割。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用固定数量的可学习查询令牌桥接VLM和扩散模型，虽然计算高效但存在任务泛化崩溃问题，无法适应远离预训练任务的新任务，需要更灵活的桥接机制。

Method: 提出噪声查询令牌，通过端到端优化学习VLM和扩散模型之间的分布式表示空间；引入带有线性投影的VAE分支来恢复细粒度图像细节。

Result: 实验结果表明该方法有效缓解了泛化崩溃问题，能够在多样化任务上实现稳定的持续学习。

Conclusion: WeMMU通过噪声查询令牌和VAE分支成功增强了VLM与扩散模型的桥接，解决了现有方法的泛化限制，为多模态模型的持续学习提供了有效解决方案。
...
中文摘要: 多模态大语言模型（MLLMs）的最新进展突显了高效桥接预训练视觉语言模型（VLMs）与扩散模型的挑战。虽然使用固定数量可学习查询令牌的方法提供了计算效率，但它们存在任务泛化崩溃的问题，无法适应远离其预训练任务的新任务。为了克服这一问题，我们提出了噪声查询令牌，通过端到端优化学习VLM和扩散模型之间的分布式表示空间，增强持续学习能力。此外，我们引入了一个带有线性投影的VAE分支来恢复细粒度图像细节。实验结果证实我们的方法缓解了泛化崩溃，并在多样化任务上实现了稳定的持续学习。
...
TL;DR: AVGGT提出了一种训练免费的两步加速方案，通过将早期全局层转换为帧注意力，并对全局注意力进行子采样，在保持精度的同时实现8-10倍的推理加速。


<details>
  <summary>Details</summary>
Motivation: VGGT和π³等模型在多视图3D重建中表现出色，但过度依赖全局自注意力导致计算成本高昂。现有稀疏注意力变体仅提供部分加速，缺乏对全局注意力在多视图推理中作用的系统分析。

Method: 首先深入分析VGGT和π³中全局注意力模块的作用，发现交替全局-帧架构中的角色分工：早期全局层不形成有意义的对应关系，中间层执行跨视图对齐，最后层仅提供微小细化。基于这些发现，提出训练免费的两步加速方案：1) 将早期全局层转换为帧注意力；2) 通过对补丁标记进行K/V子采样（保持对角线并包含均值填充组件）来子采样全局注意力。

Result: 在标准姿态和点图基准测试中，该方法实现了8-10倍的推理时间加速，同时匹配或略微提高了原始模型的精度。即使在先前稀疏注意力基线失败的极端密集多视图设置中，该方法仍保持鲁棒性。

Conclusion: 通过系统分析全局注意力在多视图3D模型中的作用，提出的训练免费加速方案显著降低了计算成本，同时保持了模型性能，为高效多视图推理提供了实用解决方案。
...
中文摘要: 自DUSt3R以来，VGGT和π³等模型展现了强大的多视图3D性能，但它们对全局自注意力的重度依赖导致了高计算成本。现有的稀疏注意力变体提供了部分加速，但缺乏对全局注意力如何贡献于多视图推理的系统分析。在本文中，我们首先深入研究了VGGT和π³中的全局注意力模块，以更好地理解它们的作用。我们的分析揭示了交替全局-帧架构中的明确角色分工：早期全局层不形成有意义的对应关系，中间层执行跨视图对齐，最后层仅提供微小细化。基于这些发现，我们提出了一个训练免费的两步加速方案：(1) 将早期全局层转换为帧注意力，(2) 通过对补丁标记进行K/V子采样（保持对角线并包含均值填充组件）来子采样全局注意力。我们在VGGT和π³上实例化了这一策略，并在标准姿态和点图基准测试中进行了评估。我们的方法在推理时间上实现了8-10倍的加速，同时匹配或略微提高了原始模型的精度，即使在先前稀疏注意力基线失败的极端密集多视图设置中仍保持鲁棒性。
...
TL;DR: OmniPerson是一个统一的身份保持行人生成框架，支持RGB/IR模态的图像/视频生成，具有多参考图像输入、姿态控制、文本描述等功能，通过PersonSyn数据集训练，在行人生成质量和身份一致性方面达到SOTA，并能有效提升ReID模型性能。


<details>
  <summary>Details</summary>
Motivation: 行人重识别（ReID）面临大规模高质量训练数据不足的问题，主要受数据隐私和标注成本限制。现有的行人生成方法在身份一致性和可控性方面存在不足，限制了其在数据集增强中的有效性。

Method: 1) 提出OmniPerson统一生成模型，支持RGB/IR模态的图像/视频生成，支持任意数量参考图像、两种姿态和文本控制；2) 设计Multi-Refer Fuser模块，从多视角参考图像中提取统一身份特征，确保身份一致性；3) 构建PersonSyn数据集及其自动标注流程，将公开的ReID基准数据集转化为多模态监督的丰富标注资源。

Result: 实验结果表明，OmniPerson在行人生成方面达到最先进水平，在视觉保真度和身份一致性方面表现优异。使用生成数据增强现有数据集能持续提升ReID模型的性能。

Conclusion: OmniPerson是第一个统一的身份保持行人生成框架，解决了ReID数据不足的问题，通过多参考融合和丰富控制能力实现了高质量行人生成，为可见光/红外图像/视频ReID任务提供了有效的数据增强解决方案。
...
中文标题: OmniPerson：统一的身份保持行人生成框架

中文摘要: 行人重识别（ReID）由于数据隐私和标注成本的挑战，缺乏大规模高质量的训练数据。虽然先前的方法探索了行人生成用于数据增强，但它们往往无法确保身份一致性，且可控性不足，从而限制了其在数据集增强中的有效性。为此，我们提出了OmniPerson，这是第一个用于可见光/红外图像/视频ReID任务的统一身份保持行人生成流程。我们的贡献有三方面：1) 我们提出了OmniPerson，一个统一的生成模型，提供对所有关键行人属性的整体和细粒度控制。支持RGB/IR模态的图像/视频生成，可输入任意数量的参考图像，支持两种姿态和文本描述。还包括RGB到IR转换和图像超分辨率能力。2) 我们设计了Multi-Refer Fuser，用于在输入任意数量参考图像时实现鲁棒的身份保持，使OmniPerson能够从一组多视角参考图像中提取统一身份，确保生成的行人实现高保真度。3) 我们引入了PersonSyn，这是第一个用于多参考、可控行人生成的大规模数据集，并提出了其自动标注流程，将公开的仅含ID的ReID基准数据集转化为具有密集、多模态监督的丰富标注资源。实验结果表明，OmniPerson在行人生成方面达到最先进水平，在视觉保真度和身份一致性方面表现出色。此外，使用我们生成的数据增强现有数据集能持续提升ReID模型的性能。我们将开源完整的代码库、预训练模型和PersonSyn数据集。
...
TL;DR: 提出了一种新的伴随语音手势视频生成方法，结合扩散模型生成手势运动，然后通过运动图检索和拼接技术生成连贯视频，显著提升了同步性和自然度。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要基于音频特征与运动特征的一对一映射进行运动图检索，但音频与手势之间本质上是多对多的复杂映射关系，这种简化处理无法充分捕捉音频与手势之间的复杂关联，导致生成的手势视频同步性和自然度不足。

Method: 1. 使用扩散模型隐式学习音频与运动的联合分布，从输入音频生成上下文合适的手势运动；2. 从输入音频提取多层次特征（低级和高级特征）丰富扩散模型训练；3. 设计基于运动的检索算法，通过评估全局和局部运动相似性在运动图中寻找最佳路径；4. 对检索到的非连续节点进行无缝拼接，生成连贯视频。

Result: 实验结果表明，该方法在同步准确性和生成手势的自然度方面相比先前方法有显著改进，验证了扩散模型学习音频-运动联合分布的有效性以及多层次特征提取和运动相似性评估策略的优势。

Conclusion: 通过结合扩散模型生成和运动图检索技术，该方法能够更好地处理音频与手势之间的多对多映射关系，生成更同步、更自然的伴随语音手势视频，为这一挑战性问题提供了有效的解决方案。
...
中文摘要: 合成同步且自然的伴随语音手势视频仍然是一个艰巨的挑战。最近的方法利用运动图来挖掘现有视频数据的潜力。为了从图中检索合适的轨迹，先前的方法要么利用输入音频提取的特征与图中运动相关特征之间的距离，要么将输入音频和运动嵌入到共享特征空间中。然而，这些技术可能不是最优的，因为音频和手势之间存在多对多的映射关系，而一对一映射无法充分解决这一问题。为了缓解这一限制，我们提出了一个新颖的框架，首先使用扩散模型生成手势运动。扩散模型隐式地学习音频和运动的联合分布，从而能够从输入音频序列中生成上下文合适的手势。此外，我们的方法从输入音频中提取低级和高级特征，以丰富扩散模型的训练过程。随后，应用精心设计的基于运动的检索算法，通过评估运动的全局和局部相似性来识别图中最合适的路径。由于检索到的路径中并非所有节点都是顺序连续的，最后一步涉及将这些片段无缝拼接以产生连贯的视频输出。实验结果证实了我们提出的方法的有效性，在同步准确性和生成手势的自然度方面相比先前方法有显著改进。
...
TL;DR: 提出了一种用于高斯溅射的内容感知纹理映射方法，通过自适应调整每个高斯基元的纹理分辨率来更高效地表示场景细节，减少基元数量同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 高斯溅射已成为3D重建和实时渲染的首选方法，但精细的外观细节需要大量小高斯基元来表示，当几何和外观具有不同频率特性时会造成资源浪费。受传统纹理映射启发，希望用纹理来尽可能表示详细外观。

Method: 提出了一种新的2D高斯基元外观表示方法，包含每个基元的纹理映射，纹理像素大小受图像采样频率限制并根据输入图像内容自适应调整。在优化过程中自适应上采样或下采样纹理分辨率，并基于纹理分辨率控制基元数量。

Result: 与替代的纹理化高斯基元解决方案相比，该方法在图像质量和总参数数量方面表现更优，能够更高效地表示场景细节。

Conclusion: 通过内容感知纹理映射，成功将纹理表示集成到高斯溅射优化中，实现了更高效的3D场景表示，减少了基元数量同时保持了渲染质量。
...
中文摘要: 高斯溅射已成为3D重建和捕获真实场景实时渲染的首选方法。然而，精细的外观细节需要表示为大量小高斯基元，当几何和外观呈现不同频率特性时会造成资源浪费。受纹理映射长期传统的启发，我们提出尽可能使用纹理来表示详细外观。我们的主要重点是在高斯溅射优化过程中，以原则性方式将适应场景的每个基元纹理映射集成进来。我们通过为2D高斯基元提出一种新的外观表示来实现这一点，其中纹理像素的大小受图像采样频率限制，并根据输入图像内容自适应调整。我们在优化过程中通过自适应上采样或下采样纹理分辨率来实现这一点。此外，我们的方法能够基于纹理分辨率在优化过程中控制基元数量。我们展示了与纹理化高斯基元的替代解决方案相比，我们的方法在图像质量和总参数数量方面表现更优。项目页面：https://repo-sam.inria.fr/nerphys/gs-texturing/
...
TL;DR: RULER-Bench是一个评估视频生成模型基于规则推理能力的新基准，涵盖6个规则类别40个任务，实验显示当前最先进模型在规则一致性指标上仅达到48.87%，表明视频生成模型的推理能力仍有很大提升空间。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成评估基准主要关注视觉感知和理解方面（如视觉美学、指令遵循、时间一致性），但缺乏对模型基于规则推理能力的系统评估。虽然最近研究探索了视频模型作为零样本学习者的能力，但仍缺乏对推理能力的细粒度分解和全面评估方案。

Method: 基于文本到视频和图像到视频两种基本范式，构建了包含6个规则类别40个代表性任务的基准，共622个高质量标注实例。为每个生成视频构建了包含四个指标的检查表，并利用GPT-4o进行评分，与人类判断的一致性达到85%。

Result: 实验结果显示，当前最先进的视频生成模型在规则一致性指标上仅达到48.87%，表明现有模型在基于规则的推理能力方面存在显著不足，有巨大的改进空间。

Conclusion: RULER-Bench填补了视频生成模型推理能力评估的空白，揭示了当前模型在规则推理方面的局限性。该基准的见解将促进推理感知视频生成的进一步发展，推动视频生成模型向视觉基础智能迈进。
...
中文标题: RULER-Bench：面向视觉基础智能的下一代视频生成模型基于规则推理能力探究

中文摘要: 视频生成的最新进展使得能够合成具有强时间一致性和令人印象深刻的视觉质量的视频，这标志着向视觉基础模型迈出的关键一步。为了评估这些视频生成模型，现有基准主要关注与视觉感知和理解相关的因素，如视觉美学、指令遵循和时间一致性。然而，视频生成模型的基于规则推理能力在很大程度上仍未得到探索。尽管最近的研究对视频模型是否可以作为零样本学习者进行了初步探索，但它们仍然缺乏对推理能力的细粒度分解和全面的评估方案。为了填补这一空白，我们引入了RULER-Bench，这是一个旨在从认知规则角度评估视频生成模型推理能力的基准。基于文本到视频和图像到视频两种基本范式，RULER-Bench涵盖了六个规则类别中的40个代表性任务，共622个高质量标注实例。对于每个生成的视频评估，我们构建了一个涵盖四个指标的检查表，并利用GPT-4o为每个问题分配分数，与人类判断的一致性达到85%。广泛的实验表明，最先进的模型在规则一致性指标上仅达到48.87%，突显了下一代视频模型在推理能力方面仍有显著的改进空间。我们期望从RULER-Bench获得的见解将促进推理感知视频生成的进一步发展，推动视频生成模型向视觉基础智能迈进。
...
TL;DR: PoreTrack3D是首个用于动态3D高斯泼溅的毛孔尺度面部轨迹跟踪基准数据集，包含超过44万条面部轨迹，其中5.2万条超过10帧，68条手工标注轨迹覆盖150帧，为细粒度面部表情分析提供新标准。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏针对毛孔尺度、非刚性3D面部轨迹跟踪的基准数据集，限制了细粒度面部表情分析和微妙皮肤表面运动的研究。传统面部标志点无法捕捉毛孔级别的细微运动，需要更高精度的数据支持动态3D高斯泼溅方法的发展。

Method: 创建了包含超过44万条面部轨迹的数据集，其中52,000条超过10帧，68条手工审查轨迹覆盖完整150帧。数据集同时包含传统面部标志点和毛孔尺度关键点轨迹。开发了完整的基准创建流程，包括数据采集、标注、验证和评估框架。

Result: 建立了首个毛孔尺度面部轨迹跟踪基准，系统评估了最先进的动态3D高斯泼溅方法，为该领域设立了首个性能基线。数据集公开可用，为高保真面部运动捕捉和动态3D重建建立了新框架。

Conclusion: PoreTrack3D填补了毛孔尺度面部轨迹跟踪基准的空白，推动了细粒度面部表情分析研究。建立的评估框架为动态3D高斯泼溅方法提供了标准化测试平台，促进了高精度面部运动捕捉技术的发展。
...
中文摘要: 我们介绍了PoreTrack3D，这是首个用于毛孔尺度、非刚性3D面部轨迹跟踪的动态3D高斯泼溅基准。它总共包含超过440,000条面部轨迹，其中超过52,000条超过10帧，包括68条手工审查的覆盖完整150帧的轨迹。据我们所知，PoreTrack3D是首个同时捕获传统面部标志点和毛孔尺度关键点轨迹的基准数据集，通过分析微妙的皮肤表面运动推进了细粒度面部表情的研究。我们系统评估了最先进的动态3D高斯泼溅方法在PoreTrack3D上的表现，建立了该领域的首个性能基线。总体而言，为该基准数据集创建的流程为高保真面部运动捕捉和动态3D重建建立了新框架。我们的数据集公开可用。
...
TL;DR: 本文提出了一种混合架构，将视觉语言模型的细粒度相似度评分与OCR提取的区域相结合，实现空间定位的文档检索，无需额外训练即可在推理时运行。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如ColPali）虽然能实现细粒度的文档检索，但只能返回整个页面而非特定区域，限制了检索增强生成（RAG）的实用性。而OCR系统虽然能提取带坐标的结构化文本，但缺乏语义相关性评估能力。

Method: 提出混合架构，利用ColPali的补丁级相似度分数作为空间相关性过滤器，应用于OCR提取的区域。形式化视觉变换器补丁网格与OCR边界框之间的坐标映射，引入交集度量进行相关性传播，并建立检索精度的理论界限。

Result: 开发了开源实现Snappy，展示了实际应用性，实证评估正在进行中。该方法在推理时运行，无需额外训练。

Conclusion: 该混合方法统一了两种范式，实现了空间定位的文档检索，为检索增强生成提供了更精确的上下文，解决了现有方法的局限性。
...
中文标题: 通过补丁到区域相关性传播实现空间定位的文档检索

中文摘要: 像ColPali这样的视觉语言模型通过将页面嵌入为图像并计算查询标记与视觉补丁之间的细粒度相似度，实现了最先进的文档检索。然而，它们返回的是整个页面而非特定区域，限制了检索增强生成（RAG）的实用性，因为RAG需要精确的上下文。相反，基于OCR的系统提取带有边界框坐标的结构化文本，但缺乏相关性评估的语义基础。我们提出了一种混合架构，统一了这两种范式：使用ColPali的补丁级相似度分数作为OCR提取区域的空间相关性过滤器。我们形式化了视觉变换器补丁网格与OCR边界框之间的坐标映射，引入了相关性传播的交集度量，并建立了检索精度的理论界限。我们的方法在推理时运行，无需额外训练。我们发布了Snappy，这是一个展示实际应用性的开源实现，实证评估正在进行中。
...
TL;DR: PolarGuide-GSDR提出了一种将偏振先验嵌入3D高斯溅射的框架，通过偏振与3DGS的双向耦合机制，解决了反射场景重建中的几何-反射纠缠问题，实现了无需环境地图的高保真反射分离和实时渲染。


<details>
  <summary>Details</summary>
Motivation: 现有偏振感知NeRF方法存在训练慢、渲染效率低、对材质/视角假设依赖强的问题；而3D高斯溅射虽能实时渲染，但在反射场景中面临反射-几何纠缠的挑战，且添加延迟反射模块会引入环境地图依赖。需要一种既能实现高保真反射分离，又保持实时渲染能力的方法。

Method: 提出偏振前向引导范式，建立偏振与3DGS的双向耦合机制：1）利用3DGS的几何先验解决偏振歧义；2）使用精炼的偏振信息引导3DGS的法线和球谐表示。该方法无需环境地图或限制性材质假设，实现了反射分离和全场景重建。

Result: 在公开和自收集数据集上，PolarGuide-GSDR在镜面反射重建、法线估计和新视角合成方面达到了最先进的性能，同时保持了实时渲染能力。这是首个将偏振先验直接嵌入3DGS优化的框架。

Conclusion: PolarGuide-GSDR通过偏振与3DGS的双向耦合，成功解决了反射场景重建中的关键挑战，实现了高保真反射分离和实时渲染，为复杂反射场景建模提供了具有优越可解释性和性能的解决方案。
...
中文标题: PolarGuide-GSDR：基于偏振先验和延迟反射驱动的3D高斯溅射用于真实世界反射场景

中文摘要: 偏振感知神经辐射场（NeRF）能够实现镜面反射场景的新视角合成，但面临训练缓慢、渲染效率低以及对材质/视角假设依赖性强等挑战。然而，3D高斯溅射（3DGS）虽然能够实现实时渲染，但在从反射-几何纠缠中准确重建反射方面存在困难，添加延迟反射模块会引入环境地图依赖。我们通过提出PolarGuide-GSDR来解决这些限制，这是一种偏振前向引导范式，建立了偏振与3DGS之间的双向耦合机制：首先利用3DGS的几何先验解决偏振歧义，然后使用精炼的偏振信息线索来指导3DGS的法线和球谐表示。这一过程实现了无需环境地图或限制性材质假设的高保真反射分离和全场景重建。我们在公开和自收集数据集上证明，PolarGuide-GSDR在镜面反射重建、法线估计和新视角合成方面达到了最先进的性能，同时保持了实时渲染能力。据我们所知，这是首个将偏振先验直接嵌入3DGS优化的框架，为建模复杂反射场景提供了优越的可解释性和实时性能。
...
TL;DR: UAUTrack是一个统一的多模态反无人机视觉跟踪框架，采用单流、单阶段、端到端架构，通过文本先验提示策略指导模型关注无人机，在多个数据集上实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前反无人机跟踪研究存在三个主要问题：1）缺乏跨模态协作的统一框架；2）现有方法多为独立任务的独立模型，忽视跨模态信息共享；3）多模态数据融合技术不成熟，反无人机跟踪仍处于起步阶段。

Method: 提出UAUTrack框架，采用单流、单阶段、端到端架构统一处理多模态数据。核心创新是文本先验提示策略，通过文本指导模型在不同场景中关注无人机目标，实现有效的跨模态信息整合。

Result: 在Anti-UAV和DUT Anti-UAV数据集上达到最先进性能，在Anti-UAV410数据集上实现精度与速度的良好平衡，展示了高精度和实际效率。

Conclusion: UAUTrack成功解决了反无人机跟踪中多模态融合的挑战，提供了一个统一、高效的框架，在不同反无人机场景中表现出色，为后续研究奠定了基础。
...
中文摘要: 反无人机跟踪研究已探索了多种模态，包括RGB、TIR和RGB-T融合。然而，跨模态协作的统一框架仍然缺乏。现有方法主要关注独立任务的独立模型，往往忽视了跨模态信息共享的潜力。此外，反无人机跟踪技术仍处于起步阶段，当前解决方案难以实现有效的多模态数据融合。为应对这些挑战，我们提出了UAUTrack，一个基于单流、单阶段、端到端架构的统一单目标跟踪框架，能有效整合多种模态。UAUTrack引入了一个关键组件：文本先验提示策略，指导模型在不同场景中关注无人机。实验结果表明，UAUTrack在Anti-UAV和DUT Anti-UAV数据集上实现了最先进的性能，并在Anti-UAV410数据集上保持了精度与速度的良好平衡，展示了在不同反无人机场景中的高精度和实际效率。
...
TL;DR: FASA提出了一种两阶段无监督场景分解框架，通过前景感知的槽注意力机制和伪掩码引导，显式分离前景和背景，提升真实世界数据中的物体发现性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于槽注意力的无监督场景分解方法通常不加区分地处理前景和背景区域，导致背景干扰和真实世界数据中实例发现性能不佳。需要一种能够显式分离前景与背景的方法来提升物体发现的精确度。

Method: FASA采用两阶段框架：第一阶段通过双槽竞争机制进行粗粒度场景分解，区分前景和背景区域，使用基于聚类的初始化策略；第二阶段引入掩码槽注意力机制，第一个槽捕获背景，其余槽竞争表示单个前景物体。为解决前景物体过分割问题，使用自监督图像特征构建的补丁亲和力图生成伪掩码来引导前景槽学习。

Result: 在合成和真实世界数据集上的广泛实验表明，FASA在场景分解和物体一致性表示方面持续优于现有最先进方法，验证了显式前景建模和伪掩码引导的有效性。

Conclusion: FASA通过前景感知的槽注意力机制和伪掩码引导，成功实现了更鲁棒的无监督场景分解，显式的前景-背景分离显著提升了物体发现性能，为无监督物体中心表示学习提供了有效解决方案。
...
中文摘要: 近年来，基于物体中心的表示学习进展表明，基于槽注意力的方法可以在无监督情况下有效地将视觉场景分解为物体槽表示。然而，现有方法通常不加区分地处理前景和背景区域，往往导致背景干扰和真实世界数据中实例发现性能不佳。为解决这一限制，我们提出了前景感知槽注意力（FASA），这是一个两阶段框架，显式地将前景与背景分离以实现精确的物体发现。在第一阶段，FASA通过双槽竞争机制进行粗粒度场景分解，区分前景和背景区域。这些槽通过基于聚类的策略初始化，产生显著区域的良好结构化表示。在第二阶段，我们引入掩码槽注意力机制，其中第一个槽捕获背景，而其余槽竞争表示单个前景物体。为进一步解决前景物体的过分割问题，我们整合了从自监督图像特征构建的补丁亲和力图导出的伪掩码引导，以指导前景槽的学习。在合成和真实世界数据集上的广泛实验表明，FASA持续优于最先进的方法，验证了显式前景建模和伪掩码引导对于鲁棒场景分解和物体一致性表示的有效性。代码将公开提供。
...
TL;DR: ClimaOoD通过物理真实的合成数据改进异常分割，提出ClimaDrive框架生成语义连贯、天气多样、物理合理的OoD驾驶数据，构建大规模基准数据集，显著提升异常分割性能。


<details>
  <summary>Details</summary>
Motivation: 异常分割对于自动驾驶安全至关重要，但现有异常数据稀缺且多样性有限，限制了模型在开放世界环境中的泛化能力。现有合成数据生成方法缺乏上下文连贯性和物理真实性，导致合成与真实数据之间存在领域差距。

Method: 提出ClimaDrive框架，统一结构引导的多天气生成与提示驱动的异常修复，通过语义引导的图像到图像合成技术生成语义连贯、天气多样、物理合理的OoD驾驶数据。基于此框架构建ClimaOoD大规模基准数据集，涵盖六种代表性驾驶场景和不同天气条件。

Result: 在四种最先进方法上的广泛实验表明，使用ClimaOoD训练显著提升了异常分割性能。所有方法的AUROC、AP和FPR95均有显著改善，其中RbA在Fishyscapes LAF上的FPR95从3.97降至3.52。这些结果证明了ClimaOoD增强了模型鲁棒性。

Conclusion: ClimaOoD通过物理真实的合成数据有效解决了异常分割中的数据稀缺问题，提供了有价值的训练数据，能够显著提升模型在开放世界异常检测中的泛化能力。
...
中文标题: ClimaOoD：通过物理真实的合成数据改进异常分割

中文摘要: 异常分割旨在检测和定位超出预定义语义类别的未知或分布外（OoD）对象，这是安全自动驾驶的关键能力。然而，异常数据的稀缺性和有限多样性严重限制了模型在开放世界环境中的泛化能力。现有方法通过合成数据生成来缓解这一问题，要么将外部对象复制粘贴到驾驶场景中，要么利用文本到图像扩散模型修复异常区域。虽然这些方法提高了异常多样性，但它们通常缺乏上下文连贯性和物理真实性，导致合成数据与真实数据之间存在领域差距。本文提出了ClimaDrive，一种语义引导的图像到图像框架，用于合成语义连贯、天气多样、物理合理的OoD驾驶数据。ClimaDrive统一了结构引导的多天气生成与提示驱动的异常修复，能够创建视觉真实的训练数据。基于此框架，我们构建了ClimaOoD，这是一个大规模基准数据集，涵盖六种代表性驾驶场景，包括晴朗和恶劣天气条件。在四种最先进方法上的广泛实验表明，使用ClimaOoD训练显著提升了异常分割性能。所有方法的AUROC、AP和FPR95均有显著改善，其中RbA在Fishyscapes LAF上的FPR95从3.97降至3.52。这些结果证明ClimaOoD增强了模型鲁棒性，为开放世界异常检测中的更好泛化提供了有价值的训练数据。
...
TL;DR: GeoBridge是一种多视图基础模型，通过语义锚定机制连接图像和文本，实现鲁棒的跨视图地理定位，超越了传统的卫星中心方法。


<details>
  <summary>Details</summary>
Motivation: 传统跨视图地理定位主要依赖卫星图像，但在高分辨率或最新卫星图像不可用时鲁棒性受限，且未能充分利用多视图（无人机、街景、卫星）和多模态（文本、图像）的互补信息。

Method: 提出GeoBridge基础模型，采用语义锚定机制通过文本描述桥接多视图特征；构建GeoLoc数据集（首个大规模跨模态多视图对齐数据集，包含5万+图像对和文本描述）；支持双向跨视图匹配和语言到图像检索。

Result: GeoLoc预训练显著提高了地理定位准确性；促进了跨领域泛化和跨模态知识迁移；模型在多个任务上表现优异。

Conclusion: GeoBridge通过语义锚定和多模态融合，为跨视图地理定位提供了更鲁棒、灵活的解决方案，超越了传统的卫星中心范式。
...
中文标题: GeoBridge：一种语义锚定的多视图基础模型，连接图像与文本用于地理定位

中文摘要: 跨视图地理定位通过检索与查询图像视觉对应的地理标记参考图像来推断位置。然而，传统的卫星中心范式在高分辨率或最新卫星图像不可用时限制了鲁棒性。它进一步未能充分利用跨视图（例如无人机、卫星和街景）和模态（例如语言和图像）的互补线索。为了解决这些挑战，我们提出了GeoBridge，一种基础模型，可以执行跨视图的双向匹配，并支持语言到图像的检索。超越传统的卫星中心公式，GeoBridge基于一种新颖的语义锚定机制，通过文本描述桥接多视图特征，实现鲁棒、灵活的地理定位。为了支持这项任务，我们构建了GeoLoc，这是第一个大规模、跨模态、多视图对齐的数据集，包含来自36个国家的超过50,000对无人机、街景全景和卫星图像及其文本描述，确保地理和语义对齐。我们在多个任务上进行了广泛评估。实验证实，GeoLoc预训练显著提高了GeoBridge的地理定位准确性，同时促进了跨领域泛化和跨模态知识迁移。数据集、源代码和预训练模型已在https://github.com/MiliLab/GeoBridge发布。
...
TL;DR: VLM-Pruner是一种无需训练的高效视觉语言模型剪枝算法，通过离心式剪枝范式和空间稀疏性缓冲机制，在保持88.9%剪枝率的同时显著提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型剪枝方法主要依赖令牌重要性，忽略了令牌间的冗余性，导致保留大量重复令牌浪费计算资源。虽然已有一些冗余感知方法，但它们往往忽视视觉令牌的空间关系，导致保留令牌过于稀疏，无法充分覆盖目标对象区域。

Method: 提出离心式令牌剪枝范式，实现从近到远的选择，优先保留细粒度对象细节。设计空间稀疏性缓冲准则，延迟选择空间距离较远的令牌。采用并行贪婪策略高效进行令牌选择，并选择性地将丢弃令牌中的显著信息融合到保留令牌中。

Result: 在五种视觉语言模型上的全面比较表明，VLM-Pruner在88.9%的剪枝率下始终优于强基线方法，同时实现了端到端推理加速。

Conclusion: VLM-Pruner通过显式平衡冗余性和空间稀疏性，解决了现有剪枝方法的局限性，为移动设备上的视觉语言模型部署提供了高效解决方案。
...
中文标题: VLM-Pruner：高效VLM离心式令牌剪枝范式中的空间稀疏性缓冲机制

中文摘要: 视觉语言模型在图像理解任务中表现出色，但大量视觉令牌带来了显著的计算成本，阻碍了在移动设备上的部署。许多剪枝方法仅依赖令牌重要性，从而忽略了令牌间的冗余性，保留了大量重复令牌并浪费了计算能力。虽然已经提出了一些冗余感知方法，但它们往往忽视了视觉令牌之间的空间关系。这可能导致保留令牌的选择过于稀疏，无法充分覆盖目标对象的区域。为了解决这些局限性，我们提出了VLM-Pruner，一种无需训练的令牌剪枝算法，显式平衡冗余性和空间稀疏性。我们引入了离心式令牌剪枝范式，实现了从近到远的选择，同时优先保留细粒度对象细节。此外，我们设计了空间稀疏性缓冲准则，延迟选择空间距离较远的令牌。我们进一步采用并行贪婪策略来高效进行令牌选择。为了减轻剪枝带来的信息损失，我们选择性地将丢弃令牌中的显著信息融合到保留令牌中。全面比较表明，VLM-Pruner在88.9%的剪枝率下始终优于五种视觉语言模型的强基线方法，同时实现了端到端推理加速。
...
TL;DR: 本研究提出了一种使用皮下脂肪组织和肌肉掩膜增强的性别分层全身MR图像配准方法，在UK Biobank数据上相比纯强度方法和现有方法显著提高了配准精度。


<details>
  <summary>Details</summary>
Motivation: UK Biobank收集了大量全身MR图像和非成像健康数据，但缺乏稳健的跨受试者图像配准方法。准确的配准能实现全身空间标准化，支持图像衍生参数与非成像数据的相关性分析，这对大规模医学研究至关重要。

Method: 提出性别分层的跨受试者全身MR图像配准方法，使用VIBESegmentator生成的皮下脂肪组织和肌肉掩膜来增强基于强度的图割配准。在4000名受试者子集上评估，与纯强度方法、uniGradICON和MIRTK进行比较。

Result: 提出的方法在男性和女性中分别获得平均Dice分数0.77和0.75，比纯强度方法高6个百分点，比uniGradICON高8-9个百分点，比MIRTK高12-13个百分点。标签错误频率在大多数组织区域降低，年龄相关性图谱噪声更少且解剖对齐更好。

Conclusion: 使用两个组织掩膜的图像配准方法显著改善了UK Biobank全身MR图像的配准质量，为大规模医学研究中的体素级相关性分析提供了更可靠的配准基础。
...
中文标题: 组织掩膜支持的UK Biobank跨受试者全身图像配准——一项方法基准研究

中文摘要: UK Biobank是一项收集全身MR成像和非成像健康数据的大规模研究。对这些全身MR图像进行稳健准确的跨受试者图像配准，将实现其全身空间标准化，以及非成像数据与图像衍生参数（如组织体积或脂肪含量）的区域/体素级相关性分析。我们提出了一种性别分层的跨受试者全身MR图像配准方法，该方法使用来自最先进的VIBESegmentator方法的皮下脂肪组织和肌肉掩膜来增强基于强度的图割配准。所提出的方法在4000名受试者的子集上进行了评估，通过将其与纯强度方法以及两种先前发表的配准方法uniGradICON和MIRTK进行比较。评估包括应用于71个VIBESegmentator掩膜的重叠度量：1）Dice分数，和2）体素级标签错误频率。此外，还研究了年龄与脂肪含量和组织体积之间的体素级相关性，以展示其在医学研究中的实用性。所提出的方法在男性和女性中分别显示出平均Dice分数为0.77/0.75。与纯强度配准相比，两种性别的平均值高出6个百分点（pp），并且在大多数组织区域中标签错误频率降低。这些差异相对于uniGradICON为9pp/8pp，相对于MIRTK为12pp/13pp。使用所提出的方法，年龄相关性图谱噪声更少，显示出更高的解剖对齐度。总之，使用两个组织掩膜的图像配准方法改善了UK Biobank图像的全身配准。
...
TL;DR: GeoViS是一个用于遥感视觉定位的框架，将复杂的遥感图像定位问题重新构建为渐进式的搜索推理过程，通过树状视觉线索序列和多模态感知来精确定位微小目标。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在视觉定位方面取得了显著进展，但将其应用于遥感图像仍面临挑战：目标在千米级场景中极其微小，查询通常涉及复杂的空间关系（如相对位置、空间层次结构或跨远距离对象的上下文依赖）。

Method: GeoViS采用地理空间奖励视觉搜索框架，将遥感视觉定位重新构建为渐进式的搜索推理过程。它通过树状结构的视觉线索序列主动探索全局图像，整合多模态感知、空间推理和奖励引导的探索，迭代地精化地理空间假设。

Result: 在五个遥感定位基准测试上的广泛实验表明，GeoViS实现了精确的地理空间理解，并在关键视觉定位指标上持续超越现有方法，展示了强大的跨领域泛化能力和可解释性。

Conclusion: GeoViS通过将遥感视觉定位重新构建为渐进式搜索推理过程，有效解决了遥感图像中微小目标检测和复杂空间关系理解的挑战，为遥感领域的视觉定位提供了新的解决方案。
...
中文标题: GeoViS：面向遥感视觉定位的地理空间奖励视觉搜索

中文摘要: 多模态大语言模型（MLLMs）的最新进展在视觉定位方面取得了显著进展，实现了文本查询与图像区域之间的细粒度跨模态对齐。然而，将这种能力转移到遥感图像仍然具有挑战性，因为目标通常在千米级场景中极其微小，而查询通常涉及复杂的地理空间关系，如相对位置、空间层次结构或跨远距离对象的上下文依赖。为了解决这些挑战，我们提出了GeoViS，一个地理空间奖励视觉搜索框架，将遥感视觉定位重新构建为渐进式的搜索推理过程。GeoViS不是直接预测目标位置，而是通过树状结构的视觉线索序列主动探索全局图像，整合多模态感知、空间推理和奖励引导的探索，迭代地精化地理空间假设。这种设计使模型能够检测微小的目标，同时保持整体场景感知。在五个遥感定位基准测试上的广泛实验表明，GeoViS实现了精确的地理空间理解，并在关键视觉定位指标上持续超越现有方法，突出了其强大的跨领域泛化能力和可解释性。
...
TL;DR: 提出CAEVL模型，仅使用卫星图像训练无人机地理定位系统，无需配对无人机-卫星数据，通过域偏移模拟实现竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 现有无人机地理定位方法依赖配对无人机-卫星数据集训练，但这类数据获取成本高且稀缺，限制了实际应用。需要开发不依赖配对数据的训练方法。

Method: 采用自监督训练范式，仅使用卫星参考图像训练。设计专门的增强策略模拟卫星与无人机视图之间的视觉域偏移。提出CAEVL模型有效利用这一训练范式。

Result: 在发布的ViLD真实无人机数据集上验证，CAEVL与使用配对数据训练的方法相比达到竞争性性能，展示了强大的泛化能力。

Conclusion: 证明了仅使用卫星图像训练无人机地理定位系统的可行性，消除了对昂贵配对数据的需求，为GNSS拒止环境中的无人机自主定位提供了实用解决方案。
...
中文摘要: 在GNSS拒止环境中的基于图像的定位对无人机自主性至关重要。现有的最先进方法依赖于将无人机图像与地理参考的卫星图像进行匹配；然而，它们通常需要大规模配对的无人机-卫星数据集进行训练。这样的数据获取成本高昂且往往不可用，限制了其适用性。为了应对这一挑战，我们采用了一种训练范式，通过直接从卫星视角参考图像学习，消除了训练过程中对无人机图像的需求。这是通过专门的增强策略实现的，该策略模拟了卫星视图和真实世界无人机视图之间的视觉域偏移。我们介绍了CAEVL，一个专门设计来利用这一范式的高效模型，并在ViLD上进行了验证，ViLD是我们向社区发布的一个新的具有挑战性的真实世界无人机图像数据集。与使用配对数据训练的方法相比，我们的方法实现了有竞争力的性能，证明了其有效性和强大的泛化能力。
...
TL;DR: 提出了一种推理感知的多模态融合框架（RAMF）用于仇恨视频检测，通过局部-全局上下文融合和语义交叉注意力解决多模态融合问题，并引入对抗推理机制增强对微妙仇恨意图的理解。
...
Result: 在两个真实世界仇恨视频数据集上的评估表明，该方法在Macro-F1和仇恨类召回率上分别比最先进方法提高了3%和7%，展现出鲁棒的泛化性能。

Conclusion: RAMF框架通过创新的多模态融合和对抗推理机制，有效提升了仇恨视频检测的性能，特别是在理解微妙仇恨意图方面表现出色。
...
中文摘要: 在线视频中的仇恨言论对数字平台构成日益严重的威胁，尤其是随着视频内容变得越来越多模态和上下文依赖。现有方法往往难以有效融合模态间的复杂语义关系，并且缺乏理解微妙仇恨内容的能力。为了解决这些问题，我们提出了一种创新的推理感知多模态融合（RAMF）框架。针对第一个挑战，我们设计了局部-全局上下文融合（LGCF）来捕获局部显著线索和全局时间结构，并提出语义交叉注意力（SCA）以实现细粒度多模态语义交互。针对第二个挑战，我们引入了对抗推理——一个结构化的三阶段过程，其中视觉语言模型生成（i）客观描述，（ii）仇恨假设推理，和（iii）非仇恨假设推理——提供互补的语义视角，丰富模型对微妙仇恨意图的上下文理解。在两个真实世界仇恨视频数据集上的评估表明，我们的方法实现了鲁棒的泛化性能，在Macro-F1和仇恨类召回率上分别比最先进方法提高了3%和7%。我们将在匿名期结束后发布代码。

</details>


### [61] [LumiX: Structured and Coherent Text-to-Intrinsic Generation](https://arxiv.org/abs/2512.02781)
*Xu Han,Biao Zhang,Xiangjun Tang,Xianzhi Li,Peter Wonka*

Main category: cs.CV

TL;DR: LumiX是一个结构化扩散框架，用于从文本生成内在属性图（如反照率、辐照度、法线、深度和最终颜色），通过查询广播注意力和张量LoRA确保结构一致性和跨图关系建模。


<details>
  <summary>Details</summary>
Motivation: 现有文本到内在属性生成方法通常缺乏结构一致性和物理一致性，难以同时生成多个内在属性图。需要一种能够生成结构化、物理一致的内在场景描述的方法。

Method: 提出结构化扩散框架LumiX，包含两个关键技术：1) 查询广播注意力机制，在自注意力块中跨所有图共享查询以确保结构一致性；2) 张量LoRA，通过张量化适配参数高效地建模跨图关系，支持高效的联合训练。

Result: 实验表明LumiX生成的结果具有连贯性和物理意义，与现有技术相比，对齐度提高了23%，偏好得分更好（0.19 vs -0.41），并且能够在同一框架内执行图像条件的内在分解。

Conclusion: LumiX通过结构化扩散框架成功实现了文本到内在属性的生成，确保了结构一致性和物理一致性，为场景理解和编辑提供了全面的内在描述。
...
中文标题: LumiX：结构化且连贯的文本到内在属性生成

中文摘要: 我们提出了LumiX，一个用于连贯文本到内在属性生成的结构化扩散框架。在文本提示的条件下，LumiX联合生成一组全面的内在属性图（如反照率、辐照度、法线、深度和最终颜色），提供底层场景的结构化和物理一致的描述。这得益于两个关键贡献：1) 查询广播注意力，一种通过在自注意力块中跨所有图共享查询来确保结构一致性的机制。2) 张量LoRA，一种基于张量的适配方法，参数高效地建模跨图关系以实现高效的联合训练。这些设计共同实现了稳定的联合扩散训练和多个内在属性的统一生成。实验表明，LumiX产生连贯且物理有意义的结果，与现有技术相比，对齐度提高了23%，偏好得分更好（0.19 vs -0.41），并且它还可以在同一框架内执行图像条件的内在分解。

</details>


### [62] [TrackNetV5: Residual-Driven Spatio-Temporal Refinement and Motion Direction Decoupling for Fast Object Tracking](https://arxiv.org/abs/2512.02789)
*Tang Haonan,Chen Yanjun,Jiang Lezhi*

Main category: cs.CV

TL;DR: TrackNetV5通过运动方向解耦模块恢复运动极性信息，并使用残差驱动的时空优化头部处理遮挡问题，在TrackNetV2数据集上实现了新的SOTA性能（F1: 0.9859），仅增加3.7%计算量。


<details>
  <summary>Details</summary>
Motivation: TrackNet系列在快速移动小目标跟踪方面已有良好基础，但存在两个主要问题：1）V1-V3版本依赖纯视觉线索，在遮挡情况下表现不佳；2）V4版本虽然引入运动输入，但使用绝对差分方法丢失了运动方向信息（极性），导致方向模糊。需要同时解决遮挡恢复和运动方向精确建模的问题。

Method: TrackNetV5包含两个核心创新：1）运动方向解耦（MDD）模块：将时间动态分解为带符号的极性场，明确编码运动发生和轨迹方向，解决V4的方向模糊问题；2）残差驱动的时空优化（R-STR）头部：基于Transformer架构，采用从粗到细的范式，利用分解的时空上下文估计校正残差，专门处理目标遮挡恢复问题。

Result: 在TrackNetV2数据集上的实验结果显示：TrackNetV5实现了F1分数0.9859和准确率0.9733的新SOTA性能，显著优于所有先前版本。计算效率方面，仅比V4增加3.7%的FLOPs，保持了实时推理能力。

Conclusion: TrackNetV5成功解决了先前版本的两个主要瓶颈：通过MDD模块恢复了运动方向信息，通过R-STR头部有效处理了遮挡问题。该架构在性能显著提升的同时保持了计算效率，为快速移动小目标跟踪提供了更鲁棒的解决方案。
...
中文标题: TrackNetV5：基于残差驱动的时空优化与运动方向解耦的快速目标跟踪

中文摘要: TrackNet系列在体育场景中的快速移动小目标跟踪方面建立了强大的基准。然而，现有版本存在显著限制：V1-V3由于依赖纯视觉线索而在遮挡情况下表现不佳，而TrackNetV4尽管引入了运动输入，但由于其绝对差分方法丢弃了运动极性而存在方向模糊问题。为了克服这些瓶颈，我们提出了TrackNetV5，这是一个集成了两种新颖机制的鲁棒架构。首先，为了恢复丢失的方向先验，我们引入了运动方向解耦（MDD）模块。与V4不同，MDD将时间动态分解为带符号的极性场，明确编码运动发生和轨迹方向。其次，我们提出了残差驱动的时空优化（R-STR）头部。该基于Transformer的模块采用从粗到细的范式，利用分解的时空上下文来估计校正残差，有效恢复被遮挡的目标。在TrackNetV2数据集上的大量实验表明，TrackNetV5实现了新的最先进F1分数0.9859和准确率0.9733，显著优于先前版本。值得注意的是，这一性能飞跃是在与V4相比仅增加3.7%FLOPs的情况下实现的，保持了实时推理能力的同时提供了卓越的跟踪精度。

</details>


### [63] [UnicEdit-10M: A Dataset and Benchmark Breaking the Scale-Quality Barrier via Unified Verification for Reasoning-Enriched Edits](https://arxiv.org/abs/2512.02790)
*Keming Ye,Zhipeng Huang,Canmiao Fu,Qingyang Liu,Jiani Cai,Zheqi Lv,Chen Li,Jing Lyu,Zhou Zhao,Shengyu Zhang*

Main category: cs.CV

TL;DR: 提出了UnicEdit-10M千万级高质量图像编辑数据集和UnicBench基准，通过统一验证流水线解决规模-质量权衡问题，引入新指标评估模型推理能力。


<details>
  <summary>Details</summary>
Motivation: 闭源与开源多模态图像编辑模型性能差距扩大，主要原因是缺乏大规模高质量训练数据和能够诊断多样化编辑行为的综合基准。现有数据构建方法面临规模与质量的权衡困境。

Method: 1) 开发轻量级数据流水线，用端到端模型和统一后验证替代多工具链；2) 训练7B双任务专家模型Qwen-Verify进行失败检测和指令重写；3) 构建UnicEdit-10M千万级数据集；4) 提出UnicBench基准评估空间和知识推理能力；5) 引入非编辑一致性和推理准确率等新指标。

Result: 成功构建了UnicEdit-10M数据集，涵盖多样化编辑任务。提出的UnicBench基准能够全面评估模型性能，新指标实现了细粒度诊断。对主流模型的分析揭示了其在空间和知识推理方面的局限性。

Conclusion: 通过统一验证流水线成功打破了数据构建中的规模-质量壁垒，为开源图像编辑模型的发展提供了高质量训练数据和综合评估框架，指明了未来研究方向。
...
中文标题: UnicEdit-10M：通过统一验证打破规模-质量壁垒的推理增强编辑数据集与基准

中文摘要: 随着GPT-4o、Nano Banana和Seedream 4.0等强大多模态模型在图像编辑领域的快速发展，闭源模型与开源模型之间的性能差距正在扩大，这主要归因于大规模高质量训练数据的稀缺以及能够诊断多样化编辑行为中模型弱点的综合基准的缺乏。现有的数据构建方法面临规模-质量的权衡：人工标注质量高但不可扩展，而自动化流程则受到错误传播和噪声的影响。为解决这一问题，我们引入了一个轻量级数据流水线，用端到端模型和统一的后验证阶段替代了多工具链。为了实现可扩展的质量控制，我们训练了一个7B的双任务专家模型\textbf{Qwen-Verify}，用于高效失败检测和指令重写。该流水线产生了\textbf{UnicEdit-10M}，一个涵盖多样化基础与复杂编辑任务的千万级数据集。我们还提出了\textbf{UnicBench}，一个超越基础编辑的通用基准，明确评估空间和知识驱动的推理能力。为了实现细粒度诊断，我们引入了新颖的指标，包括\textit{非编辑一致性}和\textit{推理准确率}。我们在UnicBench上对主流模型的分析揭示了它们的局限性，并为未来研究提供了明确方向。

</details>


### [64] [IC-World: In-Context Generation for Shared World Modeling](https://arxiv.org/abs/2512.02793)
*Fan Wu,Jiacheng Wei,Ruibo Li,Yi Xu,Junyou Li,Deheng Ye,Guosheng Lin*

Main category: cs.CV

TL;DR: IC-World是一个用于共享世界建模的新框架，通过激活大型视频模型的内在上下文生成能力，从一组输入图像并行生成多个视频，并使用强化学习和奖励模型确保场景几何和物体运动的一致性。


<details>
  <summary>Details</summary>
Motivation: 视频世界模型在合成多样动态视觉环境方面受到关注，但现有方法在共享世界建模方面存在不足——即从同一底层世界在不同相机视角下生成多个视频时，难以保持场景几何和物体运动的一致性。

Method: 提出IC-World框架：1）利用大型视频模型的上下文生成能力并行处理所有输入图像；2）通过强化学习（Group Relative Policy Optimization）微调模型；3）引入两个新的奖励模型来强制场景级几何一致性和对象级运动一致性。

Result: 大量实验表明，IC-World在几何一致性和运动一致性方面显著优于现有最先进方法，是首个系统探索基于视频世界模型的共享世界建模问题的工作。

Conclusion: IC-World成功解决了共享世界建模中的一致性挑战，通过激活大型视频模型的上下文生成能力并结合强化学习优化，实现了高质量的多视角视频生成，为视频世界模型的实际应用提供了新方向。
...
中文摘要: 基于视频的世界模型最近因其合成多样动态视觉环境的能力而受到越来越多的关注。在本文中，我们专注于共享世界建模，其中模型从一组输入图像生成多个视频，每个视频代表不同相机姿态下的相同底层世界。我们提出了IC-World，一种新颖的生成框架，通过激活大型视频模型固有的上下文生成能力，实现对所有输入图像的并行生成。我们进一步通过强化学习、组相对策略优化以及两个新提出的奖励模型对IC-World进行微调，以强制生成的视频集合中场景级几何一致性和对象级运动一致性。大量实验表明，IC-World在几何一致性和运动一致性方面显著优于最先进的方法。据我们所知，这是首个系统探索基于视频世界模型的共享世界建模问题的工作。

</details>


### [65] [PhyCustom: Towards Realistic Physical Customization in Text-to-Image Generation](https://arxiv.org/abs/2512.02794)
*Fan Wu,Cheng Chen,Zhoujie Fu,Jiacheng Wei,Yi Xu,Deheng Ye,Guosheng Lin*

Main category: cs.CV

TL;DR: PhyCustom是一个用于文本到图像生成的物理定制框架，通过两种正则化损失（等距损失和解耦损失）激活扩散模型学习物理概念，解决了现有方法无法准确反映物理属性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本到图像定制方法主要关注风格和形状等具体概念，但在物理概念定制方面存在不足。当前方法在训练过程中缺乏物理知识的显式引入，导致即使文本提示中包含物理相关词汇，生成结果也无法准确反映相应的物理属性。

Method: 提出了PhyCustom微调框架，包含两种新颖的正则化损失：1）等距损失，用于激活扩散模型学习物理概念；2）解耦损失，用于消除独立概念的混合学习。该框架在多样化数据集上进行训练和评估。

Result: 实验结果表明，PhyCustom在物理定制方面在定量和定性上都优于先前的最先进方法和流行方法。基准测试证实了该方法在准确反映物理属性方面的有效性。

Conclusion: PhyCustom成功解决了文本到图像生成中物理概念定制的挑战，通过引入物理知识激活扩散模型，实现了更真实、更准确的物理属性生成，为物理定制任务提供了有效的解决方案。
...
中文摘要: 近年来，基于扩散模型的文本到图像定制方法在理解具体概念以控制生成过程（如风格和形状）方面取得了显著成功。然而，很少有研究深入探讨物理概念的真实而具有挑战性的定制。当前方法的核心限制源于训练过程中缺乏明确引入物理知识。即使输入文本提示中出现与物理相关的词汇，我们的实验始终表明这些方法无法在生成结果中准确反映相应的物理属性。在本文中，我们提出了PhyCustom，一个包含两种新颖正则化损失的微调框架，用于激活扩散模型执行物理定制。具体而言，提出的等距损失旨在激活扩散模型学习物理概念，而解耦损失有助于消除独立概念的混合学习。我们在多样化数据集上进行了实验，基准测试结果表明，在物理定制方面，PhyCustom在定量和定性上都优于先前的最先进方法和流行方法。
...
TL;DR: 研究发现对抗训练（adversarial training）虽然能提高模型鲁棒性，但意外地增强了对抗样本的可迁移性，使得鲁棒模型生成的攻击更容易转移到其他模型，带来新的生态系统风险。


<details>
  <summary>Details</summary>
Motivation: 深度学习在计算机视觉领域取得巨大成功，但对对抗攻击仍然脆弱。对抗训练是主要的防御方法，但其对攻击可迁移性的影响尚未充分探索。本研究旨在探究对抗训练是否会无意中增加对抗样本的可迁移性。

Method: 训练了包含CNN和ViT的36个多样化模型动物园，进行了全面的可迁移性实验，比较对抗训练模型和标准模型生成的对抗样本的转移效果。

Result: 发现一个明显的悖论：对抗训练模型产生的扰动比标准模型产生的扰动转移效果更好，这引入了新的生态系统风险。所有模型、代码和实验脚本都已开源以支持可重复性和进一步研究。

Conclusion: 鲁棒性评估不仅应评估模型对转移攻击的抵抗能力，还应评估其产生可迁移对抗样本的倾向性。对抗训练在提高鲁棒性的同时可能无意中增强了攻击能力。
...
中文摘要: 深度学习在计算机视觉领域取得了巨大成功，但仍然容易受到对抗攻击。对抗训练是旨在提高模型鲁棒性的主要防御方法。然而，其对攻击可迁移性的影响尚未得到充分探索。在这项工作中，我们探讨对抗训练是否会无意中增加对抗样本的可迁移性。为了回答这个问题，我们训练了包含CNN和ViT的36个多样化模型动物园，并进行了全面的可迁移性实验。我们的结果揭示了一个明显的悖论：对抗训练模型产生的扰动比标准模型产生的扰动转移效果更好，这引入了新的生态系统风险。为了支持可重复性和进一步研究，我们发布了所有模型、代码和实验脚本。此外，我们认为鲁棒性评估不仅应评估模型对转移攻击的抵抗能力，还应评估其产生可迁移对抗样本的倾向性。
...
TL;DR: ReVSeg使用强化学习激励视频分割中的推理链，通过显式分解推理过程为语义解释、时序证据选择和空间定位三个步骤，在预训练视觉语言模型上实现可解释的多步推理。


<details>
  <summary>Details</summary>
Motivation: 现有的视频对象分割方法通常将复杂的动态、因果关系和时序交互简化为潜在嵌入的推理，导致推理链不透明且难以追踪。需要一种能够显式分解推理过程的方法，使模型能够进行可解释的多步推理。

Method: ReVSeg采用显式分解视角，将推理过程分解为三个顺序操作：语义解释、时序证据选择和空间定位。使用强化学习优化多步推理链，使模型能够根据结果驱动的信号自我改进决策质量。

Result: 实验结果表明，ReVSeg在标准视频对象分割基准测试中达到了最先进的性能，并产生了可解释的推理轨迹。

Conclusion: 通过显式分解推理过程并使用强化学习优化多步决策链，ReVSeg实现了可解释且高性能的视频对象分割，为复杂推理任务提供了新的解决方案。
...
中文摘要: 以推理为中心的视频对象分割本质上是一项复杂的任务：查询通常涉及动态性、因果关系和时间交互，而非静态外观。然而，现有解决方案通常将这些因素简化为潜在嵌入的推理，使得推理链不透明且本质上难以追踪。因此，我们采用显式分解的视角，引入了ReVSeg，它在预训练视觉语言模型（VLMs）的原生接口中将推理作为顺序决策执行。ReVSeg不是将所有推理折叠为单步预测，而是执行三个显式操作——语义解释、时序证据选择和空间定位——以对齐预训练能力。我们进一步采用强化学习来优化多步推理链，使模型能够从结果驱动的信号中自我改进其决策质量。实验结果表明，ReVSeg在标准视频对象分割基准测试中达到了最先进的性能，并产生了可解释的推理轨迹。项目页面可在https://clementine24.github.io/ReVSeg/ 访问。

</details>


### [68] [Are Detectors Fair to Indian IP-AIGC? A Cross-Generator Study](https://arxiv.org/abs/2512.02850)
*Vishal Dubey,Pallavi Tyagi*

Main category: cs.CV

TL;DR: 该研究首次系统评估了针对印度和南亚面孔的身份保持AIGC检测器的公平性和鲁棒性，发现微调虽然能提升域内性能，但在跨生成器的身份保持AIGC测试中性能显著下降，表明检测器过度拟合训练生成器的特定线索。


<details>
  <summary>Details</summary>
Motivation: 现代图像编辑器能够生成身份保持的AIGC内容，但现有检测器在这种情况下的鲁棒性和公平性尚不明确，特别是对于印度和南亚等代表性不足的人群。研究旨在填补这一空白，评估检测器在跨生成器场景中的泛化能力。

Method: 研究从FairFD和HAV-DF数据集中构建印度面孔的训练集，使用Gemini和ChatGPT等商业生成器创建了两个身份保持AIGC测试集（HIDF-img-ip-genai和HIDF-vid-ip-genai）。评估了AIDE和Effort两种先进检测器在预训练和微调两种模式下的性能，使用AUC、AP、EER和准确率等指标。

Result: 微调在域内测试集上表现良好（如Effort AUC从0.739提升到0.944），但在跨生成器的印度身份保持AIGC测试中性能显著下降（如AIDE AUC从0.923降至0.563）。在非身份保持的HIDF图像上，预训练模型性能保持较高水平，表明问题主要在于身份保持编辑而非一般分布偏移。

Conclusion: 身份保持AIGC-印度场景是一个具有挑战性的实际问题，检测器容易过度拟合训练生成器的特定线索。研究呼吁开发保持表征能力的适应方法和印度感知的基准数据集，以缩小AIGC检测中的泛化差距。
...
中文标题: 检测器对印度身份保持AIGC公平吗？一项跨生成器研究

中文摘要: 现代图像编辑器能够生成身份保持的AIGC内容，即同一人物以新服装、背景或光照出现。现有检测器在这种情况下的鲁棒性和公平性尚不明确，特别是对于代表性不足的人群。我们提出了首个针对印度和南亚面孔的身份保持AIGC检测系统研究，量化了跨生成器泛化和群体内性能。我们从FairFD和HAV-DF中构建了印度重点训练集，并使用商业生成器（Gemini和ChatGPT）通过身份保持提示构建了两个保留测试集（HIDF-img-ip-genai和HIDF-vid-ip-genai）。我们评估了两种先进检测器（AIDE和Effort）在预训练和微调模式下的性能，报告了AUC、AP、EER和准确率。微调在域内获得显著提升（例如，Effort在HAV-DF-test上的AUC从0.739提升到0.944；AIDE的EER从0.484降至0.259），但在印度群体的跨生成器身份保持AIGC测试中性能持续下降（例如，AIDE在HIDF-img-ip-genai上的AUC从0.923降至0.563；Effort从0.740降至0.533），这表明模型过度拟合了训练生成器的线索。在非身份保持的HIDF图像上，预训练模型性能保持较高水平，这表明问题主要在于身份保持编辑而非一般分布偏移。我们的研究确立了身份保持AIGC-印度作为一个具有挑战性且实际相关的场景，并呼吁开发保持表征能力的适应方法和印度感知的基准数据集，以缩小AIGC检测中的泛化差距。

</details>


### [69] [RFOP: Rethinking Fusion and Orthogonal Projection for Face-Voice Association](https://arxiv.org/abs/2512.02860)
*Abdul Hannan,Furqan Malik,Hina Jabbar,Syed Suleman Sadiq,Mubashir Noman*

Main category: cs.CV

TL;DR: 该论文重新思考了多语言环境下人脸-语音关联任务中的融合和正交投影方法，在FAME 2026挑战赛中取得了第三名的成绩。


<details>
  <summary>Details</summary>
Motivation: 多语言环境下的人脸-语音关联任务面临挑战，特别是在处理英语-德语跨语言数据时，需要有效捕捉两种模态间的相关语义信息。

Method: 重新审视融合和正交投影方法，专注于提取两种模态中的相关语义信息，以改善人脸-语音关联性能。

Result: 在英语-德语数据分割上表现良好，在FAME 2026挑战赛中排名第三，实现了33.1%的等错误率(EER)。

Conclusion: 通过重新思考融合和正交投影方法，能够有效处理多语言环境下的人脸-语音关联任务，在跨语言场景中取得了有竞争力的结果。
...
中文标题: RFOP：重新思考人脸-语音关联中的融合和正交投影

中文摘要: 多语言环境中的人脸-语音关联挑战2026旨在研究多语言场景下的人脸-语音关联任务。该挑战引入了英语-德语人脸-语音对用于评估阶段。为此，我们重新审视了人脸-语音关联中的融合和正交投影方法，有效聚焦于两种模态内的相关语义信息。我们的方法在英语-德语数据分割上表现良好，在FAME 2026挑战赛中排名第三，实现了33.1%的等错误率。

</details>


### [70] [MICCAI STSR 2025 Challenge: Semi-Supervised Teeth and Pulp Segmentation and CBCT-IOS Registration](https://arxiv.org/abs/2512.02867)
...
TL;DR: MICCAI STSR 2025挑战赛旨在解决数字牙科中CBCT和IOS数据的标注稀缺问题，通过半监督学习方法进行牙齿和牙髓管分割以及跨模态配准，提供了公开数据集和代码以促进该领域研究。


<details>
  <summary>Details</summary>
Motivation: 锥形束计算机断层扫描（CBCT）和口内扫描（IOS）是数字牙科的关键技术，但标注数据稀缺限制了牙髓管自动分割和跨模态配准解决方案的发展。为了在该领域建立半监督学习的基准，组织了此次挑战赛。

Method: 挑战赛包含两个任务：1）CBCT中牙齿和牙髓管的半监督分割；2）CBCT和IOS的半监督刚性配准。提供了60个标注和640个未标注的IOS样本，以及30个标注和250个未标注的不同分辨率和视野的CBCT扫描。参赛团队开发了基于深度学习的半监督解决方案。

Result: 在分割任务中，领先方法使用nnU-Net和Mamba-like状态空间模型，结合伪标签和一致性正则化，在隐藏测试集上获得了0.967的Dice分数和0.738的实例亲和度。在配准任务中，有效方法结合PointNetLK与可微SVD和几何增强来处理模态差异；混合神经-经典细化方法在有限标注下实现了准确对齐。

Conclusion: STSR 2025挑战赛成功推动了数字牙科中半监督学习的发展，展示了在标注数据有限的情况下，通过先进深度学习技术实现高质量牙齿分割和跨模态配准的可行性。所有数据和代码已公开以确保可重复性。
...
中文摘要: 锥形束计算机断层扫描（CBCT）和口内扫描（IOS）是数字牙科的关键技术，但标注数据稀缺限制了牙髓管自动分割和跨模态配准解决方案的发展。为了在该领域建立半监督学习（SSL）的基准，我们在MICCAI 2025组织了STSR 2025挑战赛，包含两个任务：（1）CBCT中牙齿和牙髓管的半监督分割；（2）CBCT和IOS的半监督刚性配准。我们提供了60个标注和640个未标注的IOS样本，以及30个标注和250个未标注的不同分辨率和视野的CBCT扫描。挑战赛吸引了强大的社区参与，顶尖团队提交了基于深度学习的开源半监督解决方案。对于分割任务，领先方法使用nnU-Net和Mamba-like状态空间模型，结合伪标签和一致性正则化，在隐藏测试集上获得了0.967的Dice分数和0.738的实例亲和度。对于配准任务，有效方法结合PointNetLK与可微SVD和几何增强来处理模态差异；混合神经-经典细化方法在有限标注下实现了准确对齐。所有数据和代码已在https://github.com/ricoleehduu/STS-Challenge-2025公开以确保可重复性。

</details>


### [71] [Taming Camera-Controlled Video Generation with Verifiable Geometry Reward](https://arxiv.org/abs/2512.02870)
*Zhaoqing Wang,Xiaobo Xia,Zhuolin Bie,Jinlin Liu,Dongdong Yu,Jia-Wang Bian,Changhu Wang*

Main category: cs.CV

TL;DR: 提出在线强化学习后训练框架，通过可验证几何奖励优化预训练视频生成器的相机控制能力，显著优于传统监督微调方法。


<details>
  <summary>Details</summary>
Motivation: 当前相机控制视频生成主要依赖监督微调，而在线强化学习后训练方法未被充分探索。现有方法在相机控制精度和几何一致性方面存在不足，需要更有效的优化框架来提升生成质量。

Method: 设计在线RL后训练框架，引入可验证几何奖励：1）估计生成和参考视频的3D相机轨迹；2）将轨迹划分为短片段并计算相对姿态；3）通过比较片段对生成密集奖励信号；4）构建包含大幅度相机运动和多样化场景的数据集。

Result: 实验表明该方法在相机控制精度、几何一致性和视觉质量方面明显优于监督微调基线，有效缓解了奖励稀疏性问题，提高了优化效率。

Conclusion: 在线强化学习后训练结合可验证几何奖励是推进相机控制视频生成的有效方法，为视频生成模型的优化提供了新思路，在多个评估维度上展现出优越性能。
...
中文摘要: 视频扩散模型的最新进展显著改善了相机控制视频生成，但大多数方法仅依赖于监督微调（SFT），使得在线强化学习（RL）后训练在很大程度上未被充分探索。在这项工作中，我们引入了一个在线RL后训练框架，用于优化预训练的视频生成器以实现精确的相机控制。为了使RL在此设置中有效，我们设计了一个可验证的几何奖励，提供密集的片段级反馈来指导模型优化。具体而言，我们估计生成视频和参考视频的3D相机轨迹，将每个轨迹划分为短片段，并计算片段间的相对姿态。奖励函数然后比较每个生成-参考片段对，并将对齐分数分配为奖励信号，这有助于缓解奖励稀疏性并提高优化效率。此外，我们构建了一个全面的数据集，包含多样化的大幅度相机运动和具有不同主体动态的场景。广泛的实验表明，我们的在线RL后训练在多个方面明显优于SFT基线，包括相机控制精度、几何一致性和视觉质量，证明了其在推进相机控制视频生成方面的优越性。

</details>


### [72] [MindGPT-4ov: An Enhanced MLLM via a Multi-Stage Post-Training Paradigm](https://arxiv.org/abs/2512.02895)
...
TL;DR: MindGPT-4ov是一个通过多阶段后训练范式增强的多模态大语言模型，在数据生成、模型训练和高效部署方面提出创新，以低成本在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型面临高质量跨领域数据稀缺、特定领域知识注入与通用能力保持难以平衡、推理能力与多目标优化挑战等问题，需要一种通用的后训练范式来低成本提升模型性能。

Method: 提出三阶段后训练范式：1) 基于信息密度的数据生成方案，结合双维度树状标签系统自动生成高质量跨领域数据；2) 协作课程监督微调方法，平衡领域知识注入与通用能力保持；3) 混合强化学习范式，增强推理能力同时优化多样性探索、多模态感知保持和响应简洁性等多目标。

Result: MindGPT-4ov在MMBench、MMStar、MathVision、MathVista等基准测试中超越现有最先进模型，在垂直领域任务中展现卓越用户体验，实现从学术研究到工业部署的无缝过渡。

Conclusion: MindGPT-4ov提供了一个适用于广泛多模态大语言模型的通用后训练范式，通过创新的数据生成、训练策略和基础设施优化，以低成本实现了性能提升和高效部署。
...
中文摘要: 我们提出了MindGPT-4ov，这是一个多模态大语言模型，引入了一个涵盖数据生产、模型训练和高效部署的通用后训练范式。它以低成本在多个基准测试中实现了最先进的性能，有效增强了MLLMs的基础能力和泛化能力。本工作聚焦于数据构建、监督微调策略和多模态强化学习方法，提出了三个关键创新：(1) 基于信息密度的数据生成方案，结合双维度树状标签系统，实现高质量跨领域数据的自动生成。(2) 协作课程监督微调方法，平衡领域特定知识注入与通用能力保持。(3) 混合强化学习范式，在增强推理能力的同时，解决多样性探索、多模态感知保持和响应简洁性等多目标优化问题。此外，我们实施了一系列基础设施优化，如5D并行训练、算子优化和推理量化，以提高训练和推理效率，同时降低领域适应成本。实验结果表明，MindGPT-4ov模型在MMBench、MMStar、MathVision和MathVista等基准测试中优于最先进的模型。此外，MindGPT-4ov在垂直领域任务中也展现出卓越的用户体验，实现了从学术研究到工业部署的无缝过渡。MindGPT-4ov提供了一个适用于广泛MLLMs的通用后训练范式。基于Qwen3-VL的变体模型权重、数据集和代码将近期开源，以支持社区开发MLLMs。

</details>


### [73] [Glance: Accelerating Diffusion Models with 1 Sample](https://arxiv.org/abs/2512.02899)
*Zhuobai Dong,Rui Zhao,Songjie Wu,Junchao Yi,Linjie Li,Zhengyuan Yang,Lijuan Wang,Alex Jinpeng Wang*

Main category: cs.CV

TL;DR: Glance方法通过为扩散模型添加两个轻量级LoRA适配器（Slow-LoRA和Fast-LoRA），实现了智能加速而非均匀加速，在仅用1个样本训练的情况下达到5倍加速效果，同时保持视觉质量。


<details>
  <summary>Details</summary>
Motivation: 扩散模型虽然图像生成效果出色，但部署受到高计算成本和大量推理步骤的限制。现有方法试图通过训练紧凑的学生模型来跳过冗余步骤，但存在重新训练成本高和泛化能力下降的问题。

Method: 采用阶段感知策略：对早期语义阶段应用较小的加速，对后期冗余阶段应用较大的加速。具体实现为两个专家模型：Slow-LoRA处理慢速去噪阶段，Fast-LoRA处理快速去噪阶段。仅需为基模型添加轻量级LoRA适配器，无需大规模重新训练。

Result: 方法在基模型基础上实现了高达5倍的加速，同时在多样化基准测试中保持可比较的视觉质量。LoRA专家仅用1个样本在单个V100 GPU上训练1小时，却能在未见过的提示上表现出强大的泛化能力。

Conclusion: Glance方法通过阶段感知的智能加速策略，使用轻量级LoRA适配器实现了高效加速和强泛化能力，为扩散模型的部署提供了实用解决方案。
...
中文摘要: 扩散模型在图像生成方面取得了显著成功，但其部署仍然受到高计算成本和需要大量推理步骤的限制。先前关于减少步骤的蒸馏方法试图通过训练紧凑的学生模型来跳过冗余步骤，但这些方法通常存在重新训练成本高和泛化能力下降的问题。在这项工作中，我们采取了不同的视角：我们智能地加速，而不是均匀地加速，对早期语义阶段应用较小的加速，对后期冗余阶段应用较大的加速。我们通过两个专门处理慢速和快速去噪阶段的专家模型来实例化这种阶段感知策略。令人惊讶的是，我们发现在基模型上简单地配备轻量级LoRA适配器，而不是投入大量精力重新训练学生模型，既能实现高效加速，又能保持强大的泛化能力。我们将这两个适配器称为Slow-LoRA和Fast-LoRA。通过大量实验，我们的方法在基模型基础上实现了高达5倍的加速，同时在多样化基准测试中保持可比较的视觉质量。值得注意的是，LoRA专家仅用1个样本在单个V100 GPU上训练1小时，但生成的模型在未见过的提示上表现出强大的泛化能力。
...
TL;DR: MRD提出了一种无需训练的多分辨率检索-检测融合框架，通过多分辨率语义融合解决高分辨率图像中物体被分割到不同图像块导致的语义相似性偏差问题，并结合开放词汇目标检测模型实现目标对象的全局定位。


<details>
  <summary>Details</summary>
Motivation: 现有方法将高分辨率图像分割成小块，通过预训练的RAG模型计算每个小块与查询的语义相似性，选择最相关的小块来定位目标对象并抑制无关信息。但这种方法可能导致完整物体被分割到多个小块中，破坏语义相似性计算。实验发现不同大小的物体在不同分辨率下处理效果更好。

Method: 提出MRD框架：1）多分辨率语义融合方法，整合不同分辨率下获得的语义相似性图，产生更准确的语义信息并保持目标对象的完整性；2）引入开放词汇目标检测模型，通过滑动窗口方法识别对象区域，实现目标对象的全局直接定位。

Result: 在不同MLLMs上进行的高分辨率图像理解基准测试实验证明了该方法的有效性。

Conclusion: MRD框架通过多分辨率检索-检测融合有效解决了高分辨率图像理解中的挑战，特别是物体被分割到不同图像块导致的语义相似性偏差问题，实现了更准确的目标对象定位和理解。
...
中文摘要: 理解高分辨率图像仍然是多模态大语言模型（MLLMs）面临的重要挑战。最近的研究通过将图像分割成较小的图像块，并使用预训练的检索增强生成（RAG）模型计算每个图像块与查询之间的语义相似性来解决这个问题。然后选择最相关的图像块来定位目标对象并抑制无关信息。然而，这种基于图像块的处理方式可能会将完整物体分割到多个图像块中，从而破坏语义相似性的计算。在我们的实验中，我们发现不同大小的物体图像块在不同分辨率下处理效果更好。基于这一观察，我们提出了多分辨率检索-检测（MRD），这是一个用于高分辨率图像理解的无训练框架。为了解决物体被分割到不同图像块导致的语义相似性偏差问题，我们提出了一种多分辨率语义融合方法，该方法整合了在不同分辨率下获得的语义相似性图，以产生更准确的语义信息并保持目标对象的完整性。此外，为了实现目标对象在全局尺度上的直接定位，我们引入了一个开放词汇目标检测（OVD）模型，该模型使用滑动窗口方法识别对象区域。在不同MLLMs上进行的高分辨率图像理解基准测试实验证明了我们方法的有效性。

</details>


### [75] [DiverseAR: Boosting Diversity in Bitwise Autoregressive Image Generation](https://arxiv.org/abs/2512.02931)
*Ying Yang,Zhengyao Lv,Tianlin Pan,Haofan Wang,Binxin Yang,Hubery Yin,Chen Li,Chenyang Si*

Main category: cs.CV

TL;DR: DiverseAR通过自适应对数分布缩放和基于能量的生成路径搜索，解决了比特自回归图像生成中的多样性不足问题，在保持视觉质量的同时显著提升了样本多样性。


<details>
  <summary>Details</summary>
Motivation: 研究比特自回归生成模型中样本多样性的不足问题。分析发现两个关键限制因素：1）比特建模的二元分类性质限制了预测空间；2）过于尖锐的对数分布导致采样崩溃和多样性降低。

Method: 提出DiverseAR方法，包含两个核心技术：1）自适应对数分布缩放机制，在采样过程中动态调整二元输出分布的尖锐度，实现更平滑的预测和更大多样性；2）基于能量的生成路径搜索算法，避免采样低置信度标记，保持高视觉质量。

Result: 大量实验表明，DiverseAR在比特自回归图像生成中显著提高了样本多样性，同时没有牺牲视觉质量。

Conclusion: DiverseAR通过解决比特自回归模型中的多样性限制因素，提供了一种原则性且有效的方法，在保持图像质量的同时增强了生成多样性。
...
中文标题: DiverseAR：提升比特自回归图像生成的多样性

中文摘要: 本文研究了比特自回归生成模型中样本多样性的未充分探索的挑战。我们首先分析了比特自回归模型中限制多样性的因素，识别出两个关键问题：（1）比特建模的二元分类性质限制了预测空间；（2）过于尖锐的对数分布导致采样崩溃和多样性降低。基于这些见解，我们提出了DiverseAR，一种原则性且有效的方法，可以在不牺牲视觉质量的情况下增强图像多样性。具体来说，我们引入了一种自适应对数分布缩放机制，在采样过程中动态调整二元输出分布的尖锐度，从而实现更平滑的预测和更大的多样性。为了减轻分布平滑可能导致的保真度损失，我们进一步开发了一种基于能量的生成路径搜索算法，避免采样低置信度标记，从而保持高视觉质量。大量实验表明，DiverseAR在比特自回归图像生成中显著提高了样本多样性。
...
TL;DR: EGGS提出了一种可交换的2D/3D高斯溅射混合表示方法，通过统一渲染、自适应类型交换和频率解耦优化，在保持高渲染质量的同时提升几何精度。


<details>
  <summary>Details</summary>
Motivation: 现有3D高斯溅射(3DGS)虽然渲染质量高且实时，但存在多视角不一致问题，几何精度有限；而2D高斯溅射(2DGS)虽然保证了多视角一致性，但牺牲了纹理细节。需要一种平衡外观和几何的方法。

Method: 提出交换式高斯溅射(EGGS)：1)混合高斯光栅化实现统一渲染；2)自适应类型交换机制动态调整2D/3D高斯；3)频率解耦优化策略充分利用不同类型高斯的优势；4)CUDA加速实现高效训练和推理。

Result: 实验表明EGGS在渲染质量、几何精度和效率方面均优于现有方法，为高质量新视角合成提供了实用解决方案。

Conclusion: EGGS通过创新的混合表示方法成功平衡了外观保真度和几何一致性，在保持实时渲染的同时显著提升了多视角合成的整体质量。
...
中文标题: EGGS：用于几何-外观平衡新视角合成的可交换2D/3D高斯溅射

中文摘要: 新视角合成(NVS)在计算机视觉和图形学中至关重要，在AR、VR和自动驾驶中有广泛应用。虽然3D高斯溅射(3DGS)能够实现高外观保真度的实时渲染，但它存在多视角不一致问题，限制了几何精度。相比之下，2D高斯溅射(2DGS)强制多视角一致性但牺牲了纹理细节。为解决这些限制，我们提出了交换式高斯溅射(EGGS)，这是一种集成2D和3D高斯的混合表示方法，以平衡外观和几何。为实现这一目标，我们引入了混合高斯光栅化进行统一渲染，自适应类型交换实现2D和3D高斯之间的动态适应，以及频率解耦优化有效利用每种高斯表示的优势。我们的CUDA加速实现确保了高效的训练和推理。大量实验表明，EGGS在渲染质量、几何精度和效率方面均优于现有方法，为高质量NVS提供了实用解决方案。

</details>


### [77] [LoVoRA: Text-guided and Mask-free Video Object Removal and Addition with Learnable Object-aware Localization](https://arxiv.org/abs/2512.02933)
*Zhihan Xiao,Lin Liu,Yixin Gao,Xiaopeng Zhang,Haoxuan Che,Songping Mai,Qi Tian*

Main category: cs.CV

TL;DR: LoVoRA是一个无需掩码的文本引导视频编辑框架，通过可学习的对象感知定位机制实现视频对象移除和添加，无需推理时的外部控制信号。
...
Method: LoVoRA采用独特的数据集构建流程，整合图像到视频转换、基于光流的掩码传播和视频修复技术，确保时间一致性编辑。核心创新是可学习的对象感知定位机制，为对象插入和移除任务提供密集的时空监督。通过扩散掩码预测器实现端到端视频编辑，推理时无需外部控制信号。

Result: 广泛的实验和人工评估证明了LoVoRA的有效性和高质量性能。该方法在视频对象移除和添加任务上表现出色，实现了精确的空间和时间一致性编辑。

Conclusion: LoVoRA提出了一种创新的无需掩码的视频编辑框架，通过可学习的对象感知定位机制解决了文本引导视频编辑中的空间和时间一致性挑战，为视频编辑任务提供了更灵活和可扩展的解决方案。
...
中文标题: LoVoRA：基于文本引导和无需掩码的视频对象移除与添加，具有可学习的对象感知定位机制

中文摘要: 文本引导的视频编辑，特别是对象移除和添加，由于需要精确的空间和时间一致性，仍然是一个具有挑战性的任务。现有方法通常依赖辅助掩码或参考图像进行编辑指导，这限制了它们的可扩展性和泛化能力。为了解决这些问题，我们提出了LoVoRA，这是一个使用对象感知定位机制进行无需掩码的视频对象移除和添加的新框架。我们的方法采用独特的数据集构建流程，整合了图像到视频转换、基于光流的掩码传播和视频修复技术，实现了时间一致的编辑。LoVoRA的核心创新是其可学习的对象感知定位机制，为对象插入和移除任务提供密集的时空监督。通过利用扩散掩码预测器，LoVoRA实现了端到端视频编辑，在推理时无需外部控制信号。广泛的实验和人工评估证明了LoVoRA的有效性和高质量性能。
...
TL;DR: VideoScience-Bench是一个新的视频生成基准测试，专注于评估模型对本科级科学概念的理解和推理能力，包含200个物理化学场景，通过五个维度评估模型表现。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成基准主要测试物理常识，缺乏对科学推理能力的评估。随着视频生成模型的发展，需要评估它们是否真正理解科学定律，而不仅仅是生成视觉上合理的视频。

Method: 创建包含200个提示的基准测试，涵盖14个物理化学主题和103个概念。每个提示编码复合科学场景，需要多概念推理。使用专家注释评估七个SOTA视频模型，评估维度包括：提示一致性、现象一致性、正确动态性、不变性、时空连续性。采用VLM-as-a-Judge方法评估视频生成。

Result: VLM-as-a-Judge评估与人类专家评估呈现强相关性。基准测试揭示了当前视频模型在科学理解和推理方面的局限性，为模型改进提供了明确方向。

Conclusion: VideoScience-Bench是首个专门评估视频模型科学理解和推理能力的基准测试，填补了现有评估体系的空白，为开发真正理解科学定律的视频生成模型提供了重要工具。
...
中文标题: 使用VideoScience-Bench对视频生成进行科学理解与推理基准测试

中文摘要: 视频生成的下一个前沿在于开发能够进行零样本推理的模型，其中理解真实世界的科学定律对于在不同条件下准确建模物理结果至关重要。然而，现有的视频基准测试主要基于物理常识，对视频模型的科学推理能力提供有限洞察。我们引入了VideoScience-Bench，这是一个旨在评估视频模型中本科级科学理解的基准测试。每个提示都编码了一个复合科学场景，需要理解和推理多个科学概念才能生成正确的现象。该基准测试包含200个精心策划的提示，涵盖物理和化学领域的14个主题和103个概念。我们在T2V和I2V设置下对七个最先进的视频模型进行了专家注释评估，涵盖五个维度：提示一致性、现象一致性、正确动态性、不变性和时空连续性。使用VLM-as-a-Judge评估视频生成，我们观察到与人类评估的强相关性。据我们所知，VideoScience-Bench是第一个不仅将视频模型评估为生成器，而且评估为推理器的基准测试，要求其生成结果展示与预期物理和化学现象一致的科学理解。我们的数据和评估代码可在以下网址获取：github.com/hao-ai-lab/VideoScience。
...
TL;DR: Layout Anything是一个基于Transformer的室内布局估计框架，采用OneFormer的通用分割架构进行几何结构预测，通过布局退化策略和可微几何损失实现高效端到端布局估计，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前室内布局估计方法通常需要复杂的后处理流程，且难以在保持几何约束的同时实现高效推理。本文旨在开发一个统一的端到端框架，既能保持曼哈顿世界约束，又能实现快速推理，适用于增强现实和大规模3D场景重建应用。

Method: 采用OneFormer的通用分割架构，结合任务条件查询和对比学习。引入两个关键模块：1）布局退化策略，通过拓扑感知变换增强训练数据同时保持曼哈顿世界约束；2）可微几何损失，在训练中直接强制平面一致性和锐利边界预测。形成端到端框架，无需复杂后处理。
...
Conclusion: Layout Anything通过结合几何感知和计算效率，提供了一个强大的端到端室内布局估计框架，特别适用于增强现实应用和大规模3D场景重建任务，在保持曼哈顿世界约束的同时实现了高效推理和最先进性能。
...
中文摘要: 我们提出了Layout Anything，一个基于Transformer的室内布局估计框架，将OneFormer的通用分割架构适配到几何结构预测中。我们的方法将OneFormer的任务条件查询和对比学习与两个关键模块相结合：(1)布局退化策略，通过拓扑感知变换增强训练数据同时保持曼哈顿世界约束；(2)可微几何损失，在训练中直接强制平面一致性和锐利边界预测。通过将这些组件统一在端到端框架中，该模型消除了复杂的后处理流程，同时实现了114ms的高速推理。大量实验证明了在标准基准测试中的最先进性能，在LSUN数据集上像素误差(PE)为5.43%、角点误差(CE)为4.02%，在Hedau数据集上PE为7.04%(CE 5.17%)，在Matterport3D-Layout数据集上PE为4.03%(CE 3.15%)。该框架结合了几何感知和计算效率，特别适用于增强现实应用和大规模3D场景重建任务。

</details>


### [80] [Contextual Image Attack: How Visual Context Exposes Multimodal Safety Vulnerabilities](https://arxiv.org/abs/2512.02973)
*Yuan Xiong,Ziqi Miao,Lijun Li,Chen Qian,Jie Li,Jing Shao*

Main category: cs.CV

TL;DR: 本文提出了一种新的多模态越狱攻击方法CIA，通过将有害内容嵌入视觉上下文来绕过MLLMs的安全防护，在多个模型上取得了高攻击成功率。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs越狱攻击主要关注文本-图像交互，将图像视为次要提示，未能充分利用图像承载复杂上下文信息的潜力。作者旨在探索视觉模态本身作为攻击向量的可能性。

Method: 提出上下文图像攻击（CIA）方法，使用多智能体系统通过四种可视化策略将有害查询嵌入良性视觉上下文，并结合上下文元素增强和自动毒性混淆技术。

Result: 在MMSafetyBench-tiny数据集上，CIA对GPT-4o和Qwen2.5-VL-72B分别获得4.73和4.83毒性分数，攻击成功率分别达到86.31%和91.07%，显著优于先前方法。

Conclusion: 视觉模态本身是越狱先进MLLMs的强大载体，现有安全对齐对基于视觉上下文的攻击存在显著漏洞，需要新的防御机制。
...
中文摘要: 虽然多模态大语言模型（MLLMs）展现出卓越的能力，但其安全对齐容易受到越狱攻击。现有的攻击方法通常侧重于文本-图像交互，将视觉模态视为次要提示。这种方法未能充分利用图像承载复杂上下文信息的独特潜力。为解决这一空白，我们提出了一种新的以图像为中心的攻击方法——上下文图像攻击（CIA），该方法采用多智能体系统，通过四种不同的可视化策略将有害查询巧妙地嵌入看似良性的视觉上下文中。为了进一步增强攻击效果，该系统结合了上下文元素增强和自动毒性混淆技术。在MMSafetyBench-tiny数据集上的实验结果表明，CIA对GPT-4o和Qwen2.5-VL-72B模型分别获得了4.73和4.83的高毒性分数，攻击成功率（ASR）分别达到86.31%和91.07%。我们的方法显著优于先前工作，证明视觉模态本身是越狱先进MLLMs的强大载体。

</details>


### [81] [InEx: Hallucination Mitigation via Introspection and Cross-Modal Multi-Agent Collaboration](https://arxiv.org/abs/2512.02981)
*Zhongyu Yang,Yingfang Yuan,Xuanming Jiang,Baoyi An,Wei Pang*

Main category: cs.CV

TL;DR: InEx是一个无需训练的多智能体框架，通过内省推理和跨模态多智能体协作来自主缓解大语言模型中的幻觉问题，在多个基准测试中取得显著提升。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型中的幻觉问题严重阻碍了可靠多模态大语言模型的发展。现有解决方案要么依赖人工干预，要么未能充分利用智能体自主缓解幻觉的能力。受人类在现实世界中做出可靠决策的认知过程启发——先通过内省推理减少不确定性形成初步判断，再依赖外部多视角验证做出最终决定。

Method: 提出InEx框架：1) 引入基于熵的不确定性估计指导的内省推理，提升决策智能体推理过程的可靠性；2) 智能体首先生成响应，然后通过编辑智能体和自反思智能体进行跨模态多智能体协作，迭代验证和精炼响应，进一步缓解幻觉。

Result: 大量实验表明，InEx在通用和幻觉基准测试中持续优于现有方法，取得了4%-27%的性能提升，并展现出强大的鲁棒性。

Conclusion: InEx通过模拟人类认知决策过程，结合内省推理和跨模态多智能体协作，有效缓解了大语言模型中的幻觉问题，为构建更可靠的多模态大语言模型提供了有前景的解决方案。
...
中文标题: InEx：通过内省推理和跨模态多智能体协作缓解幻觉

中文摘要: 幻觉问题仍然是大型语言模型（LLMs）中的一个关键挑战，阻碍了可靠多模态大型语言模型（MLLMs）的发展。现有解决方案通常依赖人工干预或未能充分利用智能体自主缓解幻觉的能力。为了解决这些限制，我们受到人类在现实世界中做出可靠决策方式的启发。他们首先通过内省推理减少不确定性并形成初步判断，然后依赖来自不同视角的外部验证来做出最终决定。受这种认知范式的启发，我们提出了InEx，一个无需训练的多智能体框架，旨在自主缓解幻觉。InEx引入了基于熵的不确定性估计指导的内部内省推理，以提高决策智能体推理过程的可靠性。智能体首先生成一个响应，然后通过编辑智能体和自反思智能体进行跨模态多智能体协作，迭代验证和精炼响应，进一步增强可靠性并缓解幻觉。大量实验表明，InEx在通用和幻觉基准测试中持续优于现有方法，取得了4%-27%的性能提升，并展现出强大的鲁棒性。

</details>


### [82] [GraphFusion3D: Dynamic Graph Attention Convolution with Adaptive Cross-Modal Transformer for 3D Object Detection](https://arxiv.org/abs/2512.02991)
*Md Sohag Mia,Md Nahid Hasan,Tawhid Ahmed,Muhammad Abdullah Adnan*

Main category: cs.CV

TL;DR: GraphFusion3D是一个用于3D目标检测的统一框架，通过自适应跨模态Transformer融合图像特征到点云表示，并使用图推理模块建模邻域关系，在SUN RGB-D和ScanNetV2数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 点云数据存在稀疏性、不完整结构和语义信息有限的问题，同时捕捉远距离物体之间的上下文关系也很困难。现有方法在处理这些挑战时仍有不足。

Method: 1. 自适应跨模态Transformer（ACMT）：自适应地将图像特征集成到点表示中，丰富几何和语义信息。2. 图推理模块（GRM）：通过多尺度图注意力建模邻域关系，同时捕捉局部几何结构和全局语义上下文。3. 级联解码器：通过多阶段预测逐步细化检测结果。

Result: 在SUN RGB-D数据集上达到70.6% AP25和51.2% AP50，在ScanNetV2数据集上达到75.1% AP25和60.8% AP50，显著优于现有方法。

Conclusion: GraphFusion3D通过结合多模态融合和高级特征学习，有效解决了点云数据的稀疏性和语义信息有限问题，在3D目标检测任务中取得了优异性能。
...
中文标题: GraphFusion3D：基于自适应跨模态Transformer的动态图注意力卷积用于3D目标检测

中文摘要: 尽管3D目标检测取得了显著进展，但点云数据仍然面临稀疏性、不完整结构和语义信息有限的挑战。捕捉远距离物体之间的上下文关系也存在困难。为解决这些问题，我们提出了GraphFusion3D，一个结合多模态融合与高级特征学习的统一框架。我们的方法引入了自适应跨模态Transformer（ACMT），它自适应地将图像特征集成到点表示中，丰富几何和语义信息。对于提议细化，我们引入了图推理模块（GRM），这是一个新颖的机制，通过建模邻域关系同时捕捉局部几何结构和全局语义上下文。该模块采用多尺度图注意力动态加权提议之间的空间邻近性和特征相似性。我们进一步采用级联解码器，通过多阶段预测逐步细化检测结果。在SUN RGB-D（70.6% AP25和51.2% AP50）和ScanNetV2（75.1% AP25和60.8% AP50）上的大量实验表明，相比现有方法取得了显著的性能提升。

</details>


### [83] [TEXTRIX: Latent Attribute Grid for Native Texture Generation and Beyond](https://arxiv.org/abs/2512.02993)
*Yifei Zeng,Yajie Bao,Jiachen Qian,Shuang Wu,Youtian Lin,Hao Zhu,Buyu Li,Feihu Zhang,Xun Cao,Yao Yao*

Main category: cs.CV

TL;DR: TEXTRIX提出了一种原生3D属性生成框架，通过构建潜在3D属性网格和配备稀疏注意力的扩散Transformer，直接在体积空间中为3D模型着色，避免了多视图融合的限制，实现了高保真纹理合成和精确的3D部件分割。


<details>
  <summary>Details</summary>
Motivation: 现有的3D纹理生成方法通常依赖多视图融合，但这种方法经常受到视图间不一致性和复杂表面覆盖不完整的困扰，限制了生成内容的保真度和完整性。

Method: 构建潜在3D属性网格，利用配备稀疏注意力的扩散Transformer，直接在体积空间中为3D模型着色，避免多视图融合的限制。基于这种原生表示，框架可扩展到高精度3D分割，通过训练相同架构预测网格上的语义属性。

Result: 在纹理生成和3D部件分割两个任务上都实现了最先进的性能，生成了无缝、高保真的纹理和具有精确边界的准确3D部件分割。

Conclusion: TEXTRIX通过原生3D属性生成方法有效解决了多视图融合的限制，为高保真纹理合成和精确3D分割提供了统一的解决方案，在两项任务上都表现出色。
...
中文标题: TEXTRIX：用于原生纹理生成及更多应用的潜在属性网格

中文摘要: 现有的3D纹理生成方法通常依赖多视图融合，但经常受到视图间不一致性和复杂表面覆盖不完整的困扰，限制了生成内容的保真度和完整性。为了克服这些挑战，我们引入了TEXTRIX，这是一个用于高保真纹理合成和下游应用（如精确3D部件分割）的原生3D属性生成框架。我们的方法构建了一个潜在3D属性网格，并利用配备稀疏注意力的扩散Transformer，直接在体积空间中为3D模型着色，从根本上避免了多视图融合的限制。基于这种原生表示，该框架自然地扩展到高精度3D分割，通过训练相同架构来预测网格上的语义属性。大量实验证明，在两个任务上都实现了最先进的性能，生成了无缝、高保真的纹理和具有精确边界的准确3D部件分割。

</details>


### [84] [DynamicVerse: A Physically-Aware Multimodal Framework for 4D World Modeling](https://arxiv.org/abs/2512.03000)
...
TL;DR: DynamicVerse是一个物理感知的多模态4D世界建模框架，通过整合大型视觉、几何和多模态模型，将真实世界视频转换为包含度量尺度几何、动态运动、实例掩码和描述性标注的4D格式，并在多个基准任务上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有数据集通常来自有限模拟器或使用传统运动结构恢复方法，标注信息有限，导致基础模型难以从互联网单目视频中准确理解真实世界动态。需要物理尺度、多模态的4D世界建模框架来支持具身智能体在真实环境中的感知和交互。

Method: 采用大型视觉、几何和多模态模型解析视频内容，通过基于窗口的束调整与全局优化相结合，将长视频序列转换为包含度量尺度静态几何、真实世界动态运动、实例级掩码和整体描述性标注的4D多模态格式。

Result: 构建了包含10万+视频、80万+标注掩码和1000万+帧的大规模数据集。在视频深度估计、相机姿态估计和相机内参估计三个基准任务上，DynamicVerse的4D建模方法在捕捉物理尺度测量方面表现出优于现有方法的性能，具有更高的全局精度。

Conclusion: DynamicVerse提供了一个物理感知的多模态4D世界建模框架，能够从真实世界视频中准确提取物理尺度信息，为具身智能体在真实环境中的感知和交互提供了重要基础，在多个基准任务上验证了其优越性能。
...
中文标题: DynamicVerse：一个物理感知的多模态4D世界建模框架

中文摘要: 理解动态物理世界，其特征是不断演化的3D结构、真实世界运动以及带有文本描述的语义内容，对于人机交互至关重要，并使具身智能体能够在真实环境中以类人能力感知和行动。然而，现有数据集通常源自有限的模拟器或使用传统的运动结构恢复方法进行尺度标注，且提供有限的描述性标注，这限制了基础模型从互联网常见的单目视频中准确解释真实世界动态的能力。为弥合这些差距，我们引入了DynamicVerse，一个用于动态真实世界视频的物理尺度多模态4D世界建模框架。我们采用大型视觉、几何和多模态模型来解释度量尺度的静态几何、真实世界动态运动、实例级掩码和整体描述性标注。通过将基于窗口的束调整与全局优化相结合，我们的方法将长真实世界视频序列转换为全面的4D多模态格式。DynamicVerse提供了一个大规模数据集，包含来自互联网视频的10万+视频、80万+标注掩码和1000万+帧。在三个基准任务（视频深度估计、相机姿态估计和相机内参估计）上的实验评估表明，我们的4D建模在捕捉物理尺度测量方面实现了优于现有方法的性能，具有更高的全局精度。

</details>


### [85] [DGGT: Feedforward 4D Reconstruction of Dynamic Driving Scenes using Unposed Images](https://arxiv.org/abs/2512.03004)
...
TL;DR: DGGT是一个前馈式4D动态驾驶场景重建框架，直接从无位姿图像预测3D高斯地图和相机参数，支持任意视角和长序列，无需逐场景优化或已知相机标定。


<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要快速、可扩展的4D重建和重模拟用于训练和评估，但现有方法依赖逐场景优化、已知相机标定或短帧窗口，导致速度慢且不实用。现有方法将相机位姿作为必需输入限制了灵活性和可扩展性。

Method: 提出Driving Gaussian Grounded Transformer (DGGT)统一框架，将相机位姿重新定义为模型输出而非输入。方法联合预测每帧3D高斯地图和相机参数，使用轻量级动态头解耦动态元素，通过寿命头保持时间一致性，并采用基于扩散的渲染细化减少运动/插值伪影。

Result: 在Waymo、nuScenes、Argoverse2等大规模驾驶基准测试中，DGGT在单数据集训练和跨数据集零样本迁移方面均优于先前工作，随着输入帧数增加具有良好的扩展性，实现了最先进的性能和速度。

Conclusion: DGGT提供了一个单次前馈、无位姿的算法，能够从稀疏无位姿图像直接重建动态驾驶场景，支持任意数量视角和长序列，为自动驾驶的快速4D重建和重模拟提供了实用解决方案。
...
中文标题: DGGT：使用无位姿图像进行动态驾驶场景的前馈式4D重建

中文摘要: 自动驾驶需要快速、可扩展的4D重建和重模拟用于训练和评估，然而大多数动态驾驶场景重建方法仍然依赖逐场景优化、已知相机标定或短帧窗口，导致速度慢且不实用。我们从前馈角度重新审视这个问题，提出了Driving Gaussian Grounded Transformer (DGGT)，一个用于无位姿动态场景重建的统一框架。我们注意到现有方法将相机位姿作为必需输入，限制了灵活性和可扩展性。相反，我们将位姿重新定义为模型的输出，使得能够直接从稀疏无位姿图像进行重建，并支持任意数量视角的长序列。我们的方法联合预测每帧3D高斯地图和相机参数，通过轻量级动态头解耦动态元素，并通过寿命头保持时间一致性来调节随时间变化的可见性。基于扩散的渲染细化进一步减少了运动/插值伪影，并在稀疏输入下提高了新视角质量。结果是一个单次前馈、无位姿算法，实现了最先进的性能和速度。在大规模驾驶基准测试（Waymo、nuScenes、Argoverse2）上进行训练和评估，我们的方法在单数据集训练和跨数据集零样本迁移方面均优于先前工作，并且随着输入帧数增加具有良好的扩展性。

</details>


### [86] [In-Context Sync-LoRA for Portrait Video Editing](https://arxiv.org/abs/2512.03013)
*Sagi Polaczek,Or Patashnik,Ali Mahdavi-Amiri,Daniel Cohen-Or*

Main category: cs.CV

TL;DR: Sync-LoRA是一种基于图像到视频扩散模型的肖像视频编辑方法，通过训练上下文LoRA在保持帧级精确同步的同时实现高质量视觉编辑。


<details>
  <summary>Details</summary>
Motivation: 肖像视频编辑需要灵活控制各种修改（外观、表情、添加对象等），同时保持原始时间行为和身份一致性。现有方法难以在编辑保真度和精确运动同步之间取得平衡。

Method: 1. 使用图像到视频扩散模型，通过修改第一帧定义编辑并传播到整个序列；2. 训练上下文LoRA，使用自动生成的成对视频（相同运动轨迹但不同外观）；3. 通过同步过滤筛选高质量训练数据；4. 模型学习结合源视频运动线索和编辑第一帧的视觉变化。

Result: Sync-LoRA在紧凑的同步人类肖像数据集上训练后，能够泛化到未见过的身份和多样化编辑，处理姿势和表情变化，实现高视觉保真度和强时间一致性，在编辑保真度和精确运动保持之间达到良好平衡。

Conclusion: Sync-LoRA提供了一种有效的肖像视频编辑解决方案，通过上下文LoRA训练实现了高质量视觉修改与精确时间同步的平衡，能够处理多种编辑任务并保持身份一致性。
...
中文摘要: 编辑肖像视频是一项具有挑战性的任务，需要对各种修改（如外观变化、表情编辑或添加对象）进行灵活而精确的控制。主要困难在于保持主体的原始时间行为，要求每个编辑后的帧与相应的源帧保持精确同步。我们提出了Sync-LoRA，一种用于编辑肖像视频的方法，在保持帧级精确同步和身份一致性的同时实现高质量的视觉修改。我们的方法使用图像到视频扩散模型，其中编辑通过修改第一帧定义，然后传播到整个序列。为了实现精确同步，我们使用成对的视频训练一个上下文LoRA，这些视频描绘相同的运动轨迹但外观不同。这些配对通过基于同步的过滤过程自动生成和筛选，仅选择最时间对齐的示例进行训练。这种训练设置教会模型将源视频的运动线索与编辑第一帧引入的视觉变化相结合。在紧凑、高度筛选的同步人类肖像数据集上训练，Sync-LoRA能够泛化到未见过的身份和多样化的编辑（如修改外观、添加对象或更改背景），稳健地处理姿势和表情的变化。我们的结果展示了高视觉保真度和强时间一致性，在编辑保真度和精确运动保持之间实现了稳健的平衡。

</details>


### [87] [AutoBrep: Autoregressive B-Rep Generation with Unified Topology and Geometry](https://arxiv.org/abs/2512.03018)
*Xiang Xu,Pradeep Kumar Jayaraman,Joseph G. Lambourne,Yilin Liu,Durvesh Malpure,Pete Meltzer*

Main category: cs.CV

TL;DR: AutoBrep是一种基于Transformer的自回归模型，通过统一的标记化方案将B-Rep的几何和拓扑编码为序列，实现高质量、水密的CAD模型生成。


<details>
  <summary>Details</summary>
Motivation: B-Rep是CAD中的标准数据结构，但现有方法难以直接生成具有精确几何和水密拓扑的B-Rep。需要一种能够统一处理几何和拓扑特征，并保证生成模型有效性的方法。

Method: 提出AutoBrep模型：1）统一的标记化方案将几何基元编码为潜在几何标记，拓扑关系编码为参考标记；2）采用Transformer架构进行自回归序列生成；3）序列顺序遵循B-Rep面邻接图的广度优先遍历；4）逐步生成相邻面和边及其拓扑结构。

Result: 实验表明AutoBrep在质量和拓扑水密性方面优于基线方法，能够扩展到复杂实体模型，保持良好保真度和推理速度。同时支持B-Rep自动完成功能，实现用户可控的CAD生成。

Conclusion: AutoBrep通过统一的几何-拓扑表示和自回归生成，有效解决了B-Rep生成中的质量和有效性问题，为CAD模型生成提供了可扩展且实用的解决方案。
...
中文标题: AutoBrep：具有统一拓扑和几何的自回归B-Rep生成方法

中文摘要: 边界表示（B-Rep）是计算机辅助设计（CAD）中定义实体模型的标准数据结构。尽管最近取得了进展，但直接端到端生成具有精确几何形状和水密拓扑的B-Rep仍然是一个挑战。本文提出了AutoBrep，一种新颖的Transformer模型，能够自回归地生成高质量且有效的B-Rep。AutoBrep采用统一的标记化方案，将B-Rep模型的几何和拓扑特征编码为离散标记序列。几何基元（即曲面和曲线）被编码为潜在几何标记，其结构关系被定义为特殊的拓扑参考标记。AutoBrep中的序列顺序自然地遵循B-Rep面邻接图的广度优先遍历。在推理时，相邻的面和边及其拓扑结构被逐步生成。大量实验证明了我们的统一表示与下一标记预测相结合在B-Rep生成中的优势。AutoBrep在质量和水密性方面优于基线方法。它还具有高度可扩展性，能够处理复杂实体并保持良好的保真度和推理速度。我们进一步展示了通过我们的统一标记化原生支持B-Rep的自动完成功能，实现了用户可控的CAD生成，只需最小改动。代码可在https://github.com/AutodeskAILab/AutoBrep获取。

</details>


### [88] [MAViD: A Multimodal Framework for Audio-Visual Dialogue Understanding and Generation](https://arxiv.org/abs/2512.03034)
*Youxin Pang,Jiajun Liu,Lingfeng Tan,Yong Zhang,Feng Gao,Xiang Deng,Zhuoliang Kang,Xiaoming Wei,Yebin Liu*

Main category: cs.CV

TL;DR: MAViD是一个用于音视频对话理解和生成的多模态框架，采用Conductor-Creator架构，通过分解动作和语音指令实现细粒度控制，结合自回归和扩散模型生成高质量长视频，并引入融合模块增强多模态同步。
...
Method: 提出Conductor-Creator架构：Conductor负责理解、推理和生成指令，分解为动作和语音组件；Creator基于指令生成交互响应。采用自回归模型生成音频，扩散模型生成高质量视频，并引入新颖的融合模块增强上下文连续片段和模态间的连接。

Result: 大量实验表明，该框架能够生成生动且上下文连贯的长时长对话交互，并能准确解释用户的多模态查询。

Conclusion: MAViD框架成功解决了音视频对话理解和生成的挑战，通过创新的架构设计和融合机制，实现了高质量、连贯的多模态交互生成。
...
中文摘要: 我们提出了MAViD，一种新颖的用于音视频对话理解和生成的多模态框架。现有方法主要关注非交互式系统，且仅限于生成受限且不自然的人类语音。该任务的主要挑战在于有效整合理解和生成能力，以及实现无缝的多模态音视频融合。为解决这些问题，我们提出了Conductor-Creator架构，将对话系统分为两个主要组件。Conductor负责理解、推理和生成指令，将其分解为动作和语音组件，从而实现细粒度的交互控制。Creator然后基于这些指令生成交互响应。此外，为了解决使用双重DiT结构生成具有一致身份、音色和语调的长视频的困难，Creator采用了结合自回归（AR）和扩散模型的结构。AR模型负责音频生成，而扩散模型确保高质量的视频生成。此外，我们提出了一种新颖的融合模块，以增强上下文连续片段和模态之间的连接，实现同步的长时长音视频内容生成。大量实验表明，我们的框架能够生成生动且上下文连贯的长时长对话交互，并能准确解释用户的多模态查询。
...
TL;DR: ViSAudio是一个端到端视频驱动的双耳空间音频生成框架，直接从静音视频生成空间沉浸式音频，避免了传统两阶段方法的误差累积问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频到音频生成领域主要关注单声道输出，缺乏空间沉浸感。现有双耳音频生成方法采用两阶段流程（首先生成单声道音频，然后空间化），导致误差累积和时空不一致问题。

Method: 提出端到端框架ViSAudio：1) 构建BiAudio数据集（约97K视频-双耳音频对）；2) 采用条件流匹配与双分支音频生成架构，两个专用分支分别建模音频潜在流；3) 集成条件时空模块，平衡通道间一致性同时保留空间特征。

Result: ViSAudio在客观指标和主观评估方面均优于现有最先进方法，能够生成高质量双耳音频，有效适应视角变化、声源运动和多样化声学环境。

Conclusion: ViSAudio成功实现了直接从视频生成空间沉浸式双耳音频的端到端任务，解决了传统两阶段方法的局限性，为视频驱动的空间音频生成提供了新方向。
...
中文摘要: 尽管视频到音频生成领域取得了进展，但该领域主要专注于单声道输出，缺乏空间沉浸感。现有的双耳方法仍然受限于两阶段流水线，首先生成单声道音频，然后进行空间化处理，通常会导致误差累积和时空不一致。为了解决这一限制，我们引入了直接从静音视频进行端到端双耳空间音频生成的任务。为了支持这一任务，我们提出了BiAudio数据集，包含约97K个视频-双耳音频对，涵盖多样化的真实世界场景和相机旋转轨迹，通过半自动化流水线构建。此外，我们提出了ViSAudio，一个端到端框架，采用条件流匹配与双分支音频生成架构，其中两个专用分支对音频潜在流进行建模。结合条件时空模块，它在通道间平衡一致性，同时保留独特的空间特征，确保音频与输入视频之间的精确时空对齐。综合实验表明，ViSAudio在客观指标和主观评估方面均优于现有最先进方法，生成具有空间沉浸感的高质量双耳音频，能够有效适应视角变化、声源运动和多样化声学环境。

</details>


### [90] [Video4Spatial: Towards Visuospatial Intelligence with Context-Guided Video Generation](https://arxiv.org/abs/2512.03040)
*Zeqi Xiao,Yiwei Zhao,Lingxiao Li,Yushi Lan,Yu Ning,Rahul Garg,Roshni Cooper,Mohammad H. Taghavi,Xingang Pan*

Main category: cs.CV

TL;DR: Video4Spatial是一个视频生成框架，仅使用视频场景上下文就能执行复杂的空间任务，包括场景导航和物体定位，展示了视频扩散模型的空间理解能力。


<details>
  <summary>Details</summary>
Motivation: 研究视频生成模型是否能够仅使用视觉数据展现出类似人类认知的视觉空间智能，探索视频模型在复杂空间任务中的潜力。

Method: 提出Video4Spatial框架，使用仅基于视频的场景上下文条件化视频扩散模型，通过简单的设计选择和数据整理，使模型能够执行场景导航和物体定位任务。

Result: Video4Spatial展示了强大的空间理解能力：能够端到端规划导航和定位目标物体，遵循相机姿态指令同时保持空间一致性，并能泛化到长上下文和域外环境。

Conclusion: 这些结果表明视频生成模型在通用视觉空间推理方面取得了进展，仅使用视频上下文就能展现出空间智能。
...
中文摘要: 我们研究视频生成模型是否能够仅使用视觉数据展现出视觉空间智能，这是人类认知的核心能力。为此，我们提出了Video4Spatial框架，表明仅基于视频场景上下文条件化的视频扩散模型能够执行复杂的空间任务。我们在两个任务上进行了验证：场景导航——遵循相机姿态指令同时保持与场景3D几何的一致性；物体定位——需要语义定位、指令遵循和规划。两个任务都仅使用视频输入，无需深度或姿态等辅助模态。通过框架和数据整理中简单而有效的设计选择，Video4Spatial展示了从视频上下文中获得强大的空间理解能力：它能够端到端规划导航和定位目标物体，遵循相机姿态指令同时保持空间一致性，并能泛化到长上下文和域外环境。综合来看，这些结果推动了视频生成模型向通用视觉空间推理的发展。

</details>


### [91] [MultiShotMaster: A Controllable Multi-Shot Video Generation Framework](https://arxiv.org/abs/2512.03041)
*Qinghe Wang,Xiaoyu Shi,Baolu Li,Weikang Bian,Quande Liu,Huchuan Lu,Xintao Wang,Pengfei Wan,Kun Gai,Xu Jia*

Main category: cs.CV

TL;DR: MultiShotMaster是一个可控的多镜头视频生成框架，通过两种新颖的RoPE变体扩展单镜头模型，实现灵活的镜头安排和时空接地的参考注入，支持文本驱动的镜头间一致性、自定义主题和场景。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成技术主要专注于单镜头片段生成，但在制作具有叙事性的多镜头视频方面存在明显不足。多镜头视频需要灵活的镜头安排、连贯的叙事流程以及超越简单文本提示的精细控制能力，这些都是现有方法难以实现的挑战。

Method: 1. 扩展预训练单镜头模型，集成两种新颖的RoPE变体：多镜头叙事RoPE（在镜头转换时应用相位偏移）和时空位置感知RoPE（纳入参考标记和接地信号）。2. 建立自动数据标注流程，从视频中提取多镜头结构、字幕、跨镜头接地信号和参考图像。3. 利用模型架构特性支持多镜头生成，包括镜头间一致性、自定义主题运动控制和背景场景定制。

Result: 实验结果表明，MultiShotMaster在生成多镜头视频方面表现出卓越性能，具有出色的可控性。框架能够灵活配置镜头数量和持续时间，实现文本驱动的镜头间一致性、自定义主题的运动控制以及背景驱动的场景定制，显著优于现有的单镜头视频生成方法。

Conclusion: MultiShotMaster成功解决了多镜头视频生成的挑战，通过创新的RoPE变体和自动数据标注流程，实现了高度可控的多镜头视频生成。该框架在保持叙事连贯性的同时提供了灵活的镜头安排能力，为复杂叙事视频创作开辟了新途径。
...
中文摘要: 当前的视频生成技术擅长生成单镜头片段，但在制作叙事性多镜头视频方面存在困难，这需要灵活的镜头安排、连贯的叙事以及超越文本提示的可控性。为了应对这些挑战，我们提出了MultiShotMaster，一个用于高度可控多镜头视频生成的框架。我们通过集成两种新颖的RoPE变体来扩展预训练的单镜头模型。首先，我们引入了多镜头叙事RoPE，它在镜头转换时应用显式的相位偏移，实现灵活的镜头安排同时保持时间叙事顺序。其次，我们设计了时空位置感知RoPE，以纳入参考标记和接地信号，实现时空接地的参考注入。此外，为了克服数据稀缺问题，我们建立了一个自动数据标注流程来提取多镜头视频、字幕、跨镜头接地信号和参考图像。我们的框架利用内在的架构特性来支持多镜头视频生成，具有文本驱动的镜头间一致性、带运动控制的自定义主题以及背景驱动的自定义场景等特点。镜头数量和持续时间都可以灵活配置。大量实验证明了我们框架的卓越性能和出色的可控性。

</details>


### [92] [OneThinker: All-in-one Reasoning Model for Image and Video](https://arxiv.org/abs/2512.03043)
...
TL;DR: OneThinker是一个统一的多模态推理模型，能够处理图像和视频的多种视觉任务，包括问答、描述、时空定位、跟踪和分割，通过多任务强化学习实现知识共享。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常为不同任务训练独立模型，并将图像和视频推理视为分离领域，这限制了向多模态推理通用模型的扩展性，阻碍了跨任务和模态的知识共享。

Method: 构建OneThinker-600k训练语料库覆盖所有任务，使用商业模型进行CoT标注得到OneThinker-SFT-340k用于SFT冷启动，提出EMA-GRPO方法处理多任务RL中的奖励异质性。

Result: 在31个基准测试中表现出色，涵盖10个基础视觉理解任务，展示了任务间的有效知识迁移和初步的零样本泛化能力。
...
中文标题: OneThinker：图像和视频的一体化推理模型

中文摘要: 强化学习最近在多模态大语言模型中引发视觉推理方面取得了显著成功。然而，现有方法通常为不同任务训练独立模型，并将图像和视频推理视为分离领域。这限制了向多模态推理通用模型的扩展性，限制了实际多功能性并阻碍了跨任务和模态的潜在知识共享。为此，我们提出了OneThinker，一个一体化推理模型，统一了图像和视频理解，涵盖多种基础视觉任务，包括问答、描述、空间和时间定位、跟踪和分割。为实现这一目标，我们构建了覆盖所有这些任务的OneThinker-600k训练语料库，并使用商业模型进行CoT标注，得到OneThinker-SFT-340k用于SFT冷启动。此外，我们提出了EMA-GRPO，通过跟踪奖励标准差的逐任务移动平均值来处理多任务RL中的奖励异质性，实现平衡优化。在多样化视觉基准上的广泛实验表明，OneThinker在31个基准测试中表现出色，涵盖10个基础视觉理解任务。此外，它展示了任务间的有效知识迁移和初步的零样本泛化能力，标志着向统一多模态推理通用模型迈出了一步。所有代码、模型和数据都已发布。

</details>


### [93] [CAMEO: Correspondence-Attention Alignment for Multi-View Diffusion Models](https://arxiv.org/abs/2512.03045)
*Minkyung Kwon,Jinhyeok Choi,Jiho Park,Seonghu Jeon,Jinhyuk Jang,Junyoung Seo,Minseop Kwak,Jin-Hwa Kim,Seungryong Kim*

Main category: cs.CV

TL;DR: CAMEO是一种通过监督注意力图来增强多视角扩散模型训练效率和生成质量的技术，利用几何对应关系引导模型学习精确对应，减少一半训练迭代并提升性能。


<details>
  <summary>Details</summary>
Motivation: 多视角扩散模型虽然能实现视角一致的新视角合成，但其保持视角一致性的机制尚不明确。研究发现这些模型的注意力图在训练中获得了几何对应关系，但这种对应信号不完整，特别是在大视角变化下准确性会降低。

Method: 提出CAMEO训练技术，直接使用几何对应关系监督注意力图。该方法只需监督单个注意力层就能引导模型学习精确对应关系，从而保留参考图像的几何结构，加速收敛过程。

Result: CAMEO将收敛所需的训练迭代次数减少了一半，同时在相同迭代次数下实现了更优的新视角合成性能。该方法被证明是模型无关的，可应用于任何多视角扩散模型。

Conclusion: CAMEO通过直接监督注意力图的几何对应关系，有效提升了多视角扩散模型的训练效率和生成质量，为理解这些模型的视角一致性机制提供了新见解，并展示了注意力监督在几何对应学习中的有效性。
...
中文摘要: 多视角扩散模型最近已成为新颖视角合成的强大范式，然而使其保持视角一致性的底层机制仍不清楚。在这项工作中，我们首先验证了这些模型的注意力图在整个训练过程中获得了几何对应关系，关注参考视角和目标视角之间的几何对应区域以实现视角一致的生成。然而，这种对应信号仍然不完整，其准确性在大视角变化下会降低。基于这些发现，我们引入了CAMEO，这是一种简单而有效的训练技术，直接使用几何对应关系来监督注意力图，以增强多视角扩散模型的训练效率和生成质量。值得注意的是，监督单个注意力层就足以引导模型学习精确的对应关系，从而保留参考图像的几何结构和形状，加速收敛，并提高新颖视角合成性能。CAMEO将收敛所需的训练迭代次数减少了一半，同时在相同迭代次数下实现了更优的性能。我们进一步证明CAMEO是模型无关的，可以应用于任何多视角扩散模型。

</details>


### [94] [MagicQuillV2: Precise and Interactive Image Editing with Layered Visual Cues](https://arxiv.org/abs/2512.03046)
...
TL;DR: MagicQuillV2提出了一种分层组合的图像编辑系统，通过将用户意图分解为内容、空间、结构和颜色四个可控制的可视化线索层，解决了扩散模型在精确控制方面的局限性。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型虽然语义生成能力强，但使用单一整体提示无法区分用户对内容、位置和外观的不同意图，缺乏传统图形软件的精细控制能力。

Method: 方法包括：1）将创意意图分解为四个可控视觉线索层（内容层、空间层、结构层、颜色层）；2）用于上下文感知内容集成的专门数据生成管道；3）处理所有视觉线索的统一控制模块；4）用于精确局部编辑（包括对象移除）的微调空间分支。

Result: 大量实验验证了这种分层方法有效解决了用户意图差距，赋予创作者对生成过程的直接、直观控制。

Conclusion: MagicQuillV2通过分层组合范式弥合了扩散模型的语义能力与传统图形软件精细控制之间的差距，实现了精确交互式图像编辑。
...
中文摘要: 我们提出了MagicQuill V2，这是一个新颖的系统，将分层组合范式引入生成式图像编辑，弥合了扩散模型的语义能力与传统图形软件精细控制之间的差距。虽然扩散变换器在整体生成方面表现出色，但它们使用单一整体提示无法区分用户对内容、位置和外观的不同意图。为了克服这一问题，我们的方法将创意意图解构为一系列可控视觉线索：内容层（创建什么）、空间层（放置在哪里）、结构层（如何塑造）和颜色层（调色板）。我们的技术贡献包括用于上下文感知内容集成的专门数据生成管道、处理所有视觉线索的统一控制模块，以及用于精确局部编辑（包括对象移除）的微调空间分支。大量实验验证了这种分层方法有效解决了用户意图差距，赋予创作者对生成过程的直接、直观控制。
...
### [95] [On the Difficulty of Token-Level Modeling of Dysfluency and Fluency Shaping Artifacts](https://arxiv.org/abs/2512.02027)
*Kashaf Gulzar,Dominik Wagner,Sebastian P. Bayerl,Florian Hönig,Tobias Bocklet,Korbinian Riedhammer*

Main category: eess.AS

TL;DR: 该论文研究了口吃语音识别中不流利性和流利性塑造伪影的标记级建模难度，提出了一种参数高效的适应方法，通过特殊标记解码这些特征，并在英德语数据集上验证了效果，同时揭示了多语言系统中的英语中心偏见问题。


<details>
  <summary>Details</summary>
Motivation: 当前端到端ASR系统在口吃语音转录中存在不足，经常忽略不流利性和流利性塑造伪影，导致转录不逐字，限制了临床和研究应用价值。需要开发能够准确识别和转录这些特征的系统。

Method: 提出参数高效的适应方法，将不流利性和流利性修改解码为特殊标记；采用多步微调策略，包含语言自适应预训练；在模拟（LibriStutter，英语）和自然（KSoF，德语）口吃语音数据集上进行评估；进行标记化分析以识别系统偏见。

Result: 轻量级适应技术对不流利性感知的ASR有效；标记化分析揭示了标记器的英语中心偏见，这对德语数据性能提升构成挑战；多语言E2E系统存在关键局限性，特别是在处理不同语言的口吃语音时。

Conclusion: 该研究证明了参数高效适应方法在口吃语音识别中的可行性，但同时也暴露了多语言端到端ASR系统中存在的英语中心偏见问题，这限制了系统在非英语语言上的性能表现，为未来改进提供了方向。
...
中文标题: 论口吃不流利性和流利性塑造伪影的标记级建模难度

中文摘要: 口吃语音的自动转录仍然是一个挑战，即使对于现代的端到端（E2E）自动语音识别（ASR）框架也是如此。不流利性和流利性塑造伪影经常被忽视，导致转录不逐字，临床和研究价值有限。我们提出了一种参数高效的适应方法，将不流利性和流利性修改解码为转录中的特殊标记，并在模拟（LibriStutter，英语）和自然（KSoF，德语）口吃语音数据集上进行了评估。为了缓解ASR性能差异和偏向英语的问题，我们引入了具有语言自适应预训练的多步微调策略。标记化分析进一步突出了标记器的英语中心偏见，这对提高德语数据性能构成了挑战。我们的研究结果表明，轻量级适应技术对于不流利性感知的ASR是有效的，同时也暴露了多语言E2E系统中的关键局限性。

</details>


### [96] [Towards Language-Independent Face-Voice Association with Multimodal Foundation Models](https://arxiv.org/abs/2512.02759)
*Aref Farhadipour,Teodora Vukovic,Volker Dellwo*

Main category: eess.AS

TL;DR: UZH-CL团队使用两种架构参加FAME2026多语言人脸-语音关联挑战：基线双编码器系统和基于ImageBind-LoRA的基础模型方法。后者仅在阿拉伯语数据上微调，却在英语和德语测试集上取得24.73% EER，获得比赛第二名，展示了强大的跨语言泛化能力。


<details>
  <summary>Details</summary>
Motivation: FAME2026挑战赛专注于多语言条件下的跨模态验证，特别是处理未见和未听过的语言。现有方法面临数据稀缺和语言限制的挑战，需要开发能够跨语言泛化的人脸-语音关联系统。

Method: 1. 基线双编码器系统：从头训练，使用对比损失和正交投影损失
2. 基础模型方法：基于ImageBind多模态基础模型，采用LoRA（低秩适应）进行微调
3. 数据增强：从VoxBlink整理外部阿拉伯语数据集以解决数据稀缺问题

Result: ImageBind-LoRA系统表现最佳：仅在阿拉伯语音频上微调，在英语和德语评估集上达到24.73%等错误率（EER），在FAME2026挑战赛中获得第二名，展示了出色的跨语言泛化能力。

Conclusion: 基于多模态基础模型（如ImageBind）结合LoRA微调的方法在语言无关的人脸-语音关联任务中表现优异，能够有效跨语言泛化，为解决多语言跨模态验证问题提供了有前景的方向。
...
中文标题: 迈向多模态基础模型的语言无关人脸-语音关联

中文摘要: 本文描述了提交给FAME2026挑战赛的UZH-CL系统。该挑战赛专注于独特多语言条件下的跨模态验证，特别是未见和未听过的语言。我们的方法研究了两种不同的架构：一种是从头开始训练的使用对比和正交投影损失的基线双编码器系统，另一种是利用ImageBind与LoRA的基础模型方法。为应对挑战中的数据稀缺和语言限制，我们从VoxBlink整理了一个外部阿拉伯语数据集。我们表现最佳的系统ImageBind-LoRA展示了卓越的跨语言泛化能力：尽管仅在阿拉伯语音频上进行微调，但在评估集（英语和德语）上实现了24.73%的等错误率，在比赛中获得第二名。

</details>


### [97] [Perceptual evaluation of Acoustic Level of Detail in Virtual Acoustic Environments](https://arxiv.org/abs/2512.02891)
*Stefan Fichna,Steven van de Par,Bernhard U. Seeber,Stephan D. Ewert*

Main category: eess.AS

TL;DR: 研究表明，在虚拟声学环境模拟中，大幅减少声学细节层次（ALOD）是可行的，只要适当表示扩散的后期混响，早期反射的数量和准确性对感知质量影响不大。


<details>
  <summary>Details</summary>
Motivation: 虚拟声学环境对听力研究和听力学很重要，但实时应用需要简化房间声学模拟。目前不清楚需要多少声学细节层次（ALOD）才能捕捉所有感知相关效果，因此研究不同ALOD对感知质量的影响。

Method: 研究在三个真实环境（客厅+厨房、酒吧、地铁站）中变化ALOD：1）改变早期反射的镜像源数量；2）排除特定几何细节。使用耳机与假人头测量的双耳房间脉冲响应比较，或使用扬声器进行评估。评估内容包括：脉冲、电贝斯和语音的感知差异，以及合理性、语音清晰度和外部化。

Result: 结果表明，在保持与假人头录音相似的合理性、语音清晰度和外部化的情况下，大幅减少ALOD是可行的。早期反射的数量和准确性对感知质量影响不大，关键是适当表示扩散的后期混响。

Conclusion: 虚拟声学环境模拟可以大幅简化声学细节层次而不显著影响感知质量，这为实时应用提供了优化空间。适当表示扩散的后期混响比精确模拟早期反射更重要。
...
中文摘要: 虚拟声学环境能够创建和模拟现实且生态有效的日常生活场景，这对于听力研究和听力学至关重要。混响室内环境尤为重要。对于实时应用，房间声学模拟需要简化，然而，为了捕捉所有感知相关的效果，必要的声学细节层次（ALOD）仍不明确。本研究考察了在三个真实环境模拟中变化ALOD的影响：带连接厨房的客厅、酒吧和地铁站。通过为早期反射生成不同数量的镜像源，或排除每个环境特定的几何房间细节来变化ALOD。使用耳机与在相应真实环境中用假人头测量的双耳房间脉冲响应进行比较，或使用扬声器，对模拟进行了感知评估。研究评估了脉冲刺激、播放的电贝斯和语音标记的感知总体差异。此外，还评估了合理性、语音清晰度和外部化。结果表明，在保持与假人头录音相似的合理性、语音清晰度和外部化的情况下，大幅减少ALOD是可行的。只要适当表示扩散的后期混响，早期反射的数量和准确性似乎不太重要。
...
### [98] [On the Complexity of Signed Roman Domination](https://arxiv.org/abs/2512.02083)
*Sangam Balchandar Reddy*

Main category: cs.DS

TL;DR: 本文研究了符号罗马支配问题的计算复杂性，证明了其在分裂图上保持NP完全性，在参数化复杂性方面证明了W[2]-难（权重参数）和W[1]-难（反馈顶点集参数），同时给出了邻域多样性和顶点覆盖数参数的FPT算法，并证明了顶点覆盖数参数下不存在多项式核。


<details>
  <summary>Details</summary>
Motivation: 符号罗马支配问题是图论中一个重要的组合优化问题，是经典罗马支配问题的变体。虽然已知该问题在二分图和平面图上是NP完全的，但对其在其他图类上的复杂性以及参数化复杂性了解有限。本文旨在深入研究该问题的计算复杂性，特别是在分裂图上的复杂性，并系统研究其参数化复杂性，为理解该问题的计算难度提供更全面的理论框架。

Method: 本文采用理论计算机科学中的复杂性理论方法，包括NP完全性证明、参数化复杂性分析（W层次结构）、FPT算法设计以及核复杂性分析。通过构造归约证明在分裂图上的NP完全性，使用参数化归约证明W[2]-难和W[1]-难性质，设计基于邻域多样性的动态规划FPT算法，并应用组合方法证明不存在多项式核。

Result: 主要结果包括：1）证明符号罗马支配问题在分裂图上保持NP完全性；2）证明该问题在参数化为权重时是W[2]-难的，即使在二分图上；3）证明在参数化为反馈顶点集数时是W[1]-难的；4）提出了参数化为邻域多样性和顶点覆盖数的FPT算法；5）证明了在顶点覆盖数参数下不存在多项式核，除非coNP ⊆ NP/poly。

Conclusion: 符号罗马支配问题具有丰富的复杂性结构：在经典复杂性方面是NP完全的，在参数化复杂性方面对大多数自然参数都是难的，但对邻域多样性和顶点覆盖数参数存在FPT算法。该问题在顶点覆盖数参数下不存在多项式核的证明表明，即使对于结构参数，该问题也具有内在的复杂性。这些结果为理解符号罗马支配问题的计算难度提供了完整的理论图景。
...
中文标题: 关于符号罗马支配问题的复杂性研究

中文摘要: 给定一个图$G = (V, E)$，符号罗马支配函数是一个函数$f: V \rightarrow \{-1, 1, 2\}$，满足对于每个顶点$u \in V$：$\sum_{v \in N[u]} f(v) \geq 1$，并且对于每个满足$f(u) = -1$的顶点$u \in V$，存在一个顶点$v \in N(u)$满足$f(v) = 2$。符号罗马支配函数$f$的权重为$\sum_{u \in V} f(u)$。符号罗马支配(SRD)问题的目标是计算具有最小权重的符号罗马支配函数。已知该问题即使在二分图和平面图上也是NP完全的。本文通过证明该问题在分裂图上仍然保持NP完全性来推进复杂性研究。在参数化复杂性领域，我们证明了该问题在参数化为权重时是W[2]-难的，即使在二分图上也是如此。我们进一步证明该问题在参数化为反馈顶点集数时是W[1]-难的（因此在参数化为树宽或团宽时也是如此）。在积极方面，我们提出了一个参数化为邻域多样性（以及顶点覆盖数）的FPT算法。最后，我们通过证明除非coNP $\subseteq$ NP/poly，否则该问题在参数化为顶点覆盖数时不允许多项式核来补充这一结果。

</details>


### [99] [Markov Chains Approximate Message Passing](https://arxiv.org/abs/2512.02384)
*Amit Rajaraman,David X. Wu*

Main category: cs.DS

TL;DR: 本文建立了马尔可夫链蒙特卡洛算法（特别是Glauber动力学）与近似消息传递（AMP）在贝叶斯推断问题中的理论联系，证明了在尖峰Wigner模型中，通过受限高斯动力学（RGD）分析，马尔可夫链方法能够达到与AMP相同的贝叶斯最优性能。


<details>
  <summary>Details</summary>
Motivation: 马尔可夫链蒙特卡洛算法在实际应用中经常表现出接近最优的性能，但缺乏严谨的理论支持。本文旨在填补这一理论空白，特别是在经典的贝叶斯推断问题中，建立马尔可夫链方法与已知最优算法（AMP）之间的理论联系。

Method: 通过分析一个称为受限高斯动力学（RGD）的辅助马尔可夫链，将RGD简化为与AMP迭代演化相似的一维递归，并研究其在相关空间中的收敛行为。特别关注从热启动开始的快速收敛特性。

Result: 1. 成功将RGD简化为有效的一维递归，该递归精确反映了AMP的迭代演化
2. 证明了从热启动开始，RGD能够快速收敛到相关空间的固定点，该固定点能够恢复贝叶斯最优性能
3. 在SK模型的混合假设下，恢复了非平凡推断的相变

Conclusion: 本文为马尔可夫链蒙特卡洛算法在贝叶斯推断中的优异性能提供了理论支持，建立了Glauber动力学与近似消息传递之间的严格联系，证明了在尖峰Wigner模型中，马尔可夫链方法能够达到贝叶斯最优性能，为理解这些算法的理论基础迈出了重要一步。
...
中文摘要: 马尔可夫链蒙特卡洛算法长期以来在各种贝叶斯推断设置中被观察到能够获得接近最优的性能。然而，建立一个支持这些研究的严谨理论一直具有挑战性。

在本文中，我们研究了经典的尖峰Wigner推断问题，即从噪声矩阵测量中恢复一个植入的布尔尖峰。我们将退火后验上的Glauber动力学的恢复性能与近似消息传递（AMP）的性能联系起来，已知AMP能够实现贝叶斯最优性能。我们的主要结果依赖于对一个称为受限高斯动力学（RGD）的辅助马尔可夫链的分析。具体来说，我们建立了以下结果：

1. RGD可以简化为一个有效的一维递归，该递归反映了AMP迭代的演化。
2. 从热启动开始，RGD在相关空间中快速收敛到一个固定点，当在后验上运行时，该固定点能够恢复贝叶斯最优性能。
3. 基于对SK模型广泛相信的混合结果，我们恢复了非平凡推断的相变。

</details>


### [100] [New Bounds for Circular Trace Reconstruction](https://arxiv.org/abs/2512.02412)
*Arnav Burudgunte,Paul Valiant,Hongao Wang*

Main category: cs.DS

TL;DR: 本文改进了圆形轨迹重建问题的下界至$\tildeΩ(n^5)$，并证明对于恒定稀疏字符串，$\tilde{O}(n^6)$个轨迹就足够重建，表明未来研究应关注非恒定稀疏字符串。


<details>
  <summary>Details</summary>
Motivation: 轨迹重建问题在理论上存在指数级的上界-下界差距，圆形轨迹重建变体是为了探索缩小这一差距的新方法而引入的。研究圆形轨迹重建有助于理解随机循环移位对重建难度的影响。

Method: 使用傅里叶分析技术，通过分析两个稀疏字符串在圆形删除通道下轨迹的不可区分性来建立下界，同时为恒定稀疏字符串设计重建算法。

Result: 1. 将圆形轨迹重建的下界从$\tildeΩ(n^3)$改进到$\tildeΩ(n^5)$；2. 证明恒定稀疏字符串在圆形删除通道中只需$\tilde{O}(n^6)$个轨迹即可重建。

Conclusion: 圆形轨迹重建中，恒定稀疏字符串的重建复杂度远低于一般字符串，未来研究需要关注非恒定稀疏字符串才能取得实质性进展。
...
中文摘要: "轨迹重建"问题询问：给定一个未知的二进制字符串$x$和一个通道，该通道重复返回$x$的"轨迹"，其中每个比特以概率$p$被随机删除，需要多少轨迹才能恢复$x$？该问题的最佳已知上界和下界之间存在指数级差距。为了激发或揭示缩小这一差距的新方法，已经引入了该模型的许多变体。我们研究了Narayanan和Ren（ITCS 2021）引入的圆形轨迹重建变体，其中轨迹除了随机删除外还经历随机循环移位。

我们展示了圆形轨迹重建的改进下界$\tildeΩ(n^5)$。这与先前已知的圆形情况下$\tildeΩ(n^3)$和线性情况下$\tildeΩ(n^{3/2})$的最佳下界形成对比。我们的边界显示了来自两个稀疏字符串$x,y$的轨迹的不可区分性，这两个字符串每个都有恒定数量的非零位。这种技术能否显著扩展？在循环删除通道下重建稀疏字符串$x$有多难？我们通过使用傅里叶技术解决了这些问题，表明$\tilde{O}(n^6)$个轨迹足以在圆形删除通道中重建任何恒定稀疏字符串，这与圆形删除通道中一般字符串的$\exp(\tilde{O}(n^{1/3}))$上界形成对比。这表明新算法或新下界必须专注于非恒定稀疏字符串。

</details>


### [101] [Approximation schemes for covering and packing mixed-integer programs with a fixed number of constraints](https://arxiv.org/abs/2512.02571)
*Kobe Grobben,Phablo F. S. Moura,Hande Yaman*

Main category: cs.DS

TL;DR: 本文针对具有固定约束数量的覆盖型混合整数线性规划问题，提出了多项式时间近似方案。通过分析多面体顶点性质，将问题分解为多维背包覆盖问题，并设计了多种近似算法。


<details>
  <summary>Details</summary>
Motivation: 覆盖型混合整数线性规划问题在经典优化问题中广泛存在，如多维背包、设施选址和供应商选择问题。现有算法对这些问题的近似性能有限，特别是对于具有固定约束数量的情况，缺乏有效的近似方案。本文旨在填补这一空白，为这类问题提供首个多项式时间近似方案。

Method: 首先分析相关多面体的顶点性质，利用这些性质将原问题分解为每个维度只有一个连续变量的多维背包覆盖问题。基于这种分解，为具有固定约束数量的问题设计了多项式时间近似方案。对于单约束情况，设计了完全多项式时间近似方案和近似线性规划公式。对于所有变量具有相同上下界的情况，展示了完美的紧凑公式。

Result: 1. 为具有固定约束数量的覆盖型混合整数线性规划问题提出了首个多项式时间近似方案；2. 对于单约束情况，设计了完全多项式时间近似方案和近似线性规划公式，改进了先前已知的2-近似算法；3. 对于所有变量具有相同上下界的情况，展示了完美的紧凑公式；4. 类似结果也适用于问题的包装和分配变体。

Conclusion: 本文为覆盖型混合整数线性规划问题提供了系统的近似算法框架，特别是对于具有固定约束数量的情况。提出的分解方法和近似方案不仅理论上有重要意义，也为实际应用中的经典优化问题提供了更高效的求解方法。这些结果扩展了混合整数规划近似算法的理论边界。
...
中文摘要: 本文对一类覆盖型混合整数线性规划问题进行了算法研究，这类问题涵盖了经典覆盖问题，包括多维背包、设施选址和供应商选择问题。我们首先展示了相关多面体顶点的一些性质，然后利用这些性质将问题分解为每个维度只有一个连续变量的多维背包覆盖问题实例。所提出的分解方法用于为具有固定约束数量的问题设计多项式时间近似方案。据我们所知，这是针对如此广泛的覆盖型混合整数规划问题的首个近似方案。此外，我们为单约束情况设计了完全多项式时间近似方案和近似线性规划公式。这些结果改进了先前已知的具有单个连续变量的背包覆盖问题的2-近似算法。最后，我们展示了当所有变量具有相同上下界时的完美紧凑公式。类似的结果也适用于问题的包装和分配变体。

</details>


### [102] [The Support of Bin Packing is Exponential](https://arxiv.org/abs/2512.02758)
*Klaus Jansen,Lis Pirotton,Malte Tutas*

Main category: cs.DS

TL;DR: 该论文证明了装箱问题解的支撑集（不同填充方式的箱子数量）下界为指数级2^{Ω(d)}，与已知上界2^d匹配，并开发了新的整数线性规划聚合技术。


<details>
  <summary>Details</summary>
Motivation: 研究装箱问题解的支撑集规模对于理解算法复杂度至关重要。先前工作给出了支撑集上界为2^d，但下界问题尚未解决。确定支撑集的确切规模有助于分析多种装箱算法的时间复杂度，并为解决更一般的整数线性规划问题提供新思路。

Method: 开发了一种创新的整数线性规划聚合技术，能够将具有多个等式约束的整数线性规划问题聚合为仅含单个约束的等价问题。该技术的关键创新在于成功地将变量的上界整合到聚合后的约束中，这与现有聚合方法形成鲜明对比。

Result: 证明了装箱问题支撑集的下界为2^{Ω(d)}，与已知上界2^d在常数因子内匹配。这一结果直接影响了多个装箱算法的时间复杂度分析，并为解决更一般的整数规划问题提供了新工具。

Conclusion: 装箱问题的支撑集规模呈指数级增长，这一发现对算法复杂度分析有重要意义。新开发的整数线性规划聚合技术不仅解决了支撑集下界问题，还可能应用于更广泛的整数规划问题，如d维背包问题，为相关领域研究提供了新方向。
...
中文摘要: 考虑具有d种不同物品尺寸s_i和物品数量a_i的经典装箱问题。装箱问题解的支撑集是指不同填充方式的箱子数量。在这项工作中，我们证明了该问题支撑集的下界为2^{Ω(d)}。我们的下界与Eisenbrand和Shmonin [Oper.Research Letters '06]给出的上界2^d在常数因子内匹配。这一结果对多种装箱算法的时间复杂度有直接影响，如Goemans和Rothvoss [SODA '14]、Jansen和Klein [SODA '17]以及Jansen和Solis-Oba [IPCO '10]。为实现我们的主要结果，我们开发了一种技术，将具有多个约束的等式约束整数线性规划聚合为具有单个约束的等价整数线性规划。我们的技术与现有聚合技术形成对比，因为我们成功地将变量的上界整合到结果约束中。我们相信这项技术对于解决一般整数线性规划或d维背包问题可能有用。
...
### [103] [Story2MIDI: Emotionally Aligned Music Generation from Text](https://arxiv.org/abs/2512.02192)
*Mohammad Shokri,Alexandra C. Salem,Gabriel Levine,Johanna Devaney,Sarah Ita Levitan*

Main category: cs.SD

TL;DR: Story2MIDI是一个基于Transformer的序列到序列模型，能够从文本生成情感对齐的音乐。通过合并文本情感分析和音乐情感分类数据集构建Story2MIDI数据集，模型学习音乐中的情感相关特征并生成具有多样情感响应的音乐样本。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏能够从文本生成情感对齐音乐的系统。文本和音乐都能传达情感，但现有方法难以确保生成音乐的情感与输入文本的情感保持一致。本研究旨在开发一个能够理解文本情感并生成相应情感音乐的系统。

Method: 1. 构建Story2MIDI数据集：合并文本情感分析和音乐情感分类的现有数据集，创建文本片段与音乐片段的配对，确保两者唤起相同情感。2. 开发基于Transformer的序列到序列模型，学习文本到音乐的映射关系。3. 模型学习音乐中的情感相关特征并将其融入生成过程。

Result: 尽管数据集规模较小且计算资源有限，模型仍能有效学习音乐中的情感相关特征。生成样本展现出多样化的情感响应。通过客观音乐指标和人类听感研究评估，确认模型能够捕捉预期的情感线索。

Conclusion: Story2MIDI模型能够从文本生成情感对齐的音乐，成功学习并整合情感相关特征到生成过程中。即使在有限的数据和计算资源下，模型仍表现出良好的性能，为文本到音乐的跨模态情感生成提供了有效解决方案。
...
中文摘要: 本文介绍了Story2MIDI，一个基于Transformer的序列到序列模型，用于从给定文本生成情感对齐的音乐。为开发此模型，我们通过合并文本情感分析和音乐情感分类的现有数据集构建了Story2MIDI数据集。该数据集包含文本片段和音乐片段的配对，这些配对能在读者或听众中唤起相同的情感。尽管我们的数据集规模较小且计算资源有限，但结果表明，我们的模型有效地学习了音乐中的情感相关特征，并将其融入生成过程，产生了具有多样化情感响应的样本。我们使用客观音乐指标和人类听感研究评估生成输出，确认了模型捕捉预期情感线索的能力。

</details>


### [104] [Continual Learning for Singing Voice Separation with Human in the Loop Adaptation](https://arxiv.org/abs/2512.02432)
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度学习歌唱人声分离方法虽然表现优异，但缺乏用户交互机制。在实际部署中，音乐曲目可能在流派和乐器方面与原始训练数据存在差异，因此需要允许用户与模型交互以提高性能。

Method: 采用基于U-Net的基础模型架构，从频谱图中生成分离人声的掩码。然后进行人机交互任务，用户通过标记提取人声中的误报区域（应为静音的区域）提供反馈。提出了两种持续学习算法来整合用户反馈。

Result: 实验证明，在数据集内和跨数据集设置中，所提出的算法相比基础模型在歌唱人声分离性能上都有显著提升。

Conclusion: 本文提出的交互式持续学习框架能够有效提升歌唱人声分离性能，通过用户反馈使模型适应新的目标歌曲，在实际部署中具有重要价值。
...
中文摘要: 基于深度学习的歌唱人声分离方法在最近表现非常出色。然而，大多数这些工作并未关注允许用户与模型交互以提高性能。这在将模型部署到现实场景中时可能至关重要，因为音乐曲目在流派和乐器方面可能与原始训练数据存在差异。在本文中，我们提出了一种基于深度学习的交互式持续学习框架，用于歌唱人声分离，允许用户微调人声分离模型以适应新的目标歌曲。我们使用基于U-Net的基础模型架构，从频谱图中生成分离人声的掩码，然后进行人机交互任务，用户通过标记一些误报（即提取人声中应为静音的区域）来提供反馈。我们提出了两种持续学习算法。实验证实了所提出算法在数据集内和跨数据集设置中相比基础模型在歌唱人声分离性能上的改进。

</details>


### [105] [Generative Multi-modal Feedback for Singing Voice Synthesis Evaluation](https://arxiv.org/abs/2512.02523)
*Xueyan Li,Yuxin Wang,Mengjie Jiang,Qingzi Zhu,Jiang Zhang,Zoey Kim,Yazhe Niu*

Main category: cs.SD

TL;DR: 提出生成式多模态反馈框架，为歌声合成评估提供语言和音频反馈，替代传统单维评分方法


<details>
  <summary>Details</summary>
Motivation: 当前歌声合成评估方法依赖单一数值评分，难以捕捉音调、表达等多维度特征，且需要昂贵的人工标注，限制了可解释性和泛化能力

Method: 使用音频-语言模型生成文本和音频反馈，涵盖旋律、内容、听觉质量等方面；在混合数据集上微调，结合人类音乐反应和MLLMs生成的合成评论

Result: 定量实验验证了数据集和训练策略的有效性，框架能产生音乐准确且可解释的评估，适用于指导生成模型改进

Conclusion: 提出的生成式多模态反馈框架为歌声合成评估提供了更全面、可解释的解决方案，有助于提升生成模型质量
...
中文标题: 歌声合成评估的生成式多模态反馈

中文摘要: 歌声合成技术已取得显著进展，使模型能够生成具有准确音调和一致风格的歌声。随着这些能力的提升，可靠的评估和优化需求变得日益重要。然而，当前方法如奖励系统通常依赖单一数值评分，难以捕捉音调、表达等多维度特征，且需要昂贵的人工标注，限制了可解释性和泛化能力。为解决这些问题，我们提出了一个生成式反馈（即奖励模型）框架，为歌声合成评估提供多维度的语言和音频反馈。我们的方法利用音频-语言模型生成文本和音频评论，涵盖旋律、内容和听觉质量等方面。该模型在混合数据集上微调，结合人类音乐反应和MLLMs生成的合成评论，增强了多样性和语言丰富性。定量实验验证了所提数据集和训练策略的有效性，表明该框架能产生音乐准确且可解释的评估，适用于指导生成模型改进。代码位于 https://github.com/opendilab/VocalCritic

</details>


### [106] [Pianist Transformer: Towards Expressive Piano Performance Rendering via Scalable Self-Supervised Pre-Training](https://arxiv.org/abs/2512.02652)
*Hong-Jie You,Jie-Jing Shao,Xiao-Wen Yang,Lin-Han Jia,Lan-Zhe Guo,Yu-Feng Li*

Main category: cs.SD

TL;DR: Pianist Transformer 是一个通过自监督预训练实现可扩展钢琴表现力演奏渲染的模型，使用统一MIDI表示、非对称架构、大规模预训练，在客观指标和主观评分上达到先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有表达性音乐演奏渲染方法依赖小规模标注数据集的有监督学习，限制了数据和模型规模的扩展，尽管存在大量未标注音乐数据。为了解决这一问题，研究者希望利用自监督预训练的优势，实现数据和模型规模的可扩展性。

Method: 1) 提出统一的MIDI数据表示，用于学习音乐结构和表达的共同原理而无需显式标注；2) 设计高效的非对称架构，支持更长上下文和更快推理而不牺牲渲染质量；3) 构建包含100亿token和1.35亿参数的自监督预训练流程；4) 实现最先进的性能模型。

Result: 该模型在客观指标和人类主观评分方面都达到了最先进水平，实现了数据和模型规模扩展的优势，为表达性演奏渲染建立了可扩展路径。

Conclusion: Pianist Transformer 为音乐领域实现类人演奏合成建立了可扩展路径，通过自监督预训练解锁了数据和模型规模扩展的优势，在表达性钢琴演奏渲染方面取得了突破。
...
中文标题: 钢琴家Transformer：通过可扩展自监督预训练实现表达性钢琴演奏渲染

中文摘要: 现有的表达性音乐演奏渲染方法依赖于小规模标注数据集的有监督学习，这限制了数据和模型规模的扩展，尽管存在大量未标注音乐数据，就像视觉和语言领域一样。为了解决这一差距，我们引入了Pianist Transformer，具有四个关键贡献：1) 统一的MIDI数据表示，用于学习音乐结构和表达的共同原理而无需显式标注；2) 高效的非对称架构，支持更长上下文和更快推理而不牺牲渲染质量；3) 包含100亿token和1.35亿参数的自监督预训练流程，为表达性演奏渲染解锁了数据和模型规模扩展的优势；4) 最先进的性能模型，在客观指标和人类主观评分方面都达到了先进水平。总体而言，Pianist Transformer为音乐领域实现类人演奏合成建立了可扩展路径。

</details>


### [107] [SAND Challenge: Four Approaches for Dysartria Severity Classification](https://arxiv.org/abs/2512.02669)
*Gauri Deshpande,Harish Battula,Ashish Panda,Sunil Kumar Kopparapu*

Main category: cs.SD

TL;DR: 该论文在SAND挑战中比较了四种构音障碍严重程度分类方法：基于视觉Transformer的频谱图图像方法、1D-CNN多数投票融合方法、BiLSTM多数投票融合方法，以及结合声门和共振峰特征的两阶段XGBoost集成方法。


<details>
  <summary>Details</summary>
Motivation: 构音障碍严重程度分类对于神经退行性疾病评估至关重要。SAND挑战旨在探索不同建模方法在相同数据集上的性能，比较传统特征工程与深度学习方法的优劣，为临床诊断提供技术支持。

Method: 1. ViT-OF：将语音频谱图作为图像输入视觉Transformer模型；2. 1D-CNN：使用八个1维CNN模型进行多数投票融合；3. BiLSTM-OF：使用九个双向LSTM模型进行多数投票融合；4. 分层XGBoost集成：通过两阶段学习框架结合声门和共振峰特征。

Result: 在53名说话者的验证集上，特征工程的XGBoost集成方法获得最高的宏F1分数（0.86），而三种深度学习方法（ViT、CNN、BiLSTM）获得相近的F1分数（约0.70），为问题提供了互补的见解。

Conclusion: 特征工程方法在构音障碍严重程度分类任务中表现最佳，但深度学习方法也展现出竞争力。不同方法提供了不同的视角，表明结合多种方法可能获得更好的分类性能。
...
中文标题: SAND挑战：构音障碍严重程度分类的四种方法

中文摘要: 本文在神经退行性疾病语音分析（SAND）挑战中，对四种不同的构音障碍严重程度分类建模方法进行了统一研究。所有模型使用相同的语音录音数据集处理相同的五分类任务。我们研究了：（1）基于频谱图图像的视觉Transformer方法（ViT-OF）；（2）使用八个1维CNN进行多数投票融合的方法；（3）使用九个双向LSTM模型进行多数投票融合的方法（BiLSTM-OF）；（4）通过两阶段学习框架结合声门和共振峰特征的分层XGBoost集成方法。每种方法都有详细描述，并在53名说话者的验证集上比较了它们的性能。结果显示，虽然特征工程的XGBoost集成方法获得了最高的宏F1分数（0.86），但深度学习方法（ViT、CNN、BiLSTM）也获得了有竞争力的F1分数（0.70），并为该问题提供了互补的见解。

</details>


### [108] [Exploring Definitions of Quality and Diversity in Sonic Measurement Spaces](https://arxiv.org/abs/2512.02783)
*Björn Þór Jónsson,Çağrı Erdem,Stefano Fasciani,Kyrre Glette*

Main category: cs.SD

TL;DR: 该研究探索了在声音测量空间中自动定义质量与多样性的方法，通过无监督降维技术（PCA和自编码器）为QD进化算法创建动态可重构的行为空间，相比手工设计的方法能获得更高的多样性并避免专家偏见。


<details>
  <summary>Details</summary>
Motivation: 数字声音合成提供了探索包含数百万配置的广阔参数空间的机会。质量多样性（QD）进化算法为利用这一潜力提供了有前景的方法，但其成功依赖于适当的声音特征表示。现有的QD方法主要使用手工设计的描述符或有监督分类器，可能引入非预期的探索偏见，并将发现限制在熟悉的声音区域。

Method: 本研究调查了无监督降维方法，用于在QD搜索过程中自动定义和动态重构声音行为空间。应用主成分分析（PCA）和自编码器将高维音频特征投影到MAP-Elites的结构化网格上，通过定期重新训练模型实现动态重构。在两个实验场景中进行比较。

Result: 自动方法相比手工设计的行为空间实现了显著更大的多样性，同时避免了专家强加的偏见。动态行为空间重构保持了进化压力并防止停滞，其中PCA在降维技术中证明最为有效。

Conclusion: 这些结果为自动化声音发现系统做出了贡献，使系统能够在无需人工干预或有监督训练约束的情况下探索广阔的参数空间。无监督方法为声音探索提供了更客观、更全面的行为空间定义。
...
中文摘要: 数字声音合成为探索包含数百万配置的广阔参数空间提供了机会。质量多样性（QD）进化算法为利用这一潜力提供了有前景的方法，但其成功依赖于适当的声音特征表示。现有的QD方法主要使用手工设计的描述符或有监督分类器，可能引入非预期的探索偏见，并将发现限制在熟悉的声音区域。本研究调查了无监督降维方法，用于在QD搜索过程中自动定义和动态重构声音行为空间。我们应用主成分分析（PCA）和自编码器将高维音频特征投影到MAP-Elites的结构化网格上，通过定期重新训练模型实现动态重构。在两个实验场景中的比较显示，自动方法相比手工设计的行为空间实现了显著更大的多样性，同时避免了专家强加的偏见。动态行为空间重构保持了进化压力并防止停滞，其中PCA在降维技术中证明最为有效。这些结果为自动化声音发现系统做出了贡献，使系统能够在无需人工干预或有监督训练约束的情况下探索广阔的参数空间。
...
### [109] [DepthScape: Authoring 2.5D Designs via Depth Estimation, Semantic Understanding, and Geometry Extraction](https://arxiv.org/abs/2512.02263)
*Xia Su,Cuong Nguyen,Matheus A. Gadelha,Jon E. Froehlich*

Main category: cs.HC

TL;DR: DepthScape是一个人类-AI协作系统，通过深度估计、语义理解和几何提取，简化2.5D设计创作，让用户能够直接在3D重建中放置设计元素，自动实现逼真的遮挡和透视缩短效果。


<details>
  <summary>Details</summary>
Motivation: 2.5D效果（如遮挡和透视缩短）能增强视觉动态和真实感，但创作这些效果仍然具有挑战性且劳动密集，主要原因是深度感知的复杂性。现有工具需要用户手动处理深度信息，过程繁琐耗时。

Method: DepthScape采用三步方法：1) 使用单目深度重建将图像转换为3D重建；2) 利用视觉语言模型分析源图像，提取关键视觉组件作为内容锚点；3) 通过2D视口简化3D放置，支持直接操作编辑。系统实现了人类-AI协作的工作流程。

Result: 系统评估显示：1) 9名不同设计背景的参与者确认了创作流程的有效性；2) 在100张专业库存图像上测试验证了系统的鲁棒性；3) 专家评估确认了DepthScape生成结果的质量。

Conclusion: DepthScape成功简化了2.5D设计创作过程，通过结合深度估计、语义理解和几何提取技术，实现了直观的人类-AI协作界面，显著降低了创作复杂2.5D效果的技术门槛。
...
中文摘要: 2.5D效果（如遮挡和透视缩短）通过将3D深度线索融入2D设计来增强视觉动态和真实感。然而，由于深度感知的复杂性，创作此类效果仍然具有挑战性且劳动密集。我们介绍了DepthScape，这是一个人类-AI协作系统，通过直接将设计元素放置到3D重建中来促进2.5D效果创作。使用单目深度重建，DepthScape将图像转换为3D重建，其中视觉内容被放置以自动实现逼真的遮挡和透视缩短。为了通过2D视口进一步简化3D放置，DepthScape使用视觉语言模型分析源图像并提取关键视觉组件作为直接操作编辑的内容锚点。我们评估了DepthScape，涉及九名不同设计背景的参与者，确认了我们创作流程的有效性。我们还测试了100张专业库存图像以评估鲁棒性，并进行了专家评估，确认了DepthScape结果的质量。

</details>


### [110] [Understanding Down Syndrome Stereotypes in LLM-Based Personas](https://arxiv.org/abs/2512.02275)
*Chantelle Wu,Peinan Wang,Nafi Nibras,Meida Li,Dajun Yuan,Zhixiao Wang,Jiahuan He,Mona Ali,Mirjana Prpa*

Main category: cs.HC

TL;DR: 本研究探讨了基于LLM的人物角色系统如何产生唐氏综合征刻板印象，通过访谈发现刻板印象源于训练数据、界面设计和LLM输出语气，强调需要参与式方法来捕捉真实多样性。


<details>
  <summary>Details</summary>
Motivation: 现有的人物角色创建方法常常导致对唐氏综合征患者的过度简化或刻板印象化描述，这促使研究者开发具有刻板印象检测功能的系统，以更好地理解和解决这一问题。

Method: 研究者开发了Persona-L系统，结合LLM和RAG技术建模唐氏综合征患者角色，并构建刻板印象检测功能。通过访谈10名护理人员和医疗专业人员，分析刻板印象在LLM内容和表达方式以及界面设计中的表现。

Result: 研究发现刻板印象定义存在挑战，刻板印象可能从训练数据、界面设计和LLM输出语气中涌现。访谈揭示了这些系统如何无意中强化或产生新的刻板印象。

Conclusion: 需要采用参与式方法来捕捉唐氏综合征患者生活经验的异质性，避免过度简化或刻板印象化的人物角色建模，确保技术系统能够更准确地反映真实多样性。
...
中文标题: 理解基于大语言模型的人物角色中的唐氏综合征刻板印象

中文摘要: 我们提出了Persona-L系统的案例研究，该系统利用大语言模型（LLMs）和检索增强生成（RAG）来建模唐氏综合征患者的人物角色。现有的人物角色创建方法常常导致对唐氏综合征患者的过度简化或刻板印象化描述。为此，我们在Persona-L中构建了刻板印象检测功能。通过对护理人员和医疗专业人员（N=10）的访谈，我们研究了唐氏综合征刻板印象如何在大语言模型的内容和表达方式以及界面设计中显现。我们的研究结果显示刻板印象定义面临的挑战，并揭示了从训练数据、界面设计到LLM输出语气中潜在的刻板印象涌现。这突显了需要采用参与式方法来捕捉唐氏综合征患者生活经验的异质性。

</details>


### [111] [Artographer: a Curatorial Interface for Art Space Exploration](https://arxiv.org/abs/2512.02288)
*Shm Garanganao Almeda,John Joon Young Chung,Bjoern Hartmann,Sophia Liu,Brett Halperin,Yuwen Lu,Max Kreminski*

Main category: cs.HC

TL;DR: Artographer是一个基于嵌入模型的艺术探索系统，通过可缩放的2D地图展示15,000+历史艺术品的相似性聚类，支持用户发现和关联艺术作品。研究通过20名参与者（包括9名艺术史学者）的使用分析，识别了探索行为和空间艺术发现的价值。


<details>
  <summary>Details</summary>
Motivation: 现有AI界面往往掩盖艺术作品之间的关系，而不是帮助用户探索这些关系。嵌入模型为通过空间交互发现和关联艺术品提供了新机会，需要设计能够支持艺术探索关系的系统。

Method: 构建Artographer系统，使用15,000+历史艺术品的嵌入模型创建相似性聚类的可缩放2D地图。通过20名参与者（包括9名艺术史学者）进行用户研究，分析他们在目标驱动任务和自由探索中的地图遍历行为。

Result: 观察到发散和收敛的探索行为（跳跃、漫游、固定、重访），并识别了空间艺术发现的价值（可见性、自主性、偶然性、摩擦）。将空间地图置于策展界面系统中，讨论在艺术策展AI设计中强调多元主义和自主性。

Conclusion: 空间艺术探索界面能够支持艺术关系的发现和探索。通过Artographer的研究，提出了策展界面的设计原则，强调在艺术策展AI系统中应重视多元主义和用户自主性，以创建更负责任的AI系统。
...
中文摘要: 将一件作品与先前确立的作品联系起来对于创作和参与艺术至关重要，但AI界面往往掩盖了这种关系，而不是帮助用户探索它们。嵌入模型为通过空间交互发现和关联艺术品提供了新机会。我们构建了Artographer，这是一个艺术探索系统，具有可缩放的2D地图，由15,000+历史艺术品的相似性聚类嵌入构建而成。使用Artographer作为探究空间艺术探索的工具，我们分析了20名参与者（包括9名艺术史学者）在目标驱动任务和自由探索期间如何遍历地图。我们观察到发散和收敛的探索行为（跳跃、漫游、固定、重访），并识别了空间艺术发现的价值（可见性、自主性、偶然性、摩擦）。我们将空间地图置于策展界面的空间中，这些系统选择和呈现艺术品，并讨论在艺术策展的AI系统设计中以多元主义和自主性为中心，以创建更负责任的系统。

</details>


### [112] [A Visual Analytics System to Understand Behaviors of Multi Agents in Reinforcement Learning](https://arxiv.org/abs/2512.02442)
*Changhee Lee,Jeongmin Rhee,DongHwa Shin*

Main category: cs.HC

TL;DR: MARLViz是一个可视化分析系统，用于理解和分析多智能体强化学习环境中智能体的策略与交互行为，帮助用户理解复杂交互模式。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习（MARL）涉及多个智能体在相同环境中同时交互学习，分析这些复杂交互具有挑战性，现有分析方法难以充分反映和解释这种复杂性。

Method: 开发了MARLViz可视化分析系统，通过视觉化展示不同环境设置下智能体的行为差异，分析具有相似行为的智能体并选择场景来理解智能体交互。

Result: 系统能够可视化展示智能体在不同环境设置下的行为差异，帮助用户理解复杂交互模式，使MARL中智能体策略更容易理解。

Conclusion: MARLViz系统有效解决了MARL中智能体行为分析和交互理解的问题，为多智能体强化学习的可视化分析提供了实用工具。
...
中文摘要: 多智能体强化学习（MARL）是机器学习的一个分支，智能体通过试错交互学习最优策略，解决多个智能体在同一环境中同时交互学习的复杂场景。分析和理解这些复杂交互具有挑战性，现有分析方法在充分反映和解释这种复杂性方面存在局限。为应对这些挑战，我们提供了MARLViz，一个用于可视化分析MARL环境中智能体策略和交互的可视化分析系统。该系统旨在视觉化展示不同环境设置下智能体的行为差异，帮助用户理解复杂交互模式。在本研究中，我们分析了具有相似行为的智能体并选择场景来理解智能体交互，这使得理解MARL中智能体策略变得更加容易。

</details>


### [113] [Reframing Human-Robot Interaction Through Extended Reality: Unlocking Safer, Smarter, and More Empathic Interactions with Virtual Robots and Foundation Models](https://arxiv.org/abs/2512.02569)
*Yuchong Zhang,Yong Ma,Danica Kragic*

Main category: cs.HC

TL;DR: 本文提出通过扩展现实（XR）重构人机交互，利用大型基础模型驱动的虚拟机器人作为认知基础、共情的智能体，突破物理硬件限制，实现更安全、智能、共情的交互。


<details>
  <summary>Details</summary>
Motivation: 传统物理机器人受硬件限制，难以实现灵活、可扩展且具有深度认知共情能力的交互。本文旨在探索通过XR技术和大型基础模型结合，创建虚拟机器人作为认知和共情中介，重塑人机交互范式。

Method: 整合XR、人机交互和认知AI领域研究，提出利用多模态大型基础模型（如大语言模型、大视觉模型、视觉语言模型）赋能虚拟机器人，实现情境感知推理、情感敏感交互和长期适应能力。

Result: 虚拟机器人能够支持安全关键场景、跨领域的社会认知共情交互，并通过XR与AI集成扩展物理能力。同时识别了过度信任、文化偏见、隐私风险等挑战。

Conclusion: 提出以人为中心、伦理基础的XR智能体研究议程，强调多层评估框架、多用户生态系统、混合虚拟-物理具身化，以及社会伦理设计实践，将XR虚拟智能体重塑为更高效、自适应的人机交互范式。
...
中文标题: 通过扩展现实重构人机交互：利用虚拟机器人和基础模型解锁更安全、智能、共情的交互

中文摘要: 本文从扩展现实（XR）的角度重构人机交互（HRI），论证了由大型基础模型（FMs）驱动的虚拟机器人可以作为具有认知基础和共情能力的智能体。与物理机器人不同，XR原生智能体不受硬件限制，可以按需实例化、适应和扩展，同时仍能提供具身化和共在感。我们综合了XR、HRI和认知AI领域的研究，展示了此类智能体如何支持安全关键场景、跨领域的社会认知共情交互，以及通过XR与AI集成扩展物理能力。然后，我们讨论了多模态大型基础模型（如大语言模型、大视觉模型和视觉语言模型）如何实现情境感知推理、情感敏感情境和长期适应，将虚拟机器人定位为认知和共情中介而非仅仅是仿真资产。同时，我们强调了挑战和潜在风险，包括过度信任、文化和表征偏见、生物识别传感的隐私问题，以及数据治理和透明度。本文最后概述了以人为中心、伦理基础的XR智能体研究议程——强调多层评估框架、多用户生态系统、混合虚拟-物理具身化，以及社会伦理设计实践，展望由基础模型驱动的XR虚拟智能体将未来HRI重塑为更高效、自适应的范式。

</details>


### [114] [Investigating the Integrated Digital Interventions Delivered by a Therapeutic Companion Agent for Young Adults with Symptoms of Depression: A Proof-of-Concept Study](https://arxiv.org/abs/2512.02608)
*Youngjae Yoo,Minuk Kim,Soyoung Kim,Gayeon Lee,Jinwoo Kim*

Main category: cs.HC

TL;DR: 本研究开发了一个基于治疗伴侣智能体（TCA）的数字干预原型，整合了生态瞬时评估、生态瞬时干预、行为激活和游戏化，对58名有抑郁症状的年轻成年人进行了6周概念验证研究。结果显示干预组在第6周抑郁症状显著减少，生活质量显著改善，依从性水平与现有数字干预相当。


<details>
  <summary>Details</summary>
Motivation: 尽管数字干预对年轻成年人抑郁症具有临床有效性，但低参与度和依从性仍然是主要障碍。建立强大的数字治疗联盟被认为是解决这些挑战的关键途径。本研究旨在探索通过对话式治疗伴侣智能体（TCA）来增强用户参与和依从性的创新方法。

Method: 研究采用概念验证设计，开发了基于Wizard-of-Oz的TCA中心原型，整合了社会支持的生态瞬时评估（EMA）、生态瞬时干预（EMI）、行为激活和游戏化元素。招募了58名20-39岁韩国年轻成年人（PHQ-9显示轻度至中度抑郁症状），随机分为干预组（n=29）和对照组（n=29）。干预组接受为期6周的TCA数字干预，包括每日行为激活任务、情绪评估、冥想等。评估工具包括BDI-II、GAD-7和Q-LES-Q-SF，分别在基线、第2、4、6周进行测量。

Result: 57名参与者完成研究（干预组1人退出）。第6周时，干预组相比对照组显示出：1）抑郁症状（BDI-II）显著减少；2）生活质量（Q-LES-Q-SF）显著改善。依从性数据：生态瞬时评估（EMA）78%、生态瞬时干预（EMI）51%、日常任务65%。这些依从性水平与以往数字健康干预研究相当。

Conclusion: 基于治疗伴侣智能体（TCA）的数字干预能够有效改善年轻成年人的抑郁症状和生活质量，且依从性水平可接受。研究证明了TCA作为增强数字干预参与度的可行策略。未来需要进一步优化TCA设计，并进行更大规模的随机对照试验来验证其长期效果和机制。
...
中文标题: 针对有抑郁症状的年轻成年人的治疗伴侣智能体数字干预整合研究：概念验证研究

中文摘要: 背景：尽管数字干预对年轻成年人抑郁症具有临床有效性，但低参与度和依从性仍然是持续存在的挑战。建立强大的数字治疗联盟已被提出以解决这些障碍。本研究强调了基于对话式治疗伴侣智能体（TCA）干预设计的必要性。目标：本研究旨在开发一个基于Wizard-of-Oz的TCA中心原型，整合基于社会支持的生态瞬时评估（EMA）、生态瞬时干预（EMI）、行为激活和游戏化。我们评估了该干预对有抑郁症状的年轻成年人进行的为期六周的概念验证效果。方法：在线招募了20-39岁韩国年轻成年人，具有轻度至中度抑郁症状（PHQ-9）。干预组（n=29）接受了为期六周的基于TCA的数字干预，而对照组（n=29）在四周后招募，继续其常规生活。TCA指导四项日常行为激活任务、三次情绪评估、冥想、每日总结和每周任务反馈。两组在基线、第2周、第4周和第6周使用BDI-II、GAD-7和Q-LES-Q-SF进行评估。结果：58名参与者中，57人完成了研究（干预组1人退出）。在第6周，干预组显示出比对照组显著更大的抑郁症状减少和生活质量改善。依从性为EMA 78%、EMI 51%、日常任务65%。结论：基于TCA的数字干预改善了抑郁症状和生活质量，依从性水平与以往数字健康干预相当。未来研究应完善TCA设计并进行更大规模评估。

</details>


### [115] [Real-Time Multimodal Data Collection Using Smartwatches and Its Visualization in Education](https://arxiv.org/abs/2512.02651)
*Alvaro Becerra,Pablo Villegas,Ruth Cobos*

Main category: cs.HC

TL;DR: 本文开发了Watch-DMLT和ViSeDOPS两个工具，用于通过智能手表实时收集多模态教育数据并进行可视化分析，在65名学生的课堂部署中验证了系统的可行性和实用性。


<details>
  <summary>Details</summary>
Motivation: 可穿戴传感器在教育领域提供了研究认知和情感过程的新机会，但缺乏可扩展、同步和高分辨率的多模态数据采集工具，阻碍了多模态学习分析在教育环境中的广泛应用。

Method: 开发了两个互补工具：1) Watch-DMLT - 用于Fitbit Sense 2智能手表的数据采集应用，支持实时多用户生理和运动信号监测；2) ViSeDOPS - 基于仪表板的可视化系统，用于分析口语演示期间收集的同步多模态数据。

Result: 在65名学生和最多16个智能手表的课堂部署中，成功捕获并分析了包括心率、运动、注视、视频和上下文注释在内的数据流，证明了系统在真实学习环境中支持细粒度、可扩展和可解释的多模态学习分析的可行性和实用性。

Conclusion: 所提出的系统能够有效支持真实教育环境中的细粒度、可扩展和可解释的多模态学习分析，为教育技术研究提供了实用的工具和方法。
...
中文摘要: 可穿戴传感器（如智能手表）在医疗保健、体育和教育等领域日益普及，能够持续监测生理和行为数据。在教育背景下，这些技术为研究认知和情感过程（如参与度、注意力和表现）提供了新的机会。然而，缺乏可扩展、同步和高分辨率的多模态数据采集工具仍然是多模态学习分析在真实教育环境中广泛采用的重要障碍。本文提出了两个互补工具来解决这些挑战：Watch-DMLT，一个用于Fitbit Sense 2智能手表的数据采集应用，支持实时多用户生理和运动信号监测；以及ViSeDOPS，一个基于仪表板的可视化系统，用于分析在口语演示期间收集的同步多模态数据。我们报告了一个涉及65名学生和最多16个智能手表的课堂部署，其中捕获并分析了包括心率、运动、注视、视频和上下文注释在内的数据流。结果证明了所提出系统在真实学习环境中支持细粒度、可扩展和可解释的多模态学习分析的可行性和实用性。

</details>


### [116] [Perception of AI-Generated Music -- The Role of Composer Identity, Personality Traits, Music Preferences, and Perceived Humanness](https://arxiv.org/abs/2512.02785)
*David Stammer,Hannah Strauss,Peter Knees*

Main category: cs.HC

TL;DR: 研究发现对AI的态度是预测AI生成音乐喜好和情感强度的最佳指标，同时听众的伦理、文化和情境考虑也影响评价。


<details>
  <summary>Details</summary>
Motivation: AI生成艺术的快速发展引发了关于观众如何感知和评价此类作品的潜在偏见问题，需要研究作曲家信息和听众特征如何影响AI生成音乐的感知。

Method: 采用混合方法，使用两个AI音乐模型生成的各种流派的多样化音乐刺激集，考察感知作者身份对喜好和情感反应的影响，分析对AI的态度、人格特质、音乐相关变量、感知人性化，并进行开放式回答的主题分析。

Result: 对AI的态度是预测AI生成音乐喜好和情感强度的最佳预测因子；定性分析揭示了伦理、文化和情境考虑是听众评价AI生成音乐的重要标准。

Conclusion: 研究提供了人们体验AI生成音乐的细致视角，指出了未来人机交互中音乐感知研究的关键因素和方法学考虑，强调了对AI的态度在音乐评价中的核心作用。
...
中文标题: AI生成音乐的感知——作曲家身份、人格特质、音乐偏好与感知人性化的作用

中文摘要: AI生成艺术的迅速崛起引发了关于观众如何感知和评价此类作品的潜在偏见的争论。本研究采用混合方法，探讨作曲家信息和听众特征如何塑造AI生成音乐的感知。使用来自两个AI音乐模型的各种流派的多样化刺激集，我们考察了感知作者身份对喜好和情感反应的影响，并探讨了对AI的态度、人格特质和音乐相关变量如何影响评价。我们进一步评估了感知人性化的影响，并分析了开放式回答以揭示听众判断AI生成音乐的标准。对AI的态度被证明是预测AI生成音乐的喜好和情感强度的最佳预测因子。这一定量发现得到了我们主题分析中定性主题的补充，该分析确定了伦理、文化和情境考虑因素作为听众评价AI生成音乐的重要标准。我们的结果提供了人们如何体验AI工具创作音乐的细致视角，并为未来人机交互中音乐感知研究指出了关键因素和方法学考虑。

</details>


### [117] [Humans incorrectly reject confident accusatory AI judgments](https://arxiv.org/abs/2512.02848)
*Riccardo Loconte,Merylin Monaro,Pietro Pietrini,Bruno Verschuere,Bennett Kleinberg*

Main category: cs.HC

TL;DR: 研究发现人类会错误地拒绝AI的自信指控性判断，即使AI在谎言检测上比人类更准确。当AI模型表现出高置信度时，人们反而更倾向于偏离其预测，特别是当AI指控某人说谎时。


<details>
  <summary>Details</summary>
Motivation: 虽然AI在谎言检测方面已证明优于人类，且研究表明算法的透明度和可解释性有助于提高人类对AI决策的接受度，但人类在多大程度上会接受AI的谎言检测判断仍不清楚。本研究旨在探索AI模型的准确性和置信度如何影响人类对其判断的采纳。

Method: 研究采用实验方法，373名参与者被呈现具有高或低总体准确性的AI模型的真实性判断，以及不同程度的预测置信度。参与者需要评估AI对陈述的谎言检测预测，并决定是否采纳这些判断。

Result: 结果显示：1）人类更倾向于采纳高准确性模型的预测；2）有趣的是，模型越自信，人们越偏离其预测，特别是当模型预测欺骗时；3）人类与算法预测的互动要么降低了机器的性能，要么无效；4）这种对高置信度算法预测的厌恶部分源于参与者高估人类谎言检测能力。

Conclusion: 人类会错误地拒绝AI的自信指控性判断，即使AI在谎言检测上更准确。这种现象可以用真相默认理论和社会指控成本来解释。研究揭示了在敏感的社会判断任务中，人类对AI决策的接受存在心理障碍。
...
中文摘要: 使用人工智能方法进行自动言语欺骗检测已被证明在区分谎言和真相方面优于人类。研究表明，计算方法透明度和可解释性倾向于增加人类接受使用AI支持决策的程度。然而，人类在多大程度上接受AI的欺骗检测判断仍不清楚。我们通过实验研究了AI模型的准确性（即其在欺骗检测中的整体表现）和置信度（即模型对单个陈述预测的不确定性）如何影响人类对模型判断的采纳。参与者（n=373）被呈现具有高或低总体准确性的AI模型的真实性判断，以及不同程度的预测置信度。结果显示，人类更倾向于采纳高准确性模型的预测。有趣的是，模型越自信，人们越偏离其预测，特别是当模型预测欺骗时。我们还发现，人类与算法预测的互动要么恶化了机器的性能，要么无效。虽然这种人类不愿接受高置信度算法预测的现象部分可以通过参与者高估人类欺骗检测能力的倾向来解释，但我们也讨论了真相默认理论以及指控某人说谎的社会成本如何帮助解释这些发现。

</details>


### [118] [In Silico Development of Psychometric Scales: Feasibility of Representative Population Data Simulation with LLMs](https://arxiv.org/abs/2512.02910)
*Enrico Cipriani,Pavel Okopnyi,Danilo Menicucci,Simone Grassini*

Main category: cs.HC

TL;DR: LLM生成的模拟数据能复制群体水平的心理测量结构，但不能准确反映个体水平的数据特性，适用于早期原型设计而非个体验证。


<details>
  <summary>Details</summary>
Motivation: 传统心理测量量表开发需要大量样本、多阶段测试和资源投入，研究者希望探索LLM生成合成参与者数据的可行性，以在真实数据收集前进行计算机模拟试点，降低开发成本和时间。

Method: 通过四项预注册研究（每项N≈300），比较LLM模拟数据与真实数据集：1) 使用两个已验证量表比较LLM与人类数据；2) 在模拟数据上进行探索性因子分析创建新量表，然后检验这些结构在新收集人类样本中的推广性。

Result: 模拟数据在3/4研究中复制了预期因子结构，显示出构型和度量不变性，新量表实现了标量不变性。但相关测试显示真实与合成数据存在显著差异，分数分布和方差不一致。LLM能捕捉群体水平结构，但不能近似个体水平特性。

Conclusion: LLM生成的数据适用于早期阶段、群体水平的心理测量原型设计，但不能替代个体水平的验证。需要考虑方法学限制、偏见风险、数据污染和伦理问题。
...
中文标题: 心理测量量表的计算机模拟开发：利用大语言模型进行代表性人口数据模拟的可行性研究

中文摘要: 开发和验证心理测量量表需要大样本、多阶段测试和大量资源。大语言模型（LLMs）的最新进展使得通过提示模型以特定人口统计学特征个体的身份回答项目来生成合成参与者数据成为可能，这可能在真实数据收集之前实现计算机模拟试点。在四项预注册研究中（每项N≈300），我们测试了LLM模拟数据集是否能复制人类反应的潜在结构和测量特性。在研究1-2中，我们比较了LLM生成的数据与两个已验证量表的真实数据集；在研究3-4中，我们使用模拟数据上的探索性因子分析创建了新量表，然后检验这些结构是否推广到新收集的人类样本。模拟数据集在四项研究中的三项中复制了预期的因子结构，并显示出一致的构型和度量不变性，两个新开发的量表实现了标量不变性。然而，基于相关性的测试揭示了真实数据集与合成数据集之间的显著差异，并且在分数分布和方差方面出现了明显的不一致。因此，虽然LLMs捕捉了群体水平的潜在结构，但它们并未近似个体水平的数据特性。模拟数据集还显示出跨性别的完全内部不变性。总体而言，LLM生成的数据对于早期阶段、群体水平的心理测量原型设计似乎有用，但不能作为个体水平验证的替代品。我们讨论了与计算机心理测量模拟相关的方法学限制、偏见风险和数据污染以及伦理考虑。
...
TL;DR: 该论文提出将力感知融入强化学习，以提升机器人在复杂操作任务中的安全性和可靠性，特别是在仿真到现实的迁移中。


<details>
  <summary>Details</summary>
Motivation: 在非结构化环境中执行复杂操作任务时，传统手工编码方法效果不佳，而强化学习虽然能提供更通用的策略，但其稳定性和可靠性难以保证，存在安全隐患。此外，从仿真到现实的迁移也会带来不可预测的情况。

Method: 将力和触觉感知引入强化学习框架。力感和触觉在机器人动态控制和人机交互中起关键作用，通过基于力的强化学习方法使机器人更能适应环境变化。

Result: 在物体推动任务中，实验结果表明该策略在仿真和现实世界中都更安全、更高效。基于力的强化学习方法在仿真到现实迁移中表现出更好的适应性。

Conclusion: 将力感知融入强化学习能显著提升机器人的安全性和可靠性，特别是在仿真到现实迁移中。该方法在物体推动任务中表现出优越性能，具有广泛的机器人应用前景。
...
中文摘要: 对于非结构化环境中的复杂操作任务，传统手工编码方法效果不佳，而强化学习能提供更通用、有用的策略。尽管强化学习能取得令人印象深刻的结果，但其稳定性和可靠性难以保证，可能造成安全隐患。此外，从仿真到现实的迁移也会导致不可预测的情况。为增强机器人的安全性和可靠性，我们将力和触觉感知引入强化学习。力和触觉感知在机器人动态控制和人机交互中起关键作用。我们证明基于力的强化学习方法能更好地适应环境，特别是在仿真到现实的迁移中。实验结果表明，在物体推动任务中，我们的策略在仿真和现实世界中都更安全、更高效，因此具有广泛的机器人应用前景。
...
TL;DR: VIGS-SLAM是一个结合视觉和惯性测量的3D高斯泼溅SLAM系统，通过紧密耦合IMU数据解决纯视觉SLAM在运动模糊、低纹理等场景下的性能下降问题，实现鲁棒实时跟踪和高保真重建。


<details>
  <summary>Details</summary>
Motivation: 现有基于3D高斯泼溅的纯视觉SLAM方法虽然在密集和照片级真实感地图构建方面表现出色，但在运动模糊、低纹理和曝光变化等挑战性场景下性能显著下降。需要结合惯性测量单元(IMU)数据来提高系统的鲁棒性和稳定性。

Method: 1. 在统一优化框架内紧密耦合视觉和惯性线索
2. 联合优化相机位姿、深度和IMU状态
3. 采用鲁棒的IMU初始化策略
4. 实现时变偏差建模
5. 包含具有一致高斯更新的闭环检测机制

Result: 在四个具有挑战性的数据集上的实验表明，VIGS-SLAM在鲁棒性和重建质量方面优于现有的最先进方法，特别是在运动模糊、低纹理和曝光变化等困难场景下表现出色。

Conclusion: VIGS-SLAM成功地将视觉和惯性测量紧密耦合到3D高斯泼溅SLAM框架中，显著提高了系统在挑战性环境下的鲁棒性，同时保持了高保真重建能力，为实时SLAM系统提供了更可靠的解决方案。
...
中文摘要: 我们提出了VIGS-SLAM，一个视觉惯性3D高斯泼溅SLAM系统，实现了鲁棒的实时跟踪和高保真重建。尽管最近的3DGS-based SLAM方法实现了密集和照片级真实感的地图构建，但其纯视觉设计在运动模糊、低纹理和曝光变化下性能下降。我们的方法在统一优化框架内紧密耦合视觉和惯性线索，联合优化相机位姿、深度和IMU状态。它具有鲁棒的IMU初始化、时变偏差建模和具有一致高斯更新的闭环检测。在四个具有挑战性的数据集上的实验证明了我们相对于最先进方法的优越性。项目页面：https://vigs-slam.github.io
...
TL;DR: VDD方法将自车动力学与环境转移动力学解耦，通过PAD和PAT策略增强鲁棒性，在自动驾驶中显著提升性能和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有世界模型通常从图像输入中联合学习自车动力学和环境转移动力学，导致效率低下且对车辆动力学变化缺乏鲁棒性。需要一种能有效泛化到不同车辆参数的方法。

Method: 提出车辆动力学嵌入Dreamer（VDD）方法，将自车动力学建模与环境转移动力学解耦。引入两种增强策略：部署期间策略调整（PAD）和训练期间策略增强（PAT）。

Result: 在模拟环境中的综合实验表明，该模型显著提高了驾驶性能和对车辆动力学变化的鲁棒性，优于现有方法。

Conclusion: VDD方法通过解耦自车动力学和环境动力学，结合PAD和PAT策略，为自动驾驶世界模型提供了更高效、更鲁棒的解决方案。
...
中文摘要: 世界模型作为一种有前景的自动驾驶方法受到了广泛关注。通过模拟人类感知和决策过程，这些模型能够预测和适应动态环境。现有方法通常将高维观测映射到紧凑的潜在空间，并在这些潜在表示中学习最优策略。然而，先前的工作通常从图像输入中联合学习自车动力学和环境转移动力学，导致效率低下且对车辆动力学变化缺乏鲁棒性。为解决这些问题，我们提出了车辆动力学嵌入Dreamer（VDD）方法，该方法将自车动力学建模与环境转移动力学解耦。这种分离使世界模型能够有效地泛化到具有不同参数的车辆。此外，我们引入了两种策略来进一步增强学习策略的鲁棒性：部署期间策略调整（PAD）和训练期间策略增强（PAT）。在模拟环境中的综合实验表明，所提出的模型显著提高了驾驶性能和对车辆动力学变化的鲁棒性，优于现有方法。
...
TL;DR: AID是一个基于扩散模型的多智能体信息路径规划框架，通过非自回归方式生成长时程轨迹，结合行为克隆和强化学习微调，实现了比传统方法更快的执行速度和更高的信息增益。


<details>
  <summary>Details</summary>
Motivation: 多智能体信息路径规划（MAIPP）在大规模环境监测和搜索救援等场景中至关重要，但现有自回归意图预测方法存在计算成本高和累积误差的问题，需要更高效的协调机制。

Method: 提出AID框架：1）使用扩散模型以非自回归方式生成长时程轨迹；2）采用两阶段训练：首先对专家轨迹进行行为克隆，然后通过DPPO（扩散策略策略优化）进行强化学习微调。

Result: AID相比其训练的MAIPP规划器实现了显著改进：执行速度提升高达4倍，信息增益增加17%，并且能够有效扩展到更多智能体。

Conclusion: 扩散模型为多智能体信息路径规划提供了有效的非自回归意图生成方法，AID框架通过结合行为克隆和强化学习，实现了高效协调和性能提升，为大规模多智能体系统提供了实用解决方案。
...
中文摘要: 在大规模或时间紧迫的场景（如环境监测、搜索救援）中进行信息收集需要在有限的时间预算内实现广泛覆盖，这促使了多智能体系统的使用。这些场景通常被表述为多智能体信息路径规划（MAIPP），其中多个智能体必须在预算约束下协调工作以最大化信息增益。MAIPP中的一个核心挑战是在环境信念随着测量结果不断演变的情况下确保有效的协调。最近基于学习的方法通过使用未来位置的分布作为"意图"来支持协调来解决这个问题。然而，这些自回归意图预测器计算成本高昂且容易产生累积误差。受扩散模型作为表达能力强的长时程策略的有效性启发，我们提出了AID，一个完全去中心化的MAIPP框架，利用扩散模型以非自回归方式生成长时程轨迹。AID首先对现有MAIPP规划器生成的轨迹进行行为克隆，然后通过扩散策略策略优化（DPPO）使用强化学习对策略进行微调。这种两阶段管道使策略能够继承专家行为，同时通过在线奖励反馈学习改进的协调。实验表明，AID始终优于其训练的MAIPP规划器，实现了高达4倍的执行速度提升和17%的信息增益增加，同时能够有效地扩展到更多智能体。我们的实现可在https://github.com/marmotlab/AID公开获取。
...
TL;DR: 本文提出了机器人能力框架作为边界对象和中间层知识产物，用于跨学科共同设计机器人流程，通过关注高级能力而非技术细节，促进技术人员与工人之间的对话，以确定任务分配。


<details>
  <summary>Details</summary>
Motivation: 随着机器人变得更加适应性强、响应灵敏且能够与人类互动，有效的人机协作设计变得至关重要。然而，当前设计过程通常由单一学科主导，往往忽视了跨学科知识和最终与机器人共享任务的工人的经验知识。需要一种工具来促进跨学科合作，有意义地塑造工作场所整合机器人系统时的未来工作。

Method: 通过反思性和迭代过程开发机器人能力框架，创建了一个词汇表，关注高级能力而非机器人内部工作原理。该框架在两个不同场景中应用：1) 让机器人专家使用该词汇描述现有商业机器人；2) 与从事机器人相关项目的学生进行设计活动。

Result: 该框架被证明是一个有效的中间层知识产物和边界对象，能够桥接技术领域和经验领域。它支持围绕任务分配的对话，指导设计者，赋能工人，并有助于创造更公正和协作的未来工作环境。

Conclusion: 机器人能力框架作为边界对象和中间层知识产物，成功促进了跨学科合作，使技术人员和工人能够共同设计机器人流程，确定任务分配，为实现更公正和协作的未来工作环境做出了贡献。
...
中文标题: 机器人能力框架：作为边界对象和中间层知识产物的机器人流程协同设计工具

中文摘要: 随着机器人变得更加适应性强、响应灵敏且能够与人类互动，有效的人机协作设计变得至关重要。然而，这种设计过程通常由单一学科方法主导，往往忽视了跨学科知识和最终与这些系统共享任务的工人的经验知识。为了解决这一差距，我们引入了机器人能力框架，这是一个词汇表，使跨学科合作能够有意义地塑造工作场所整合机器人系统时的未来工作。该框架不关注机器人的内部工作原理，而是将讨论集中在高级能力上，支持围绕任务的哪些元素应保持由人类主导、哪些可以委托给机器人的对话。我们通过反思性和迭代过程开发了该框架，并在两个不同的场景中应用了它：通过让机器人专家使用其词汇描述现有商业机器人，以及通过与从事机器人相关项目的学生进行设计活动。该框架作为一个中间层知识产物和边界对象出现，桥接了技术领域和经验领域，指导设计者，赋能工人，并有助于创造更公正和协作的未来工作环境。
...
TL;DR: SAM2Grasp通过将多模态抓取问题转化为提示条件的时序动作预测，利用冻结的SAM2模型进行视觉跟踪，仅训练轻量级动作头，实现无歧义的抓取轨迹生成。


<details>
  <summary>Details</summary>
Motivation: 模仿学习在多物体抓取场景中存在多模态问题：多个有效抓取目标导致冲突的训练信号，传统方法会平均这些动作产生无效结果，需要解决这种歧义性问题。

Method: 1. 使用冻结的SAM2模型提取时序视觉特征；2. 设计轻量级可训练动作头与SAM2分割头并行；3. 通过初始提示（如边界框）指定目标物体；4. 利用SAM2的时序跟踪能力持续跟踪目标；5. 仅训练动作头在预计算特征上学习抓取轨迹。

Result: 在杂乱、多物体抓取任务中实现了最先进的性能，有效解决了多模态歧义问题，能够为指定物体生成明确、稳定的抓取轨迹。

Conclusion: SAM2Grasp通过提示条件的时序动作预测框架成功解决了模仿学习中的多模态问题，利用预训练视觉模型的跟踪能力，仅需少量参数训练即可实现高效、无歧义的多物体抓取。
...
中文摘要: 机器人抓取的模仿学习经常受到多模态问题的困扰：当场景包含多个有效目标时，抓取不同物体的演示会产生冲突的训练信号。标准的模仿学习策略会将这些不同的动作平均成一个单一、无效的动作而失败。在本文中，我们介绍了SAM2Grasp，这是一个新颖的框架，通过将任务重新表述为单模态、提示条件的预测问题来解决这个问题。我们的方法利用冻结的SAM2模型来使用其强大的视觉时序跟踪能力，并引入了一个轻量级的、可训练的动作头，与其原生分割头并行运行。这种设计允许仅在SAM2预计算的时序视觉特征上训练小动作头。在推理过程中，初始提示（例如由上游物体检测模型提供的边界框）指定要抓取的具体物体。这个提示条件使动作头能够为该物体单独预测一个独特、明确的抓取轨迹。在随后的所有视频帧中，SAM2内置的时序跟踪能力自动保持对所选物体的稳定跟踪，使我们的模型能够从视频流中持续预测抓取轨迹，无需进一步的外部指导。这种时序提示方法有效地消除了视觉运动策略中的歧义。我们通过大量实验证明，SAM2Grasp在杂乱、多物体抓取任务中实现了最先进的性能。
...
TL;DR: RoboWheel是一个数据引擎，能将真实世界的人类手部物体交互视频转换为跨形态机器人学习的训练数据，通过物理约束优化、重定向到不同机器人形态，并利用仿真增强扩展数据覆盖范围。


<details>
  <summary>Details</summary>
Motivation: 当前机器人学习需要大量高质量的演示数据，但传统遥操作数据采集成本高、扩展性差。人类手部物体交互视频丰富且易于获取，但缺乏将这类数据有效转换为机器人可执行动作的方法。

Method: 1. 从单目RGB或RGB-D输入进行高精度手部物体交互重建；2. 使用强化学习优化器在接触和穿透约束下细化相对位姿；3. 将重建的接触丰富轨迹重定向到不同机器人形态；4. 在Isaac Sim中构建仿真增强框架，通过领域随机化扩展数据分布。

Result: 验证表明，该管道生成的轨迹与遥操作数据同样稳定，能带来持续的性能提升。这是首次定量证明手部物体交互模态可作为机器人学习的有效监督信号。相比遥操作，RoboWheel更轻量，仅需单目RGB(D)相机即可提取通用的、形态无关的运动表示。

Conclusion: RoboWheel提供了一个端到端的数据管道，能够将人类演示视频转换为跨形态机器人学习的有效训练数据，为利用丰富的人类交互视频资源进行机器人学习开辟了新途径。
...
中文标题: RoboWheel：从真实世界人类演示到跨形态机器人学习的数据引擎

中文摘要: 我们介绍了RoboWheel，这是一个数据引擎，能够将人类手部物体交互视频转换为跨形态机器人学习的训练就绪监督信号。从单目RGB或RGB-D输入中，我们执行高精度的手部物体交互重建，并通过强化学习优化器在接触和穿透约束下细化手部物体相对位姿来确保物理合理性。重建的接触丰富轨迹随后被重定向到跨形态机器人（包括简单末端执行器的机械臂、灵巧手和人形机器人），产生可执行的动作和轨迹。为了扩大覆盖范围，我们在Isaac Sim上构建了仿真增强框架，通过多样化的领域随机化（形态、轨迹、物体检索、背景纹理、手部运动镜像）来丰富轨迹和观测的分布，同时保持空间关系和物理合理性。整个数据管道形成了从视频、重建、重定向到增强数据采集的端到端流程。我们在主流的视觉语言动作和模仿学习架构上验证了数据，证明我们的管道生成的轨迹与遥操作数据同样稳定，并能带来可比的持续性能提升。据我们所知，这首次提供了定量证据，表明手部物体交互模态可以作为机器人学习的有效监督信号。与遥操作相比，RoboWheel更轻量，单个单目RGB(D)相机就足以提取通用的、形态无关的运动表示，可以灵活地重定向到不同形态。我们进一步组装了一个大规模多模态数据集，结合多相机捕捉、单目视频和公共手部物体交互语料库，用于训练和评估具身模型。
...
TL;DR: CogDrive提出了一种认知驱动的多模态预测-规划融合框架，通过显式的模态推理和安全感知轨迹优化，解决混合交通中自动驾驶的安全问题。
...
Method: 预测模块采用基于拓扑运动语义和最近邻关系编码的交互模态认知表示，通过可微分模态损失和多模态高斯解码学习稀疏不平衡交互行为。规划模块引入应急响应概念，优化安全稳定轨迹，短期一致分支确保重规划期间安全，长期分支支持低概率切换模态下的平滑无碰撞运动。
...
中文标题: CogDrive：面向安全自动驾驶的认知驱动多模态预测-规划融合

中文摘要: 混合交通中的安全自动驾驶需要统一理解多模态交互和不确定性下的动态规划。现有基于学习的方法难以捕捉罕见但安全关键的行为，而基于规则的系统在复杂交互中缺乏适应性。为应对这些限制，CogDrive引入了一种认知驱动的多模态预测和规划框架，将显式模态推理与安全感知轨迹优化相结合。预测模块采用基于拓扑运动语义和最近邻关系编码的交互模态认知表示。通过可微分模态损失和多模态高斯解码，CogDrive学习稀疏不平衡交互行为并改进长时域轨迹预测。规划模块引入应急响应概念并优化安全稳定轨迹，其中短期一致分支确保重规划周期内的安全，长期分支支持低概率切换模态下的平滑无碰撞运动。在Argoverse2和INTERACTION数据集上的实验表明，CogDrive在轨迹精度和漏检率方面表现优异，而闭环仿真验证了在汇入和交叉口场景中的自适应行为。通过结合认知多模态预测和安全导向规划，CogDrive为复杂交通中的安全自动驾驶提供了一个可解释且可靠的范式。
...
TL;DR: 提出了ViFailback框架，通过视觉符号诊断机器人操作失败并提供纠正指导，包含大规模真实世界数据集和评估基准，构建的ViFailback-8B模型显著提升了失败诊断和纠正能力。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在机器人操作方面取得进展，但在失败诊断和从失败中学习方面能力有限；现有失败数据集多为模拟生成，缺乏真实世界泛化能力；需要更有效的失败诊断和纠正框架。

Method: 1. 提出ViFailback框架，利用显式视觉符号增强标注效率；2. 构建ViFailback数据集（58,126个VQA对，5,202个真实世界操作轨迹）；3. 建立ViFailback-Bench基准（11个细粒度VQA任务）；4. 开发ViFailback-8B VLM模型；5. 集成ViFailback-8B与VLA模型进行真实世界实验。

Result: 1. ViFailback-8B在基准测试中实现显著性能提升；2. 模型能生成用于纠正指导的视觉符号；3. 真实世界实验证明框架能有效帮助VLA模型从失败中恢复；4. 提供了大规模真实世界失败诊断数据集和评估基准。

Conclusion: ViFailback框架通过视觉符号有效解决了机器人操作失败诊断和纠正问题，提供的大规模真实世界数据集和评估基准为相关研究提供了重要资源，集成方法展示了在实际机器人系统中的实用价值。
...
中文摘要: 视觉-语言-动作（VLA）模型最近在机器人操作方面取得了显著进展，但在失败诊断和从失败中学习方面仍然有限。此外，现有的失败数据集大多是在模拟中程序化生成的，这限制了它们对现实世界的泛化能力。有鉴于此，我们引入了ViFailback框架，旨在诊断机器人操作失败并提供文本和视觉纠正指导。我们的框架利用显式视觉符号来增强标注效率。我们进一步发布了ViFailback数据集，这是一个包含58,126个视觉问答（VQA）对及其对应的5,202个真实世界操作轨迹的大规模集合。基于该数据集，我们建立了ViFailback-Bench基准，包含11个细粒度VQA任务，旨在评估视觉语言模型（VLM）的失败诊断和纠正能力，其中ViFailback-Bench Lite用于封闭式评估，ViFailback-Bench Hard用于开放式评估。为了展示我们框架的有效性，我们构建了ViFailback-8B VLM，该模型不仅在ViFailback-Bench上实现了显著的总体性能提升，还能生成用于纠正动作指导的视觉符号。最后，通过将ViFailback-8B与VLA模型集成，我们进行了真实世界机器人实验，展示了其帮助VLA模型从失败中恢复的能力。项目网站：https://x1nyuzhou.github.io/vifailback.github.io/
...
TL;DR: TACO是一个测试时缩放框架，通过轻量级伪计数估计器验证动作块，使VLA模型能够从所有采样的动作块中选择具有最大伪计数的动作，防止分布偏移同时保持泛化能力。


<details>
  <summary>Details</summary>
Motivation: VLA模型在预训练阶段整合了多样化的数据模式，而微调数据集通常包含在运动学上不理想的方式收集的演示数据，存在与下游任务成功动作模式无关的冗余动作模式。这导致在监督微调后，各种采样噪声在推理时出现脆弱性。

Method: 提出TACO（测试时缩放框架），使用轻量级伪计数估计器作为动作块的高保真验证器。VLA模型集成TACO后，可以从所有采样的动作块中执行具有最大伪计数的动作，防止分布偏移同时保持泛化能力。该方法类似于离线强化学习中的反探索原则，且无需梯度计算。

Result: 在四个仿真基准（RoboTwin2.0、Robotwin、LIBERO、SimplerEnv）和一个双臂平台上的广泛实验表明，该方法显著提高了下游任务适应中的推理稳定性和成功率。

Conclusion: TACO框架通过测试时缩放方法有效解决了VLA模型在推理时的脆弱性问题，防止了分布偏移，同时保持了模型的泛化能力，为基于流或扩散的VLA模型提供了一种计算高效的改进方案。
...
中文摘要: 通过流匹配或扩散目标训练的视觉-语言-动作（VLA）模型擅长从大规模多模态数据集（如人类遥操作、脚本策略）中学习复杂行为。然而，由于VLA在预训练阶段整合了多样化的数据模式，而微调数据集通常包含以运动学上不理想或不可取的方式收集的演示数据，存在与下游任务成功动作模式无关的冗余动作模式。具体而言，我们观察到在预训练VLA的监督微调后，各种采样噪声在推理时存在关键脆弱性。在本文中，我们将这种不稳定性归因于VLA策略与下游任务数据集稳定成功模式所诱导的策略之间的分布偏移。因此，我们提出了TACO，一个测试时缩放（TTS）框架，应用轻量级伪计数估计器作为动作块的高保真验证器。集成TACO的VLA模型可以从所有采样的动作块中执行具有最大伪计数的动作，从而防止分布偏移，同时保持VLA的泛化能力，因为约束仅在推理时应用。我们的方法类似于离线强化学习（RL）中的经典反探索原则，并且由于无需梯度计算，与RL更新相比具有显著的计算优势，特别是对于基于流或扩散的VLA模型，这些模型由于去噪过程而难以执行RL更新。在四个仿真基准（RoboTwin2.0、Robotwin、LIBERO、SimplerEnv）和一个双臂平台上的广泛实验表明，我们的方法显著提高了下游任务适应中的推理稳定性和成功率。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶系统的安全部署需要全面测试，但现实世界中能够暴露系统漏洞的安全关键场景极为稀少。现有方法在构建长尾场景时面临保真度、关键性和交互性的平衡挑战，且缺乏对被测车辆的实时动态响应能力。

Method: 提出三层分层架构：1）战略层使用VLM确定场景生成目标；2）战术层制定引导函数；3）操作层执行引导扩散。首先建立学习真实驾驶场景分布的基础扩散模型，然后设计自适应引导扩散方法实现背景车辆的实时精确控制，最后通过VLM的深度场景理解和风险推理指导扩散模型生成场景。

Result: 实验结果表明，该方法能够高效生成真实、多样且高度交互的安全关键测试场景。案例研究验证了方法的适应性和VLM指导的生成性能。

Conclusion: 提出的VLM与引导扩散结合框架有效解决了自动驾驶安全关键场景生成的挑战，实现了高效、自适应且交互性强的场景生成，为自动驾驶系统测试提供了有力工具。
...
中文摘要: 自动驾驶系统（ADS）的安全部署依赖于全面的测试和评估。然而，能够有效暴露系统漏洞的安全关键场景在现实世界中极为稀疏。现有的场景生成方法在高效构建确保保真度、关键性和交互性的长尾场景方面面临挑战，尤其缺乏对被测车辆（VUT）的实时动态响应能力。为解决这些挑战，本文提出了一种安全关键测试场景生成框架，将视觉语言模型（VLM）的高层语义理解能力与自适应引导扩散模型的细粒度生成能力相结合。该框架建立了三层分层架构，包括用于VLM指导的场景生成目标确定的战略层、用于引导函数制定的战术层以及用于引导扩散执行的操作层。我们首先建立了一个高质量的基础扩散模型，学习真实驾驶场景的数据分布。接下来，我们设计了一种自适应引导扩散方法，能够在闭环仿真中实现对背景车辆（BVs）的实时精确控制。然后引入VLM，通过深度场景理解和风险推理自主生成场景生成目标和引导函数，最终引导扩散模型实现VLM指导的场景生成。实验结果表明，所提出的方法能够高效生成真实、多样且高度交互的安全关键测试场景。此外，案例研究验证了所提出方法的适应性和VLM指导的生成性能。
...
TL;DR: SwarmDiffusion是一个轻量级端到端扩散模型，从单张RGB图像联合预测可通行性并生成可行轨迹，无需手动提示或外部规划器，支持异构机器人导航。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的可通行性估计方法依赖手工提示，跨机器人平台泛化能力差，且仅输出可通行性地图，需要外部规划器生成轨迹，速度慢且不统一。

Method: 提出无规划器的轨迹构建流程：随机化航点采样、贝塞尔平滑和正则化（连通性、安全性、方向性、路径细度）。使用扩散模型联合预测可通行性和轨迹，通过紧凑的机器人状态编码实现跨平台适应。

Result: 在室内环境和两种机器人平台（四足和空中）上实现80-100%导航成功率，推理时间仅0.09秒。仅需500个额外视觉样本即可适应新机器人，在仿真和真实环境中可靠泛化。

Conclusion: SwarmDiffusion提供了一种可扩展、无需提示的统一方法，用于可通行性推理和轨迹生成，支持异构机器人导航，实现了端到端的物理一致性路径生成。
...
中文标题: SwarmDiffusion：面向异构机器人平台无关导航的端到端可通行性引导扩散方法

中文摘要: 视觉可通行性估计对于自主导航至关重要，但现有的基于视觉语言模型的方法依赖手工提示，跨机器人平台泛化能力差，且仅输出可通行性地图，将轨迹生成留给缓慢的外部规划器。我们提出SwarmDiffusion，一个轻量级端到端扩散模型，从单张RGB图像联合预测可通行性并生成可行轨迹。为了消除对标注或规划器生成路径的需求，我们引入了基于随机化航点采样、贝塞尔平滑和正则化（强制连通性、安全性、方向性和路径细度）的无规划器轨迹构建流程。这使得无需演示即可学习稳定的运动先验。SwarmDiffusion利用视觉语言模型衍生的监督而无需提示工程，并通过紧凑的机器人状态编码条件化扩散过程，生成物理一致、可通行的路径，可在不同机器人平台间迁移。在室内环境和两种机器人平台（四足和空中）上，该方法实现了80-100%的导航成功率和0.09秒的推理时间，仅需500个额外视觉样本即可适应新机器人。该方法在仿真和真实世界试验中可靠地泛化到未见环境，为统一的可通行性推理和轨迹生成提供了一种可扩展、无需提示的方法。
...
TL;DR: VLA模型的空间建模错位是泛化能力差的主要原因，而非物理建模问题。通过一次性轻量级视觉表示校准，使用FTM和FLA方法，仅需少量参数即可显著提升模型在新视角下的性能。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作（VLA）模型虽然在分布内表现良好，但在新相机视角和视觉扰动下性能急剧下降。研究发现这种脆弱性主要源于空间建模的错位，而非物理建模问题，因此需要解决视觉表示的校准问题。

Method: 提出了两种一次性适应框架：1）特征令牌调制（FTM），对视觉令牌应用全局仿射变换，仅需4K参数；2）特征线性适应（FLA），在ViT编码器中引入低秩更新，使用4.7M参数。两种方法都通过轻量级学习更新来重新校准视觉表示。

Result: FTM将Libero视角准确率从48.5%提升至87.1%；FLA达到90.8%的成功率，与LoRA规模微调相当但成本更低。结果表明预训练VLA模型具有大量未开发的鲁棒性。

Conclusion: VLA模型的泛化能力比想象中更强，空间建模错位是主要瓶颈。通过有针对性的最小视觉适应就足以恢复视角泛化能力，这为VLA模型的实用部署提供了高效解决方案。
...
中文摘要: 视觉-语言-动作（VLA）模型在分布内表现优异，但在新相机视角和视觉扰动下性能急剧下降。我们发现这种脆弱性主要源于空间建模的错位，而非物理建模问题。为此，我们提出了一个一次性适应框架，通过轻量级、可学习的更新来重新校准视觉表示。我们的第一种方法——特征令牌调制（FTM），对视觉令牌应用全局仿射变换，仅用4K参数就将Libero视角准确率从48.5%提升至87.1%。在此基础上，特征线性适应（FLA）在ViT编码器中引入低秩更新，使用4.7M参数实现了90.8%的成功率——以远低于LoRA规模微调的成本达到同等效果。这些结果表明，预训练VLA模型中存在大量未开发的鲁棒性，并且有针对性的最小视觉适应足以恢复视角泛化能力。
...
TL;DR: 该研究开发了一个3自由度串并联混合机器人手指，实现了指尖任务空间轨迹的精确跟踪控制，达到毫米级精度


<details>
  <summary>Details</summary>
Motivation: 机器人手指的任务空间控制是实现灵巧操作的关键，因为操作目标通常以指尖运动和施加的力来定义，而不是单个关节角度。虽然任务空间规划和控制已在较大的臂级机械臂中得到广泛研究，但在紧凑的多自由度机器人手指中实现精确任务空间轨迹跟踪的演示仍然很少。

Method: 1. 物理原型设计和制造了一个三自由度、连杆驱动的串并联机器人手指
2. 建立了分析正向运动学和封闭形式的雅可比矩阵
3. 实现了解析运动速率控制（RMRC）方案，用于闭环任务空间轨迹跟踪
4. 实验评估了指尖在各种轨迹上的跟踪性能，包括直线、圆形和更复杂的曲线

Result: 实验结果表明，该机器人手指能够实现毫米级的精确任务空间轨迹跟踪。这是首次系统性地展示连杆驱动机器人手指的精确任务空间轨迹跟踪性能，为未来灵巧手内操作设计建立了基准。

Conclusion: 本研究成功实现了3自由度串并联混合机器人手指的精确任务空间轨迹跟踪控制，填补了紧凑型多自由度机器人手指在任务空间控制方面的空白，为未来灵巧操作系统的开发提供了重要参考。
...
中文标题: 3自由度串并联混合机器人手指指尖轨迹跟随的实验特性分析

中文摘要: 机器人手指的任务空间控制是实现灵巧操作的关键推动因素，因为操作目标最自然地以指尖运动和施加的力来指定，而不是单个关节角度。虽然任务空间规划和控制已在较大的臂级机械臂中得到广泛研究，但在紧凑的多自由度机器人手指中实现精确任务空间轨迹跟踪的演示仍然很少。在本文中，我们展示了一个三自由度、连杆驱动、串并联机器人手指的物理原型设计和实验特性分析，该手指具有分析正向运动学和封闭形式的雅可比矩阵。实现了解析运动速率控制（RMRC）方案以实现闭环任务空间轨迹跟踪。我们通过实验评估了指尖在各种轨迹上的跟踪性能，包括直线、圆形和更复杂的曲线，并报告了毫米级的精度。据我们所知，这项工作首次系统性地展示了连杆驱动机器人手指的精确任务空间轨迹跟踪实验，从而为未来旨在实现灵巧手内操作的设计建立了基准。

</details>


### [133] [Video2Act: A Dual-System Video Diffusion Policy with Robotic Spatio-Motional Modeling](https://arxiv.org/abs/2512.03044)
*Yueru Jia,Jiaming Liu,Shengbang Liu,Rui Zhou,Wanhe Yu,Yuyang Yan,Xiaowei Chi,Yandong Guo,Boxin Shi,Shanghang Zhang*

Main category: cs.RO

TL;DR: Video2Act是一个利用视频扩散模型（VDM）提取空间和运动表示来指导机器人动作学习的双系统框架，通过异步设计平衡了感知精度和推理效率，在模拟和真实任务中显著超越了现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于视频扩散模型（VDM）的机器人策略方法未能充分利用VDM中固有的、跨帧一致的物理运动表示，这限制了策略对动态环境的理解和动作生成的物理一致性。同时，VDM的高计算成本导致推理效率低下，难以满足实时控制需求。

Method: 1. 从预训练的VDM中提取前景边界和帧间运动变化作为精炼的空间-运动表示，过滤背景噪声和无关偏差。2. 将这些表示作为条件输入到一个扩散变换器（DiT）动作头中，指导动作生成。3. 提出异步双系统架构：慢速的VDM（系统2）负责高层次的感知和表示提取；快速的DiT动作头（系统1）利用这些表示实时生成动作，从而平衡了精度和效率。

Result: 在模拟任务中，Video2Act的平均成功率比之前最先进的视觉语言动作（VLA）方法高出7.7%；在现实世界任务中，平均成功率高出21.7%。该框架还表现出强大的泛化能力，并且在VDM低频更新的情况下仍能保持稳定的操作性能。

Conclusion: Video2Act通过显式利用VDM中编码的时空运动表示，并采用异步双系统设计，有效地将高级感知与实时动作生成解耦，显著提升了机器人策略的性能、效率和泛化能力，为基于视频模型的机器人学习提供了一个有前景的方向。
...
中文摘要: 鲁棒的感知和动力学建模是现实世界机器人策略学习的基础。最近的方法采用视频扩散模型（VDMs）来增强机器人策略，改善其对物理世界的理解和建模。然而，现有方法忽视了VDMs中固有的跨帧编码的一致且物理一致的运动表示。为此，我们提出了Video2Act，这是一个通过显式整合空间和运动感知表示来有效指导机器人动作学习的框架。基于VDMs的固有表示，我们提取前景边界和帧间运动变化，同时过滤掉背景噪声和任务无关的偏差。这些精炼的表示随后被用作扩散变换器（DiT）动作头的额外条件输入，使其能够推理要操作什么以及如何移动。为了缓解推理效率低下的问题，我们提出了一种异步双系统设计，其中VDM作为慢速系统2，DiT头作为快速系统1，协同工作以生成自适应动作。通过向系统1提供运动感知条件，即使VDM更新频率较低，Video2Act也能保持稳定的操作。在评估中，Video2Act在模拟任务中比先前最先进的VLA方法平均成功率高出7.7%，在现实世界任务中高出21.7%，进一步展现出强大的泛化能力。
